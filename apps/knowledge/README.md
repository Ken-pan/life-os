...
| 项目现状 `/projects` | 自动感知项目笔记现状：融合 Vault frontmatter + git 活动（`senseGitActivity`）+ Planner 项目/任务只读快照（`cloud.svelte.js`），漂移一键写回 frontmatter，可一键生成「📡 项目现状（自动）」汇总笔记 | `.list` / `.badge` / `.chip` |
...
**除检索外，local-ai 侧还跑一整套会自动读写 Vault 的策展流水线**（每日摘要 / 主题折叠 /
周月汇总 / 园丁去重合并 / 人物笔记与看板重建，launchd 定时触发，含一处不可逆的自动删除操作）。
详细任务清单、触发时机、风险点与实际运行日志见
[AI_AUTOMATION_AUDIT.md](./AI_AUTOMATION_AUDIT.md)。
...
1. **云同步**：接 `@life-os/sync` 统一 Supabase（LWW + 墓碑，同 aios 模式），
   原生端为真源、网页端只读镜像起步。**已铺路**：`/projects` 视图经
   `cloud.svelte.js` 只读拉取 Planner 项目/任务快照（RLS 限本人），
   但 Vault 内容本身（笔记/收集箱）尚未接云，仍是本机 localStorage/Tauri fs。
