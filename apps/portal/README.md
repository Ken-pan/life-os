...
> **阶段：** INTG.EVENTS.1 · **状态：** ✅ 生产 · UI 走查 P-1–P-12 已关闭（PORT.GROWTH.8/PORT.GROWTH.9）
...
| **今日摘要五卡**（Planner / Finance / Fitness / Music / Home 实验） | ✅ PORT.GROWTH.4b-M/H |
| `app-shell` + `PORTAL.OS` 顶栏品牌                                  | ✅           |
| 顶栏用户邮箱 + ⌘K 搜索 + pending 角标                               | ✅ PORT.GROWTH.2      |
| compact 顶栏 **More sheet**（主题 / 账号 / 退出）                   | ✅ P-5b      |
| Launcher / 摘要实验卡 **虚线** 左边框统一                           | ✅ P-12      |
| **⌘K 跨站深链**（14 路由）+ 最近搜索                                | ✅ PORT.GROWTH.6      |
| Portal 内登录 / 注册（Supabase Auth）                               | ✅           |
| 继续 / 默认应用 / 跳过自动跳转（`core_*`）                          | ✅ PORT.GROWTH.1/PORT.GROWTH.3 |
| SSO 状态 chip + Lucide icon registry                                | ✅           |
| PWA manifest + 六站安装引导                                         | ✅ PORT.GROWTH.5      |
| `createCoreIdentityHandler('portal')`                               | ✅           |
| Netlify 生产部署                                                    | ✅           |
| `svelte-check`                                                      | ✅ 0 errors  |
| `test:cp` 深链单测                                                  | ✅           |
| `qa:screenshot` UI 截图走查（12 张 + manifest）                     | ✅           |
| `qa:smoke` 登录+五卡+深链+⌘K                                        | ✅ PORT.GROWTH.9      |
...
截图输出 → `docs/ui-qa-screenshots/portal/main/latest/`（含 `manifest.json` — PORT.GROWTH.8 inbox href 校验）

## 上线 checklist

见 [`docs/roadmap/INTEGRATION.md`](../../docs/roadmap/INTEGRATION.md#i-p1) INTG.EVENTS.1 收尾项。
