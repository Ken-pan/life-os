...
describe('domainIdentity.core', () => {
  it('maps Knife 4 spaces to distinct accents and glyphs', () => {
    assert.equal(DOMAIN_IDENTITY.training.accentDark, '#C45C4A')
    assert.equal(DOMAIN_IDENTITY.training.accent, '#C45C4A')
    assert.equal(DOMAIN_IDENTITY.plan.icon, 'list-todo')
    assert.equal(DOMAIN_IDENTITY.money.accentDark, '#3D9B6E')
  ...
  it('gives Plan deeper ochre on light / glass (not #C9A227)', () => {
    assert.equal(DOMAIN_IDENTITY.plan.accentLight, '#C47A08')
    assert.equal(DOMAIN_IDENTITY.plan.accentOnGlassLight, '#87580E')
    assert.notEqual(DOMAIN_IDENTITY.plan.accentLight.toLowerCase(), '#c9a227')
    assert.notEqual(DOMAIN_IDENTITY.plan.accentOnGlassLight.toLowerCase(), '#c9a227')
    assert.notEqual(DOMAIN_IDENTITY.plan.accentOnGlassLight.toLowerCase(), '#9a6410')
    assert.equal(domainAccent('plan', '#000', { scheme: 'light', surface: 'glass' }), '#87580E')
    assert.equal(domainAccent('plan', '#000', { scheme: 'dark', surface: 'glass' }), '#E0B83A')
  })

  it('exposes semantic pairs for Money / Paper / Home / Training', () => {
    for (const id of ['money', 'paper', 'home', 'training']) {
      const entry = DOMAIN_IDENTITY[id]
      assert.ok(entry.accentLight)
      assert.ok(entry.accentDark)
      assert.ok(entry.accentOnGlassLight)
      assert.ok(entry.accentOnGlassDark)
      assert.notEqual(entry.accentLight, entry.accentDark)
    }
  })
  ...
  it('emits CSS variables without duplicate work keys', () => {
    ...
    assert.match(css, /--kenos-domain-plan-on-glass-light:\s*#87580E/)
