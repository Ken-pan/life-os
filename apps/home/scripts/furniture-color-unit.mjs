...
// 「渲染出来是什么色类」得看驯化后的结果。
// resolveFurnitureColor 2026-07-16 起收 attrs 对象(colorHex/colorConfidence/
// colorSpreadE/kindConfidence/userEdited),这里包一层保持用例简洁。
const rendered = (kind, symbol, scanHex, extra = {}) => {
  const raw = resolveFurnitureColor(kind, symbol, { colorHex: scanHex, ...extra })
...
const rendered2 = (kind, symbol, scanHex, conf) =>
  rendered(kind, symbol, scanHex, { colorConfidence: conf })
...
assert.equal(
  resolveFurnitureColor('cabinet', 'cabinet', { colorHex: '#DBD6CA' }),
  resolveFurnitureColor('cabinet', 'cabinet', {}),
  '白柜:暖灰扫描不采信,保持 WHITE_CABINET(不发灰)',
)
assert.equal(
  resolveFurnitureColor('wire_rack', 'wireRack', { colorHex: '#D7D1C6' }),
  resolveFurnitureColor('wire_rack', 'wireRack', {}),
  '银架:暖扫描不采信,保持 METAL_CHROME(不发黄)',
)
assert.equal(
  resolveFurnitureColor('toilet', 'toilet', { colorHex: '#BFB4A4' }),
  resolveFurnitureColor('toilet', 'toilet', {}),
...
assert.equal(resolveFurnitureColor('mystery_thing', undefined, {}), undefined, '无类型色手绘件 → undefined 兜底')
...
assert.equal(
  resolveFurnitureColor('shelf', 'shelf', { colorHex: '#FFFFFF', colorConfidence: 0.3 }),
  resolveFurnitureColor('shelf', 'shelf', {}),
...
// ——————————————————————————————————————————————————————————
// 7. kindConfidence 守硬锁(0716 真扫:浅木「升降边桌」被几何猜成 desk@0.55)
// ——————————————————————————————————————————————————————————
{
  // 低把握猜出的 desk + 可信浅扫描 → 不硬锁炭黑,软兜底采扫描的浅色
  const guessed = rendered('desk', 'table', '#D9CFC4', {
    colorConfidence: 0.67,
    kindConfidence: 0.55,
  })
  assert.ok(!isDark(guessed), `低把握 desk 不该被硬锁涂黑,得到 ${guessed}`)
  assert.ok(hslOf(guessed).l >= 0.7, `该采信浅扫描色,得到 ${guessed}`)
  // 同 kind 但高把握 → 硬锁照旧炭黑
  assert.ok(
    isDark(rendered('desk', 'table', '#D9CFC4', { kindConfidence: 0.9 })),
    '高把握 desk 仍硬锁炭黑',
  )
  // 用户改过 kind → 即便低把握标也硬锁(用户意图最高)
  assert.ok(
    isDark(
      rendered('desk', 'table', '#D9CFC4', { kindConfidence: 0.4, userEdited: ['kind'] }),
    ),
    '用户确认的 desk 硬锁炭黑,不看 kindConfidence',
  )
  // 无 kindConfidence(老数据)→ 硬锁照旧(向后兼容)
  assert.ok(isDark(rendered('desk', 'table', undefined)), '老数据 desk 硬锁不变')
}
