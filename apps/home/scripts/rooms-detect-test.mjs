...
// ---- 缺口搭桥:扫描墙常差几英寸不闭合(508 实测卧室门侧缝 9.5″) ----

// 10×10 房间,左墙差 6″ 没接到角上 → 搭桥后仍识别为 1 间
check(
  '缺口搭桥 — 6″ 缝隙仍成间',
  areas(
    graph(
      [['a', 0, 0], ['b', 10, 0], ['c', 10, 10], ['d', 0, 10], ['e', 0, 0.5]],
      [['a', 'b'], ['b', 'c'], ['c', 'd'], ['d', 'e']],
    ),
  ),
  [100],
...
// 同形但缺口 3′ —— 这是门洞尺寸,必须保持敞开(围不成房间)
check(
  '门洞不搭桥 — 3′ 开口不成间',
  areas(
    graph(
      [['a', 0, 0], ['b', 10, 0], ['c', 10, 10], ['d', 0, 10], ['e', 0, 3]],
      [['a', 'b'], ['b', 'c'], ['c', 'd'], ['d', 'e']],
    ),
  ),
  [],
...
check(
  ...
  areas(
    graph(
      [
        ['a', 0, 0], ['b', 10, 0], ['c', 20, 0],
        ['d', 20, 10], ['f', 0, 10], ['m', 10, 9.75],
      ],
      [['a', 'b'], ['b', 'c'], ['c', 'd'], ['d', 'f'], ['f', 'a'], ['b', 'm']],
    ),
  ),
  [100, 100],
)

// ---- extraSegments:围栏隔断当虚拟墙 ----
{
  const box = graph(
  ...
  check(
    '围栏隔断 — 贯通栏板切成两间',
    detectRooms(box, { extraSegments: [seg(10, 0, 10, 10)] }).map((r) =>
  ...
  check(
    '围栏隔断 — 两端差 6″ 也切(搭桥接上)',
    detectRooms(box, { extraSegments: [seg(10, 0.5, 10, 9.5)] }).map((r) =>
      Math.round(r.areaSqFt),
    ),
    [100, 100],
  )
  check(
    '围栏隔断 — 悬空半截不切(留人行开口)',
    detectRooms(box, { extraSegments: [seg(10, 3, 10, 7)] }).map((r) =>
      Math.round(r.areaSqFt),
    ),
    [200],
  )
