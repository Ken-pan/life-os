...
2. Kenos iOS（首选）+ HealthKit —— **已并入 `clients/apple` KenosIOS**；Settings → Apple Health 授权后自动同步。独立 companion 仍可作调试壳。
...
| **Watch/iPhone 伴侣 app**      | 睡眠 / 静息心率 / HRV / 步数(持续,被动)         | ✅ HealthKit         | 你的 Xcode + 开发者账号部署一次        | **已交付源码(本次)**            |
| **Apple Health 导出**          | 同上,但一次性快照                               | ✅ 离线              | 手动导出一次                           | 已上线(顶不上伴侣 app 时的回退) |
...
## 首选:Watch + iPhone 伴侣 app(被动、持续、零手动)

State Engine 现在**纯信号驱动**——六维全从测量数据(HRV/静息心率/睡眠/步数相对个人基线)推导,不再有任何手动自评。持续数据来自 [`../companion/`](../companion/README.md):iPhone 读 HealthKit(含 Apple Watch 自动同步来的数据)→ 交付 Mac 代理。

两条交付路:

- **iCloud Drive(推荐)**:伴侣 app 写 iCloud 容器,代理扫 `~/Library/Mobile Documents/iCloud~space~kenos~healthos/Documents/inbox` 摄入。无需联网、不暴露端口。
- **LAN 直连**:同 wifi 下 POST 代理 `:5193/ingest`,近实时。

代理侧 `POST /ingest`(单样本批量)与 inbox 扫描(每 30s + 启动时)已实现并 curl 验证;摄入按 `date` upsert 进 `health.jsonl`。iOS/watchOS app **无法在本仓库沙盒构建部署**,以 XcodeGen 规范 + 完整 Swift 源码交付,需用户 Xcode 签名。
