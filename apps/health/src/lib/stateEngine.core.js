 ...
 *
 * 纯信号驱动:六维全部从 Apple Watch/健康测量(HRV、静息心率、睡眠、步数)相对
 * 个人基线,加 Focus 代理负荷推导——**不需要任何手动输入**。缺测量数据时如实
 * unknown 并引导连接 Watch,绝不退回手动表单。
 *
 * 原则:Raw observation ≠ Interpretation。本文件只做 Interpretation;原始测量的采集
 * 在 Watch/iPhone 伴侣 app → Mac 代理 health.jsonl。每维输出 level + 结构化 reasons
 * (来源可解释),绝不给无法追溯的裸分数。
 *
...
function baselineOf(health, key, now) {
  const todayStr = isoDate(now)
  const vals = health
    .filter((d) => typeof d?.[key] === 'number' && d.date && d.date !== todayStr)
    .map((d) => d[key])
  return { value: vals.length >= MIN_BASELINE ? median(vals) : null, n: vals.length }
}
...
const worseOf = (...levels) => levels.reduce((w, l) => (RANK[l] > RANK[w] ? l : w), 'good')
 ...
 * @param {Array}  input.health   [{date, sleepHours?, restingHR?, hrv?, steps?}]
...
export function deriveState({ now, health, agent }) {
  ...
  if (!sleepLast) {
    dims.sleepDebt = { level: 'unknown', reasons: [{ k: 'state.r_needWatchSleep' }] }
  } else {
  ...
  if (hrvRatio == null) {
    const reasons = [{ k: hrvToday ? 'state.r_needBaseline' : 'state.r_needWatchHrv', p: { n: hrvBase.n, need: MIN_BASELINE } }]
    dims.stress = { level: 'unknown', reasons }
  } else {
    let level = hrvRatioLevel(hrvRatio)
    const reasons = [
      { k: 'state.r_hrvToday', p: { hrv: Math.round(hrvToday.value), base: Math.round(hrvBase.value) } },
    ]
  ...
  // ---- Recovery:HRV + 静息心率 + 睡眠;都缺则回落到 Focus 负荷 ----
  {
    ...
    if (hrvRatio != null) {
      ...
      reasons.push({ k: 'state.r_hrvToday', p: { hrv: Math.round(hrvToday.value), base: Math.round(hrvBase.value) } })
    }
    if (rhrDelta != null) {
      signals.push(rhrDeltaLevel(rhrDelta))
      reasons.push({ k: 'state.r_rhrToday', p: { rhr: Math.round(rhrToday.value), base: Math.round(rhrBase.value), delta: rhrDelta > 0 ? `+${Math.round(rhrDelta)}` : Math.round(rhrDelta) } })
    }
    ...
    if (signals.length) {
      ...
      if (netMin < 30) dims.recovery = { level: 'good', reasons: [{ k: 'state.r_netToday', p: { min: netMin } }] }
      else {
        const breaks = a.breaksToday ?? 0
        dims.recovery =
          breaks >= Math.floor(netMin / 60)
            ? { level: 'ok', reasons: [{ k: 'state.r_netToday', p: { min: netMin } }, { k: 'state.r_breaks', p: { n: breaks } }] }
            : { level: 'watch', reasons: [{ k: 'state.r_fewBreaks', p: { min: netMin, n: breaks } }] }
      }
    } else {
      dims.recovery = { level: 'unknown', reasons: [{ k: 'state.r_needWatchHrv', p: { n: hrvBase.n, need: MIN_BASELINE } }] }
    }
  }

  // ---- Energy:睡眠为主,静息心率升高与高负荷各下调一档 ----
  if (!sleepLast) {
    dims.energy = { level: 'unknown', reasons: [{ k: 'state.r_needWatchSleep' }] }
  } else {
    let level = sleepLevel(sleepLast.hours)
    const reasons = [{ k: 'state.r_sleepLastMeasured', p: { hours: sleepLast.hours } }]
    if (rhrDelta != null && rhrDelta > 5) {
      level = stepDown(level)
      reasons.push({ k: 'state.r_rhrElevated', p: { delta: `+${Math.round(rhrDelta)}` } })
    }
    if (netMin >= 150) {
      level = stepDown(level)
  ...
  // ---- Physical readiness:睡眠 + 静息心率 + 活动量 ----
  if (!sleepLast && rhrDelta == null) {
    dims.physical = { level: 'unknown', reasons: [{ k: 'state.r_needWatch' }] }
  } else {
    const signals = []
    ...
    let level = worseOf(...signals)
    if (stepsToday) reasons.push({ k: 'state.r_stepsToday', p: { steps: Math.round(stepsToday.value) } })
