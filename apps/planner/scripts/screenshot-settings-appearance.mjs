...
function auditAppearance(root) {
  ...
  const pressed = [
    ...section.querySelectorAll('button[aria-pressed="true"]'),
  ].map((btn) => btn.textContent?.trim())
  const labels = [
    ...section.querySelectorAll('.pref-label, .sr-label, .pref-copy .sr-label'),
  ].map((node) => node.textContent?.trim())

  const themeButtons = groups[1]
    ? [...groups[1].querySelectorAll('button')].map((btn) =>
        btn.textContent?.trim(),
      )
    : []
  const localeButtons = groups[0]
    ? [...groups[0].querySelectorAll('button')].map((btn) =>
        btn.textContent?.trim(),
      )
    : []
...
async function seedFinanceSession(page) {
  ...
  await page.goto(`${APPS.finance.base}/`, {
    waitUntil: 'domcontentloaded',
    timeout: 20000,
  })
  await page.evaluate(
...
async function captureApp(browser, appKey, viewport) {
  ...
  if (visible) {
    audit = await appearance.evaluate((root) => {
      ...
      const labels = [
        ...section.querySelectorAll('.pref-label, .sr-label'),
      ].map((node) => node.textContent?.trim())
      const themeButtons = groups[1]
        ? [...groups[1].querySelectorAll('button')].map((btn) =>
            btn.textContent?.trim(),
  ...
  if (visible) {
    ...
    await page.screenshot({
      path: path.join(outDir, `${appKey}-settings-${viewport.id}.png`),
      fullPage: true,
    })
  }
...
for (const viewport of viewports) {
  for (const appKey of Object.keys(APPS)) {
    try {
      ...
      results.push(result)
      console.log(
        `${result.audit?.ok ? 'ok' : 'FAIL'} ${appKey}-${viewport.id}`,
        result.audit,
      )
    } catch (err) {
      results.push({
