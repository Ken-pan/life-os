/**
 * UI/UX 走查：三条主流程 + 截图
 * 输出：docs/ui-qa-screenshots/uiux-walkthrough-2026-07-05/
 */
...
const OUT = path.join(__dirname, '../docs/ui-qa-screenshots/uiux-walkthrough-2026-07-05');
...
async function auditPage(page) {
  return page.evaluate(() => {
    ...
    const tapTargets = [...document.querySelectorAll('button, a, input, [role="button"]')];
    ...
    const lastTask = document.querySelector('.task-row:last-child, .empty-state');
    ...
    return {
      ...
      contrastHints: {
        mutedText: getComputedStyle(document.body).getPropertyValue('--t3') || null
      }
    };
  });
}
...
async function seed(page) {
  ...
  await page.waitForSelector('[data-testid="fab-add"]', { state: 'visible', timeout: 15000 });
}
...
async function flow1TaskDaily(browser) {
  ...
  if (audit.scrollExcess > 0) {
    note(flow, '04', '打开编辑器时背景页仍可感知滚动余量（已部分修复 scroll lock）', 'low');
  }
...
async function flow3Settings(browser) {
  ...
  if (auditBottom.gapLastToNav != null && auditBottom.gapLastToNav > 48) {
    note(flow, '03', `设置页滚到底内容与底栏间距偏大 (${Math.round(auditBottom.gapLastToNav)}px)`, 'medium');
  }
  ...
  if (!auditAuth.path.startsWith('/auth')) note(flow, '05', '认证页路由异常', 'high');
...
async function main() {
  ...
  findings.forEach((f) => console.log(`  [${f.severity}] ${f.flow}/${f.step}: ${f.issue}`));
}
