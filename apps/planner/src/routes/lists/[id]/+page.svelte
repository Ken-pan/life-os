...
<AppBar
  title={list ? listLabel(list) : t('nav.lists')}
  historyBack
/>

<div class="wrap">
  {#if list}
    <QuickAddBar listId={list.id} dueDate={null} placeholder={t('home.quickAdd')} />
    <TaskGroup title={listLabel(list)} {tasks} hideHeader empty={t('common.empty')} onToggle={completeTask} onEdit={editTask} />
  {/if}
</div>
