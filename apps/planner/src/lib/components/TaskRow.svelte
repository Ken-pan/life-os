...
<script>
  ...
  const fullDeleteThreshold = $derived(Math.max(actionsWidth + 70, rowWidth * 0.55));
  ...
  function onPointerMove(e) {
    ...
    if (!captured) {
      ...
      if (Math.abs(rawDx) < 10 || Math.abs(rawDx) < Math.abs(rawDy) * 1.2) return;
      captured = true;
      dragging = true;
...
>
  {#if showComplete}
    <div
      ...
      aria-hidden="true"
    >
      <Icon name={task.completed ? 'rotate-ccw' : 'check'} size={18} strokeWidth={3} />
    </div>
  {:else if showActions}
