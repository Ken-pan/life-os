// 本地演示模式（localhost）—— 空库时自动灌入 demo 数据，全面展示核心功能。
// 严格只在 localhost 生效，生产域名永不激活。语义与 finance 的 demoMode 对齐：
//   - localhost 下库为空 → 默认灌 demo；
//   - ?demo=0 显式关闭（持久化，露出真实空态）；?demo=1 显式开启；
//   - 有真实数据（localStorage 已存）时永不覆盖（只在「空」分支调用）。
import { browser } from '$app/environment'

const FLAG_KEY = 'planos_demo'
