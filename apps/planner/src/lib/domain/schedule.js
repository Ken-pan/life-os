...
export const SCHEDULE_START_TIMES = ['09:00', '09:30', '10:00', '13:00', '15:00', '19:00']
...
export function blockLayout(
  ...
  if (startMinutes >= dayEndMinutes || endMinutes <= dayStartMinutes) return null

  const top =
    ((startMinutes - dayStartMinutes) / 60) * hourHeight
...
export function timelineHeightPx(dayStart = DAY_START_HOUR, dayEnd = DAY_END_HOUR) {
  return (dayEnd - dayStart) * HOUR_HEIGHT_PX
}

/** @param {number} [now] @param {number} [dayStart] @param {number} [dayEnd] */
export function currentTimeMarkerTop(now = Date.now(), dayStart = DAY_START_HOUR, dayEnd = DAY_END_HOUR) {
