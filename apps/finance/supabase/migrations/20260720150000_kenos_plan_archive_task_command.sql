-- Kenos Plan archive/delete (soft tombstone) command (additive).
-- Track B final Plan lifecycle slice. Prerequisites: tip >= 20260720140000.

create or replace function private.kenos_archive_plan_task_action(action_request jsonb)
returns jsonb
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  if v_action_type <> 'plan.archive_task' then raise exception 'unsupported_action'; end if;
  ...
  v_data := jsonb_set(v_data, '{deletedAt}', to_jsonb(v_now_ms), true);
  ...
  values (v_user_id, v_action_id, v_action_type, v_idempotency_key, v_correlation_id, v_entity_ref, jsonb_build_object('taskId', v_task_id, 'deletedAt', v_now_ms))
  ...
  values (v_user_id, v_action_id, v_action_type, v_correlation_id, coalesce(action_request #>> '{actor,type}', 'user'), coalesce(action_request ->> 'producer', 'plan'),
    jsonb_build_object('requestId', v_action_id, 'outcome', 'allow', 'evaluatedRisk', 'R1', 'policyVersion', 'kenos-phase1-2026-07-20', 'reasons', jsonb_build_array('explicit archive-task soft-delete command'), 'decidedAt', v_now),
    v_entity_ref, 'Archived Plan task', 'succeeded', jsonb_build_object('taskId', v_task_id, 'deletedAt', v_now_ms), jsonb_build_object('supported', false))
...
revoke all on function private.kenos_archive_plan_task_action(jsonb) from public, anon, authenticated;

create or replace function public.kenos_archive_plan_task_action(action_request jsonb)
returns jsonb language sql security definer set search_path = '' as $$
  select private.kenos_archive_plan_task_action(action_request);
$$;

revoke all on function public.kenos_archive_plan_task_action(jsonb) from public, anon, authenticated;
grant execute on function public.kenos_archive_plan_task_action(jsonb) to authenticated;
