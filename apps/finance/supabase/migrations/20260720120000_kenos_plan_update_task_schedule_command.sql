-- Kenos Plan update-task-schedule command (additive).
-- Track B scheduling slice: scheduledDate / scheduledStart / durationMinutes.
-- Prerequisites: tip >= 20260720110000.
-- Does not revoke Legacy planner_tasks upsert.

create or replace function private.kenos_update_plan_task_schedule_action(action_request jsonb)
returns jsonb
language plpgsql
security definer
set search_path = ''
...
declare
  ...
  v_scheduled_date_raw jsonb := action_request #> '{payload,scheduledDate}';
  v_scheduled_start_raw jsonb := action_request #> '{payload,scheduledStart}';
  v_duration_raw jsonb := action_request #> '{payload,durationMinutes}';
  v_scheduled_date text := null;
  v_scheduled_start text := null;
  v_duration_minutes int := null;
...
begin
  ...
  if v_action_type <> 'plan.update_task_schedule' then
  ...
  if not (
    (action_request -> 'payload' ? 'scheduledDate')
    and (action_request -> 'payload' ? 'scheduledStart')
    and (action_request -> 'payload' ? 'durationMinutes')
  ) then
    raise exception 'schedule_fields_required';
  end if;

  if jsonb_typeof(v_scheduled_date_raw) = 'null' then
    v_scheduled_date := null;
  elsif jsonb_typeof(v_scheduled_date_raw) = 'string' then
    v_scheduled_date := nullif(btrim(v_scheduled_date_raw #>> '{}'), '');
    if v_scheduled_date is not null and v_scheduled_date !~ '^\d{4}-\d{2}-\d{2}$' then
  ...
  if jsonb_typeof(v_scheduled_start_raw) = 'null' then
    v_scheduled_start := null;
  elsif jsonb_typeof(v_scheduled_start_raw) = 'string' then
    v_scheduled_start := nullif(btrim(v_scheduled_start_raw #>> '{}'), '');
    if v_scheduled_start is not null and v_scheduled_start !~ '^\d{2}:\d{2}$' then
      raise exception 'scheduled_start_invalid';
    end if;
  else
    raise exception 'scheduled_start_invalid';
  end if;

  if jsonb_typeof(v_duration_raw) = 'null' then
    v_duration_minutes := null;
  elsif jsonb_typeof(v_duration_raw) = 'number' then
    v_duration_minutes := (v_duration_raw #>> '{}')::int;
    if v_duration_minutes is null or v_duration_minutes < 1 or v_duration_minutes > 24 * 60 then
      raise exception 'duration_minutes_invalid';
    end if;
  else
    raise exception 'duration_minutes_invalid';
  end if;

  if v_scheduled_date is null and (v_scheduled_start is not null or v_duration_minutes is not null) then
    raise exception 'schedule_requires_date';
  end if;
  ...
  if v_scheduled_date is null then
    v_data := jsonb_set(v_data, '{scheduledDate}', 'null'::jsonb, true);
  else
    v_data := jsonb_set(v_data, '{scheduledDate}', to_jsonb(v_scheduled_date), true);
  end if;
  if v_scheduled_start is null then
    v_data := jsonb_set(v_data, '{scheduledStart}', 'null'::jsonb, true);
  else
    v_data := jsonb_set(v_data, '{scheduledStart}', to_jsonb(v_scheduled_start), true);
  end if;
  if v_duration_minutes is null then
    v_data := jsonb_set(v_data, '{durationMinutes}', 'null'::jsonb, true);
  else
    v_data := jsonb_set(v_data, '{durationMinutes}', to_jsonb(v_duration_minutes), true);
  end if;
  ...
  values (
    ...
    jsonb_build_object(
      'taskId', v_task_id,
      'scheduledDate', to_jsonb(v_scheduled_date),
      'scheduledStart', to_jsonb(v_scheduled_start),
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'reasons', jsonb_build_array('explicit update-task-schedule command'),
      'decidedAt', v_now
    ),
    v_entity_ref,
    'Updated Plan task schedule',
    'succeeded',
    jsonb_build_object(
      'taskId', v_task_id,
      'scheduledDate', to_jsonb(v_scheduled_date),
...
revoke all on function private.kenos_update_plan_task_schedule_action(jsonb) from public, anon, authenticated;

create or replace function public.kenos_update_plan_task_schedule_action(action_request jsonb)
returns jsonb
language sql
security definer
set search_path = ''
as $$
  select private.kenos_update_plan_task_schedule_action(action_request);
$$;

revoke all on function public.kenos_update_plan_task_schedule_action(jsonb) from public, anon, authenticated;
grant execute on function public.kenos_update_plan_task_schedule_action(jsonb) to authenticated;
