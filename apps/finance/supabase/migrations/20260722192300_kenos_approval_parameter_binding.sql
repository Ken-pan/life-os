-- Kenos Productivity Spine — Phase A: approval ↔ execution parameter binding.
-- Recreates private.kenos_request_action_approval_action so that:
--   * payload.normalizedParametersHash (sha256 hex of canonical sorted-key JSON
--     of the proposed action parameters) is validated and persisted onto
--     kenos_action_approvals.normalized_parameters_hash
--   * a new request for the same proposed action with a different hash
--     auto-supersedes older pending approvals (parameter change invalidates)
-- Executors must recompute the hash before executing an approved action and
-- refuse on mismatch or expiry. Additive; body otherwise identical to
-- 20260720170000_kenos_action_approval_request_decide.sql.
...
begin
  ...
  v_params_hash := nullif(btrim(action_request #>> '{payload,normalizedParametersHash}'), '');
  if v_params_hash is not null and v_params_hash !~ '^[0-9a-f]{64}$' then raise exception 'invalid_parameters_hash'; end if;
  ...
  -- Parameter binding: a new request for the same proposed action with a
  -- different normalized parameter hash invalidates older pending approvals.
  update public.kenos_action_approvals
  set status = 'superseded',
      decided_at = v_now,
      decided_by = 'system:parameter_change',
      decision_reason = 'superseded: parameters changed by newer request',
      updated_at = v_now
  where owner_id = v_user_id
    and status = 'pending'
    and action_type = v_proposed_action_type
    and action_id = v_proposed_action_id
    and coalesce(normalized_parameters_hash, '') is distinct from coalesce(v_params_hash, '');

  perform private.kenos_store_action_approval(v_approval_record);

  update public.kenos_action_approvals
  set normalized_parameters_hash = v_params_hash
  where id = v_approval_id and owner_id = v_user_id;
