-- Kenos Plan update-task-title command (additive).
-- Track B first slice under Autonomous Production Completion Program.
-- Prerequisites: tip >= 20260719130500; kenos_plan_* tables live.
-- Does not revoke Legacy planner_tasks upsert.

create or replace function private.kenos_update_plan_task_title_action(action_request jsonb)
returns jsonb
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  if v_action_type <> 'plan.update_task_title' then
  ...
  v_data := jsonb_set(v_data, '{title}', to_jsonb(v_title), true);
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'reasons', jsonb_build_array('explicit update-task-title command'),
      'decidedAt', v_now
    ),
    v_entity_ref,
    'Updated Plan task title',
    'succeeded',
    jsonb_build_object('taskId', v_task_id, 'title', v_title),
...
revoke all on function private.kenos_update_plan_task_title_action(jsonb) from public, anon, authenticated;

create or replace function public.kenos_update_plan_task_title_action(action_request jsonb)
returns jsonb
language sql
security definer
set search_path = ''
as $$
  select private.kenos_update_plan_task_title_action(action_request);
$$;

revoke all on function public.kenos_update_plan_task_title_action(jsonb) from public, anon, authenticated;
grant execute on function public.kenos_update_plan_task_title_action(jsonb) to authenticated;
