...
import type { DecisionSection } from '../lib/appRoute'
...
function getTemplates(t: TranslateFn): { id: TemplateType; label: string }[] {
  return [
    ...
    {
      id: 'recurring_cost',
      label: t('decisionStudio.template.recurring_cost'),
    },
    {
      id: 'cash_vs_finance',
      label: t('decisionStudio.template.cash_vs_finance'),
    },
    ...
    { id: 'career_break', label: t('decisionStudio.template.career_break') },
    {
      ...
      label: t('decisionStudio.template.partner_contribution'),
    },
    {
      id: 'wait_buy_later',
      label: t('decisionStudio.template.wait_buy_later'),
    },
  ]
}

function getTemplateFieldConfig(
  t: TranslateFn,
): Record<TemplateType, TemplateFieldConfig> {
  ...
  return {
    ...
    partner_contribution: {
      ...
      partnerPercent: {
        label: t(
          'decisionStudio.field.partner_contribution.partnerPercentLabel',
        ),
...
function getAdvancedFieldLabels(
  t: TranslateFn,
): Record<
  AdvancedField,
...
function estimatedFinancePayment(input: {
  ...
  return (
    ((input.amount - input.downPayment) * (1 + input.apr)) /
    Math.max(1, input.termMonths)
  )
}
...
function scenarioTypeFromTemplate(template: TemplateType): ScenarioType {
  if (template === 'purchase' || template === 'wait_buy_later')
    return 'purchase'
...
function eventDisplayType(t: TranslateFn, e: ScenarioEvent): string {
  if (e.eventType === 'one-time-purchase')
    return t('decisionStudio.eventType.oneTimePurchase')
  if (e.eventType === 'windfall') return t('decisionStudio.eventType.windfall')
  if (e.eventType === 'expense-change')
    return t('decisionStudio.eventType.expenseChange')
  if (e.eventType === 'salary-change')
    return t('decisionStudio.eventType.salaryChange')
    ...
    <div className="card card-compact">
      ...
      <div className="grid kpi-row-4">
        <div className="item">
          ...
          <div className="name">
            {money(c.scenario.safeToSpendToday, privacy)}
          </div>
          <div
            className={c.delta.safeToSpendToday >= 0 ? 'text-pos' : 'text-neg'}
          >
        ...
        <div className="item">
          <div className="meta">
            {t('decisionStudio.compare.lowestCash30d')}
          </div>
          <div className="name">
          ...
          <div
            className={
              c.delta.lowestProjectedOperatingCash30d >= 0
                ? 'text-pos'
                : 'text-neg'
            }
          >
        ...
        <div className="item">
          <div className="meta">
            {t('decisionStudio.compare.monthlySurplus')}
          </div>
          <div className="name">
            {money(c.scenario.monthlySurplus, privacy)}
          </div>
          <div
            className={c.delta.monthlySurplus >= 0 ? 'text-pos' : 'text-neg'}
          >
        ...
        <div className="item">
          <div className="meta">
            {t('decisionStudio.compare.netWorth10yDelta')}
          </div>
...
export function DecisionStudioView({
  ...
  active?: DecisionSection
  onChange?: (section: DecisionSection) => void
} = {}) {
  const { t } = useLocale()
  ...
  const [internalTab, setInternalTab] = useState<StudioTab>('compare')
  ...
  const [previewComparison, setPreviewComparison] =
    useState<DecisionComparison | null>(null)
  ...
  const [recordStatus, setRecordStatus] =
    useState<DecisionStatus>('considering')
  ...
  useEffect(() => {
    if (!applyScenarioId && savedScenarios.length > 0) {
      void Promise.resolve().then(() =>
        setApplyScenarioId(savedScenarios[0].id),
      )
    }
  ...
  useEffect(() => {
    ...
    void repo
      ...
      .catch((e) =>
        console.error('[decision] load apply scenario events failed', e),
      )
  }, [applyScenarioId])
  ...
  const recordDecision = async () => {
    const row: DecisionRecord = {
      ...
      decisionSummary:
        recordSummary || t('decisionStudio.defaultDecisionSummary'),
  ...
  const applyToPlan = async () => {
    ...
    try {
      const result = await repo.applyScenarioToPlan(
        applyScenarioId,
        applySelectedIds,
      )
      setApplyMessage(
      ...
      setApplyMessage(
        e instanceof Error ? e.message : t('decisionStudio.applyFailed'),
      )
    } finally {
      setApplyBusy(false)
    }
  ...
  const undoLatestApply = async () => {
    ...
    try {
      ...
      setApplyMessage(
        e instanceof Error ? e.message : t('decisionStudio.undoFailed'),
      )
    } finally {
      setUndoBusy(false)
    }
        ...
        <TabPanel tabId="compare" active={tab === 'compare'}>
          <div className="grid gap-3">
            <div className="card card-compact">
              ...
              />
              <p className="muted-note mt-1 mb-3">{fieldConfig.summary}</p>
              <div className="row">
                {fieldConfig.amount && (
                  <div className="field field-flex">
                    ...
                    />
                    <p className="muted-note mt-1">{fieldConfig.amount.hint}</p>
                  </div>
                )}
                {fieldConfig.monthly && (
                  <div className="field field-flex">
              ...
              <div className="item inset-panel">
            ...
            {previewComparison && (
              <div className="card card-compact">
                    ...
                    <label key={e.id} className="item">
                      ...
                      <div className="grow">
                        ...
                        <div className="meta">
                          ...
                          {e.date ??
                            t('decisionStudio.preview.monthOffset', {
                              offset: e.monthOffset,
                            })}
                        </div>
                      </div>
                    </label>
                ...
                {previewComparison &&
                  previewComparison.confidence !== 'Ready to compare' && (
                ...
                <div className="row mt-2">
                  <button
                    className="btn ghost"
                    onClick={undoLatestApply}
                    disabled={undoBusy}
                  >
                    {undoBusy
                      ? t('decisionStudio.undoBusy')
                      : t('decisionStudio.undoLatest')}
                  </button>
                  <button
                    className="btn"
                    disabled={
                  ...
                  >
                    {applyBusy
                      ? t('decisionStudio.applyBusy')
                      : t('decisionStudio.applySelected')}
                  </button>
                </div>
                {applyMessage && (
                  <p className="muted-note mt-2">{applyMessage}</p>
                )}
              </div>
            )}
          </div>
              ...
              <div className="row">
                ...
                <SelectField<string>
                  ...
                  options={[
                    ...
                    {
                      value: 'purchase',
                      label: t('terminology.scenarioTypePurchase'),
                    },
                    {
                      value: 'recurring_cost',
                      label: t('terminology.scenarioTypeRecurringCost'),
                    },
                    {
                      value: 'rent_change',
                      label: t('terminology.scenarioTypeRentChange'),
                    },
                    {
                      value: 'travel',
                      label: t('terminology.scenarioTypeTravel'),
                    },
                    {
                      value: 'career_break',
                      label: t('terminology.scenarioTypeCareerBreak'),
                    },
                    {
                    ...
                    {
                      value: 'cash_vs_finance',
                      label: t('terminology.scenarioTypeCashVsFinance'),
                    },
                  ]}
                ...
                <SelectField<string>
                  ...
                  options={[
                    {
                      value: 'all',
                      label: t('decisionStudio.filterAllStatuses'),
                    },
                    {
                      value: 'draft',
                      label: t('terminology.scenarioStatusDraft'),
                    },
                    {
                      value: 'saved',
                      label: t('terminology.scenarioStatusSaved'),
                    },
                    {
                      value: 'chosen',
                      label: t('terminology.scenarioStatusChosen'),
                    },
                    {
                      value: 'archived',
                      label: t('terminology.scenarioStatusArchived'),
                    },
                  ]}
              ...
              <div key={s.id} className="card card-compact">
                <div className="row">
                  ...
                  <div className="field">
                    ...
                    <div className="input">{scenarioStatusLabel(s.status)}</div>
                  </div>
                  <div className="field field-actions">
                    ...
                    <div className="flex-row-tight">
                      <button
                        className="btn ghost"
                        onClick={() => store.setActiveScenario(s.id)}
                      >
                      ...
                      <button
                        ...
                        onClick={() =>
                          store.upsertScenario({
                            ...s,
                            status:
                              s.status === 'archived' ? 'saved' : 'archived',
                            updatedAt: new Date().toISOString(),
                            archivedAt:
                              s.status === 'archived'
                                ? undefined
                      ...
                      <button
                        ...
                        onClick={() => store.removeScenario(s.id)}
                      >
                  ...
                  <div className="row">
                    <div className="item grow">
                      ...
                      <div className="meta">
                        ...
                        · {decisionStatusLabel(r.decisionStatus)} ·{' '}
