...
export function ExtensionSyncBridge() {
  ...
  useEffect(() => {
    ...
    if (sessionTxnsRef.current.length > 0) {
      ...
      for (const t of txns) counts.set(keyOf(t), (counts.get(keyOf(t)) ?? 0) + 1);
  ...
  useEffect(() => {
    ...
    const applyOne = async (env: CaptureEnvelope): Promise<ApplyResult> => {
      ...
      if (env.kind === "holdings") {
        ...
        if (syncResult.transactions.length > 0) {
          ...
          for (const t of syncResult.transactions) sessionTxnsRef.current.push(t);
        }
        rpcFinalized = true;
    ...
    const emitSyncResult = (result: BridgeSyncResult) => {
      const payload = { type: BRIDGE_MSG.syncResult, result: { ...result, at: new Date().toISOString() } };
    ...
    const drainLocked = async () => {
      ...
      while (queueRef.current.length > 0) {
        ...
        try {
          ...
          applyFailed = true;
          setSyncError(e instanceof Error ? e.message : t("extension.applyFailed"));
          break;
        }
      }
      ...
      const failed = applyFailed ? 1 : 0;
      if (batchReports.length > 0 || failed > 0 || pending > 0) {
        emitSyncResult({
          ok: !applyFailed && pending === 0,
    ...
    const onCaptures = (captures: unknown[]) => {
      for (const c of captures) {
        if (isCaptureEnvelope(c) && !queueRef.current.some((q) => q.id === c.id)) {
    ...
    const onRequestSnapshot = (e: MessageEvent) => {
      if (e.source !== window || e.origin !== window.location.origin) return;
      if ((e.data as { type?: string })?.type === BRIDGE_MSG.requestSnapshot) emitSnapshot();
    };
    ...
    <div className="ext-sync-toasts" aria-live="polite">
      {syncError && (
        <div className="ext-sync-toast ext-sync-toast--warn" role="alert">
          ...
          <span>{syncError}</span>
          <button aria-label={tl("extension.close")} onClick={() => setSyncError(null)}>
            ×
          </button>
        </div>
      )}
      {reports.map((r) => (
        <div key={r.envelopeId} className="ext-sync-toast">
          <div className="ext-sync-toast-head">
            ...
            <strong>{tl("extension.synced", { source: SOURCE_LABEL[r.source] })}</strong>
          </div>
          ...
          <button
            ...
            onClick={() => setReports((prev) => prev.filter((x) => x.envelopeId !== r.envelopeId))}
          >
            ×
          </button>
        </div>
