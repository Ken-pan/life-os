...
function reviewFilters(
  t: (key: string) => string,
): { id: ReviewFilterId; label: string }[] {
  return [
...
function proposedActionLabels(
  t: (key: string) => string,
): Record<'increase' | 'decrease' | 'keep', string> {
  return {
    ...
    <div className="card">
        ...
        <div className="grid gap-3">
          ...
          {validation && (
            <div className="grid kpi-row-4">
              <StatChip
                label={t('review.statFileName')}
                value={maskFileName(validation.fileName)}
              />
              <StatChip
              ...
              />
              <StatChip
                label={t('review.statDelimiter')}
                value={showDelimiter(validation.delimiter)}
              />
              <StatChip
                label={t('review.statRowCount')}
                value={validation.rowCount.toLocaleString()}
              />
            </div>
          )}
        ...
        <div className="grid gap-3">
          ...
          <div className="grid cols-2">
            ...
            <MappingSelect
              ...
              headers={parsed.headers}
              onChange={(v) =>
                setMapping({ ...mapping, merchantName: v || undefined })
              }
            />
            <MappingSelect
              ...
              headers={parsed.headers}
              onChange={(v) =>
                setMapping({ ...mapping, category: v || undefined })
              }
            />
            <MappingSelect
              ...
              headers={parsed.headers}
              onChange={(v) =>
                setMapping({ ...mapping, accountName: v || undefined })
              }
            />
          </div>
          <div className="row">
            <label className="field">
              ...
              >
                <option value="negative_is_outflow">
                  {t('review.signOptionNegative')}
                </option>
                <option value="positive_is_outflow">
                  {t('review.signOptionPositive')}
                </option>
              </select>
            </label>
          </div>
      ...
      {step === 3 && (
        <div className="grid gap-3">
          <div className="grid kpi-row-4">
            <StatChip
              label={t('review.statTotalRows')}
              value={summary.totalRows.toLocaleString()}
            />
            <StatChip
              label={t('review.statAcceptedRows')}
              value={summary.acceptedRows.toLocaleString()}
            />
            <StatChip
              label={t('review.statExcludedRows')}
              value={summary.excludedRows.toLocaleString()}
            />
            <StatChip
              label={t('review.statReviewRows')}
              value={summary.reviewRows.toLocaleString()}
            />
          </div>
          <div className="grid cols-2">
            <BucketCard
              title={t('review.bucketHighImpact')}
              lines={buildImpactLines(drafts)}
            />
            <BucketCard
      ...
      {step === 5 && (
        <div className="grid gap-3">
          <div className="grid kpi-row-4">
            ...
            />
            <StatChip
              label={t('review.statRecurringSuggestions')}
              value={recurring.length.toLocaleString()}
            />
          </div>
          <p className="muted-note">
      ...
      {step === 6 && completion && (
        <div className="grid gap-3">
          <div className="grid kpi-row-4">
            <StatChip
              label={t('review.statAcceptedTxns')}
              value={completion.accepted.toLocaleString()}
            />
            <StatChip
              label={t('review.statExcludedTxns')}
              value={completion.excluded.toLocaleString()}
            />
            <StatChip
              label={t('review.statRemainingReview')}
              value={completion.review.toLocaleString()}
            />
            <StatChip
              label={t('review.statBaselineStatus')}
              value={
                completion.accepted > 0
                  ? t('review.baselineAvailable')
                  : t('review.baselineEmpty')
              }
            />
          </div>
          <div className="row">
...
function ReviewQueue({
  ...
  const filtered = useMemo(
    () => rows.filter((r) => matchReviewFilter(r, filter)),
    [rows, filter],
  )

  const applyStatus = async (
    row: ReviewItemRecord,
    ...
    <div className="card">
      ...
      {loading ? (
        ...
        <div className="life-os-scroll-x mt-3">
          ...
          {filtered.length === 0 && (
            <p className="muted-note">{t('review.noItemsInFilter')}</p>
          )}
        </div>
      )}
...
function BaselineWindowCard({
  ...
  return (
    <>
      <div className="grid kpi-row-4">
        <StatChip
          label={t('review.baselineAvgSpending')}
          value={money(w.averageMonthlySpending, privacy)}
        />
        <StatChip
          label={t('review.baselineMedianSpending')}
          value={money(w.medianMonthlySpending, privacy)}
        />
        <StatChip
          label={t('review.baselineMonthlyIncome')}
          value={money(Math.abs(w.monthlyIncome), privacy)}
        />
        <StatChip
          label={t('review.baselineMonthlyNet')}
          value={signedMoney(w.monthlyNetCashFlow, privacy)}
        />
      </div>
      <div className="grid baseline-kpi-pair">
        <StatChip
          label={t('review.baselineRecurring')}
          value={money(w.recurringSpending, privacy)}
        />
        <StatChip
          label={t('review.baselineOneTime')}
          value={money(w.oneTimeSpending, privacy)}
        />
      </div>
        ...
        windows.map((w) => (
          <div className="card" key={w.windowMonths}>
            <div className="card-head">
              ...
              <span
                className={`tag${w.confidence === 'Not ready' ? ' warn' : ''}`}
              >
...
function CalibrationView({
  ...
  const [windowMonths, setWindowMonths] = useState<3 | 6 | 12>(
    defaultWindow as 3 | 6 | 12,
  )
  const [calibrationMode, setCalibrationMode] = useState<'item' | 'category'>(
    'item',
  )
  ...
  const itemRows = useMemo(
    () =>
      buildItemCalibrationRows(data.cashFlows, occurrences, {
    ...
    for (const row of chosen) {
      const target =
        row.sourceId != null
          ...
          : data.cashFlows.find(
              (c) =>
                c.type === 'expense' && (c.category ?? c.name) === row.category,
            )
      if (!target) continue
  ...
  return (
    <div className="card">
      ...
      <p className="muted-note">
        {activeMode === 'item'
          ? t('review.calibrateModeItem')
          : t('review.calibrateModeCategory')}
      ...
      <div className="row">
        <label className="field">
          ...
          <select
            ...
            value={windowMonths}
            onChange={(e) =>
              setWindowMonths(Number(e.target.value) as 3 | 6 | 12)
            }
          >
        ...
        {itemRows.length > 0 && (
          <label className="field">
            ...
            <select
              ...
              value={activeMode}
              onChange={(e) =>
                setCalibrationMode(e.target.value as 'item' | 'category')
              }
            >
              <option value="item">
                {t('review.calibrateModeItemOption', {
        ...
        <table className="review-table">
          <thead>
            <tr>
              ...
              <th>
                {activeMode === 'item'
                  ? t('review.colPlanItem')
                  : t('review.colPlanCategory')}
              </th>
              {activeMode === 'item' && <th>{t('review.colHits')}</th>}
              <th>{t('review.colPlannedMonthly')}</th>
              <th>
                {activeMode === 'item'
                  ? t('review.colActualMedian')
                  : t('review.colActualBaseline')}
              </th>
      ...
      <div className="grid kpi-row-4 mt-3">
        <StatChip
          label={t('review.calibrateMonthlyDelta')}
          value={signedMoney(-monthlyDelta, privacy)}
        />
        <StatChip
          label={t('review.calibrateInvestableDelta')}
          value={signedMoney(-monthlyDelta, privacy)}
        />
        <StatChip
          label={t('review.calibrateDelta1y')}
          value={signedMoney(-yearlyDelta, privacy)}
        />
        <StatChip
          label={t('review.calibrateDelta5y')}
          value={signedMoney(-(yearlyDelta * 5), privacy)}
        />
      </div>
      <div className="grid cols-2 mt-2">
        <StatChip
          label={t('review.calibrateDelta10y')}
          value={signedMoney(-(yearlyDelta * 10), privacy)}
        />
        <StatChip
          label={t('review.calibrateStsPreview', {
            safeToSpend: quoteSafeToSpend(),
          })}
...
function buildImpactLines(drafts: NormalizedTransactionDraft[]): string[] {
  ...
  return [
    t('review.impactMirror', {
      count: flagCount('mirror_duplicate_candidate'),
    }),
    t('review.impactSameFileDup', {
      count: flagCount('same_account_duplicate_candidate'),
    }),
    t('review.impactTransfer', { count: flagCount('likely_transfer') }),
    t('review.impactCcPayment', {
      count: flagCount('likely_credit_card_payment'),
    }),
    t('review.impactUncategorized', {
      count: flagCount('large_uncategorized'),
    }),
  ]
}

function matchReviewFilter(
  ...
  if (filter === 'duplicates') {
    return (
      item.reviewType.includes('duplicate') ||
      item.reviewType.includes('reimport')
    )
  }
  if (filter === 'transfer') {
    return (
  ...
  if (filter === 'uncategorized')
    return item.reviewType.includes('uncategorized')
