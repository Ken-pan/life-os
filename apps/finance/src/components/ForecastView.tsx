...
export function ForecastView({
  ...
  const adj = (v: number, m: number) =>
    adjustForDisplay(v, m, a.displayMode, a.inflation)
  ...
  const lowVal = adj(read(projection.conservative[endIdx]), endIdx)
  const highVal = adj(read(projection.aggressive[endIdx]), endIdx)
  ...
  const displayModeNote =
    a.displayMode === 'today'
      ? t('forecast.displayToday')
      : t('forecast.displayFuture')
      ...
      <div className="card forecast-card">
        ...
        <div className="forecast-controls-mobile chart-controls">
          ...
          <SortBySelect
            ...
            options={FORECAST_METRICS.map((m) => ({
              id: m,
          ...
          <button
            ...
            aria-label={t('forecast.metricHintAria', {
        ...
        <div className="forecast-controls-desktop">
          ...
          <div className="chart-controls forecast-controls forecast-controls-metrics mt-2">
            <div className="forecast-controls-cluster">
              <span
                className="seg seg-scroll life-os-scroll-x life-os-scroll-x--snap life-os-scroll-x--fade-edge forecast-metric-seg"
                role="group"
            ...
            <div className="forecast-controls-cluster forecast-controls-cluster-end">
              <span className="text-muted forecast-display-mode">
                {displayModeNote}
              </span>
            </div>
          </div>
        ...
        <div className="forecast-hero">
          <div className="forecast-hero-main">
            ...
            {metric === 'accessible' && (
              <p className="forecast-hero-breakdown text-muted">
                ...
                {nowSnap.reserve > 0 && (
                  <>
                    {' '}
                    {t('forecast.reservePart', {
                      amount: money(nowSnap.reserve, privacy),
                    })}
                  </>
                )}
                ...
                {nowSnap.property > 0 && (
                  <>
                    {' '}
                    {t('forecast.propertyPart', {
                      amount: money(nowSnap.property, privacy),
                    })}
                  </>
                )}
                {nowSnap.liabilities > 0 && (
                  <>
                    {' '}
                    {t('forecast.liabilitiesPart', {
                      amount: money(nowSnap.liabilities, privacy),
                    })}
                  </>
                )}
              </p>
            ...
            {metric === 'liquid' && cashAnchors?.hasAnchoredAccounts && (
              <p className="forecast-hero-breakdown text-muted">
                {t('forecast.inTransitNote', {
                  amount: money(cashAnchors.cacheLiquid, privacy),
                })}
              </p>
            )}
          </div>
              ...
              {reaching.map((r) => (
                <li key={r.goal.id}>
                  <div className="forecast-milestone-main">
                    <span className="forecast-milestone-name">
                      {r.goal.name}
                    </span>
                    <span className="forecast-milestone-meta">
                      {t('forecast.goalTarget', {
                        amount: money(r.goal.target, privacy),
                      })}
                    </span>
                  </div>
                  <span className="forecast-milestone-when text-muted">
                    {t('forecast.goalEta', {
                      when: monthOffsetToCalendarLabel(r.month as number),
                    })}
                  </span>
                </li>
              ))}
        ...
        {reaching.length === 0 &&
          data.goals.some((g) => g.metric !== metric) && (
