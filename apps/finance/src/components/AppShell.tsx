...
function useNavConfig(): {
  ...
  return useMemo(() => {
    ...
    const mobilePrimaryTabs = mobilePrimaryIds.map((id) => tabs.find((tab) => tab.id === id)!);
    const mobileMoreGroups = [
      ...navGroups
        .map((group) => ({
          label: group.label,
          items: group.items.filter((tab) => !mobilePrimaryIds.includes(tab.id)),
        }))
...
export function AppShell() {
  ...
  const { navGroups, settingsTab: settingsNavTab, tabs, mobilePrimaryTabs, mobileMoreGroups } = useNavConfig();
  ...
  const [recordsTab, setRecordsTab] = useState<RecordsSection>(initial.recordsTab);
  const [forecastTab, setForecastTab] = useState<ForecastSection>(initial.forecastTab);
  const [reviewTab, setReviewTab] = useState<ReviewTab>(initial.reviewTab);
  const [settingsTab, setSettingsTab] = useState<SettingsSection>(initial.settingsTab);
  ...
  const switchTab: GoTab = (next, section, opts) => {
    ...
    else if (next !== "history" || section !== "insights") setLedgerSearch(undefined);
    if (opts?.focusEventId) setFocusEventId(opts.focusEventId);
    else if (next !== "history" || section !== "oneoff") setFocusEventId(undefined);
  ...
  let activeFab: { label: string; icon: LucideIcon; onClick: () => void } | null =
    null;
  if (tab === "history" && recordsTab === "oneoff") {
    activeFab = { label: t("nav.fabLargeCashflow"), icon: Plus, onClick: () => setPlanDrawer(true) };
  } else if (tab === "today" || tab === "overview" || tab === "history") {
    activeFab = { label: t("nav.fabLogTxn"), icon: Plus, onClick: () => setTxnDrawer(true) };
  }
  ...
  return (
    <div className="app">
      ...
      <aside className="sidebar">
        <div className="brand">
          <img src="/assets/brand/mark-on-dark.svg" alt="" className="brand-mark" width={28} height={28} />
      ...
      {!drawer && !txnDrawer && !planDrawer && activeFab && (
        <button className="fab" onClick={activeFab.onClick} aria-label={activeFab.label}>
        ...
        <div className="mobile-tabbar-inner">
          {mobilePrimaryTabs.map((tabItem) => {
            ...
            return (
              ...
              >
                <Icon {...MOBILE_ICON} weight={activeTab ? "fill" : "regular"} />
                  ...
                  {group.items.map((t) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {activeTab ? (
                          <span className="mobile-more-row-check" aria-hidden="true">
                            ✓
                          </span>
                        ) : (
                          <ChevronRight
