...
export function LedgerProductStrip({
  ...
  return (
    <ul
      className="ledger-product-strip"
      aria-label={tl('history.ledgerProductsAria')}
    >
      {shown.map((item) => {
        const imgSrc = lineItemImageSrc(item)
        return (
          <li
            className="ledger-product-chip"
            key={item.asin || item.detailUrl || item.title}
          >
            {imgSrc && !privacy && (
              <img
            ...
            <span className="ledger-product-chip-text">
              ...
              {(item.quantity ?? 1) > 1 && (
                <span className="ledger-product-chip-qty">
                  {' '}
                  ×{item.quantity}
                </span>
              )}
