...
export function SettingsView({
  ...
  const [internalSection, setInternalSection] = useState<SettingsSection>("accounts");
  ...
  const selectRestoreFile = async (file?: File) => {
    ...
    try {
      ...
      if (!check.ok) {
        setDataActionResult(
          t("settings.resultRestoreValidationFailed", { errors: check.errors.join("；") })
        );
        return;
      }
  ...
  return (
    <div className="settings-page">
      ...
      >
        <TabPanel tabId="accounts" active={activeSection === "accounts"} className="settings-section">
        ...
        <div className="card">
          ...
          <div className="row">
            ...
            <NumberField
              ...
              onChange={(v) => store.setAssumptions({ horizonYears: Math.max(1, Math.min(40, Math.round(v))) })}
              step={1}
              min={1}
            />
          </div>
          <div className="row">
            <PercentField
              ...
              value={a.investRatio}
              onChange={(v) => store.setAssumptions({ investRatio: Math.min(1, Math.max(0, v)) })}
            />
            <div className="field">
            ...
            <div className="grid gap-3">
              ...
              <div className="row">
                ...
                <PercentField
                  ...
                  onChange={(v) =>
                    store.setAssumptions({ capitalGainsTaxRate: Math.min(1, Math.max(0, v)) })
                  }
                />
              </div>
            </div>
          ...
          {openExpert && (
            <div className="list mt-1">
              <div className="kv">
                ...
                <span className="text-secondary">{t("settings.expertDisplayModeSub")}</span>
              </div>
              <div className="kv">
                <span className="k">{t("settings.expertBasicFirst")}</span>
                <span className="text-secondary">{t("settings.expertBasicFirstSub")}</span>
              </div>
              <div className="kv">
                <span className="k">{t("settings.expertAdvanced")}</span>
                <span className="text-secondary">{t("settings.expertAdvancedSub")}</span>
              </div>
            </div>
          )}
        </div>
        ...
        <TabPanel tabId="app" active={activeSection === "app"} className="settings-section">
          ...
          <SettingsSection title={t("settings.uiPrefs")}>
            <SettingsPrefRow label={t("settings.amountDisplay")} desc={t("settings.amountDisplayDesc")}>
              <button
                ...
                title={
                  store.data.privacy ? t("settings.showAmountsTitle") : t("settings.hideAmountsTitle")
                }
                aria-label={
                  store.data.privacy ? t("settings.showAmountsTitle") : t("settings.hideAmountsTitle")
                }
              >
                {store.data.privacy ? <Eye size={16} strokeWidth={1.8} /> : <EyeOff size={16} strokeWidth={1.8} />}
          ...
          <SettingsSection title={t("settings.dataTitle")} testId="settings-backup">
            ...
            <p className="muted-note">{t("settings.dataBackupScope", { backupNote })}</p>
            {store.data.privacy && (
              <p className="muted-note">{t("settings.dataPrivacyBackupNote")}</p>
            )}
            {lastBackupAt && (
              ...
              <div className="kv kv-top">
                ...
                <div className="kv-actions">
                  <input
                    ...
                    onChange={(e) => void selectRestoreFile(e.target.files?.[0])}
                    disabled={dataActionBusy}
                  />
                  {restoreCandidate && (
                    <>
                      <span className="text-secondary" style={{ textAlign: "right", maxWidth: 380 }}>
