...
import type { Account, CashFlowItem, HoldingsSnapshot } from "../types";
...
export interface CaptureEnvelope {
  ...
  timezone?: string;
  pageUrl?: string;
  data: HoldingsCaptureData | AccountsCaptureData | TransactionsCaptureData | RecurringCaptureData;
}
...
export function txnDedupKey(date: string, merchant: string, amount: number): string {
...
export function buildAppSnapshot(
  ...
  const txnKeys = options.privacy ? [] : recent.map((t) => txnDedupKey(t.date, t.merchant, t.amount));
...
export function isAppSnapshot(v: unknown): v is AppSnapshot {
  ...
  return s.v === 1 && typeof s.exportedAt === "string" && Array.isArray(s.accounts);
}
...
const KNOWN_ETFS = new Set([
  "VOO", "VTI", "SPY", "QQQ", "IVV", "VUG", "SCHD", "VYM", "VXUS",
  "BND", "AGG", "DIA", "IWM", "SMH", "SOXX", "VGT", "XLK", "VEA",
  "VWO", "GLD", "IAU", "VNQ", "SCHB", "SPLG", "QQQM", "FXAIX",
]);
...
function mergeCapturedPosition(a: CapturedPosition, b: CapturedPosition): CapturedPosition {
...
export function dedupeCapturedPositions(positions: CapturedPosition[]): CapturedPosition[] {
...
export function holdingsCaptureToSnapshot(
  ...
  const holdings = dedupeCapturedPositions(data.positions).map((p) => {
    ...
    const marketValue = enriched.marketValue ?? round2(enriched.shares * enriched.price);
    ...
    return {
      ...
      sourceCapturedAt: `${env.asOfDate} ${env.asOfTimeLocal ?? ""} ${env.timezone ?? ""}`.trim(),
    };
  });
  ...
  const weighted = holdings.map((h) => ({
    ...
    calculatedPortfolioWeightPct:
      totalMarketValue > 0 ? round2((h.marketValue / totalMarketValue) * 100) : undefined,
  }));
...
export function planHoldingsBalanceUpdate(
  ...
  return planOneBalance(matches[0], data.totalValue, notes, false, env.capturedAt);
}
...
export function planAccountsBalanceUpdate(
  ...
  for (const row of data.accounts) {
    ...
    if (seen.has(target.id)) {
      notes.push(`「${row.name}」与前面某行匹配到同一账户「${target.name}」，跳过。`);
      continue;
    }
    seen.add(target.id);
    const plan = planOneBalance(target, row.balance, notes, row.approximate, env.capturedAt);
...
function matchAccountRow(
  ...
  if (byName.length > 1 && row.institution) {
    const byInst = byName.filter((a) => normalize(a.name).includes(normalize(row.institution!)));
    if (byInst.length === 1) return byInst[0];
  }
  // 1c) 名称没中但有机构 + 类型提示：机构名出现在账户名里且类型唯一。
  const kindToType: Record<string, Account["type"]> = {
    checking: "checking",
    savings: "savings",
    credit: "credit-card",
  ...
  const type = row.kindHint ? kindToType[row.kindHint] : undefined;
  if (byName.length === 0 && row.institution && type) {
    const byInstType = accounts.filter(
      (a) => a.type === type && normalize(a.name).includes(normalize(row.institution!))
    );
...
function planOneBalance(
  ...
  return {
    updates: [
      { ...account, balance: next, updatedAt: capturedAt ?? new Date().toISOString() },
    ],
    notes,
  };
}
...
function captureRowToTxn(row: CapturedTxnRow, _source: CaptureSource): NewTxn {
  ...
  switch (flow) {
    case "income":
      return { ...base, flow, amount: -abs, budgetImpact: 0, inSpending: false, inCashFlow: true };
    case "refund_or_reversal":
      return { ...base, flow, amount: -abs, budgetImpact: abs, inSpending: true, inCashFlow: true };
    case "expense":
      return { ...base, flow, amount: abs, budgetImpact: -abs, inSpending: true, inCashFlow: true };
    case "ignored":
      return {
        ...base, flow, amount: row.credit ? -abs : abs, budgetImpact: 0,
        inSpending: false, inCashFlow: false, excludeReason: "extension-import-ignored",
      };
    default:
...
export function planRecurringUpdates(
  ...
  for (const row of data.rows) {
    ...
    if (matched.length > 1) {
      notes.push(`订阅「${row.name}」匹配到 ${matched.length} 个支出项，跳过金额核对。`);
      continue;
    }
    const target = matched[0];
    const freq = captureFrequency(row.frequency);
...
export async function computeEnvelopePayloadHash(env: CaptureEnvelope): Promise<string> {
  const canonical = canonicalEnvelopeJson(env);
...
export function newTxnToExtensionSyncPayload(t: NewTxn): Record<string, unknown> {
  return {
    date: t.date,
    merchant: t.merchant,
    category: t.category,
...
export function isCaptureEnvelope(v: unknown): v is CaptureEnvelope {
  ...
  return (
    e.v === 1 &&
    typeof e.id === "string" &&
    (e.source === "robinhood" || e.source === "rocketmoney" || e.source === "fidelity") &&
    (e.kind === "holdings" ||
      e.kind === "accounts" ||
      e.kind === "transactions" ||
      e.kind === "recurring") &&
