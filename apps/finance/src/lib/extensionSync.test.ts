...
import type { Account, CashFlowItem } from "../types";
...
const accounts: Account[] = [
  { id: "acct-robinhood-6853", name: "Robinhood individual", type: "brokerage", balance: 130000 },
  { id: "acct-chk", name: "BofA Checking", type: "checking", balance: 200 },
  { id: "acct-sav", name: "Marcus Savings", type: "savings", balance: 4000 },
  { id: "acct-cc", name: "BofA Credit Card", type: "credit-card", balance: 6800 },
];

function holdingsEnvelope(overrides: Partial<CaptureEnvelope> = {}): CaptureEnvelope {
  return {
    v: 1,
...
describe("holdingsCaptureToSnapshot", () => {
  it("生成稳定 id 的快照并绑定 Robinhood 券商账户", () => {
    ...
    const again = holdingsCaptureToSnapshot(holdingsEnvelope({ id: "other" }), accounts);
...
describe("planHoldingsBalanceUpdate", () => {
  ...
  it("Fidelity 的 holdings totalValue 是多账户聚合值，不写任何账户", () => {
    const withFidBrokerage: Account[] = [
      ...accounts,
      { id: "acct-fid-brk", name: "Fidelity Brokerage", type: "brokerage", balance: 100 },
    ];
    ...
    expect(planHoldingsBalanceUpdate(env, withFidBrokerage).updates).toHaveLength(0);
  });

  it("账户在抓取之后更新过（时序保护）则跳过，且写入时 updatedAt 记抓取时刻", () => {
    // capture 于 7/2 20:00Z；账户 7/3 手动更新过 → 旧数据不倒灌
    const newer = accounts.map((a) =>
      a.id === "acct-robinhood-6853" ? { ...a, updatedAt: "2026-07-03T00:00:00Z" } : a
    );
    ...
    const older = accounts.map((a) =>
      a.id === "acct-robinhood-6853" ? { ...a, updatedAt: "2026-07-01T00:00:00Z" } : a
    );
...
describe("planAccountsBalanceUpdate", () => {
  ...
  it("Net Worth 近似值（3 位有效数字）在舍入误差内不更新，超出才更新", () => {
    const mk = (balance: number, approximate: boolean): CaptureEnvelope => ({
      ...env,
      data: { accounts: [{ name: "Robinhood individual", balance, approximate, institution: "Robinhood" }] },
    });
    // 精确值 130000，页面显示 $130k：落在近似容差内，不动
    expect(planAccountsBalanceUpdate(mk(130000, true), accounts).updates).toHaveLength(0);
    // 差 400（> 0.6%*130500 无，0.6%*129600=778 → 400 在容差内）
    expect(planAccountsBalanceUpdate(mk(129600, true), accounts).updates).toHaveLength(0);
    // 差 5000：明显变化，更新
    const plan = planAccountsBalanceUpdate(mk(125000, true), accounts);
    expect(plan.updates).toHaveLength(1);
    expect(plan.updates[0].balance).toBe(125000);
    // 同样差 400 若是精确值则更新
    expect(planAccountsBalanceUpdate(mk(129600, false), accounts).updates).toHaveLength(1);
  });

  it("名称多重命中时用机构名收窄（Robinhood Savings vs Chase Savings）", () => {
    const twoSavings: Account[] = [
      { id: "sav-chase", name: "CHASE SAVINGS", type: "savings", balance: 4380 },
      { id: "sav-rh", name: "Robinhood Savings", type: "savings", balance: 100 },
    ];
    const envNw: CaptureEnvelope = {
      ...env,
      data: {
        accounts: [
          { name: "Savings", balance: 0.77, kindHint: "savings", institution: "Robinhood" },
        ],
      },
    };
  ...
  it("Fidelity 账户列表：retirement/hsa kindHint 唯一匹配", () => {
    ...
    const envFid: CaptureEnvelope = {
      ...
      data: {
        accounts: [
          { name: "IM 401K SAVINGS PLAN", balance: 57572.08, kindHint: "retirement", institution: "Fidelity" },
          { name: "Health Savings Account", balance: 1894.5, kindHint: "hsa", institution: "Fidelity" },
        ],
      },
    };
...
describe("planRecurringUpdates", () => {
  const cashFlows: CashFlowItem[] = [
    { id: "cf-broadband", name: "Astound Broadband", type: "expense", frequency: "monthly", amount: 55 },
    { id: "cf-openai", name: "OpenAI", type: "expense", frequency: "monthly", amount: 20 },
    { id: "cf-membership", name: "Credit Card Membership Fee", type: "expense", frequency: "annual", amount: 95 },
    { id: "cf-income", name: "Salary", type: "income", frequency: "monthly", amount: 8000 },
  ];
  const env: CaptureEnvelope = {
    v: 1,
    ...
    data: {
      rows: [
        { name: "Astound Broadband", frequency: "Monthly", group: "Subscriptions", amount: 60 },
        { name: "OpenAI", frequency: "Monthly", group: "Subscriptions", amount: 20 },
        { name: "Credit Card Membership Fee", frequency: "Irregular", group: "Subscriptions", amount: 95 },
        { name: "Planet Fitness", frequency: "Monthly", group: "Subscriptions", amount: 6 },
      ],
    },
  };
  ...
  it("income 类 cashFlow 不参与匹配", () => {
    const envIncome: CaptureEnvelope = {
      ...env,
      data: { rows: [{ name: "Salary", frequency: "Monthly", group: "Other", amount: 1 }] },
    };
...
describe("planNewTransactions", () => {
  const rows: CapturedTxnRow[] = [
    { date: "2026-06-30", merchant: "Paris Baguette", category: "Dining & Drinks", amount: 6.3, credit: false, pending: false },
    { date: "2026-06-30", merchant: "Online/mobile Recurring", category: "Internal Transfers", amount: 5.6, credit: true, pending: false },
    { date: "2026-07-02", merchant: "Lemonade", category: "Bills & Utilities", amount: 5.08, credit: false, pending: true },
    { date: "2026-06-29", merchant: "Amazon Purchase", category: "Shopping", amount: 39.99, credit: false, pending: false },
  ];
  const env: CaptureEnvelope = {
    v: 1,
    id: "rm_txn_test",
  ...
  const existing: Txn[] = [
    {
      date: "2026-06-29", month: "2026-06", merchant: "Amazon Purchase",
      category: "Shopping", account: "Rocket Money", flow: "expense",
      amount: 39.99, budgetImpact: -39.99, inSpending: true, inCashFlow: true,
    },
  ];
  ...
  it("同日同商户同金额：按计数去重（multiset），两笔真交易都能入账", () => {
    const dupEnv: CaptureEnvelope = {
      ...
      data: {
        rows: [
          ...
          { date: "2026-07-01", merchant: "Blue Bottle", category: "Dining & Drinks", amount: 5.75, credit: false, pending: false, platformId: "id-1" },
          { date: "2026-07-01", merchant: "Blue Bottle", category: "Dining & Drinks", amount: 5.75, credit: false, pending: false, platformId: "id-2" },
        ],
      },
    };
    // 已有 1 笔 → 消耗配额后再插 1 笔
    const oneExisting: Txn[] = [{
      date: "2026-07-01", month: "2026-07", merchant: "Blue Bottle",
      category: "Dining & Drinks", account: "Rocket Money", flow: "expense",
      amount: 5.75, budgetImpact: -5.75, inSpending: true, inCashFlow: true,
    }];
    ...
    const keylessEnv: CaptureEnvelope = {
      ...env,
      data: {
        rows: (dupEnv.data as TransactionsCaptureData).rows.map((r: CapturedTxnRow) => ({
          ...r,
  ...
  it("跳过 pending 与已有重复，符号约定与手动记账一致", () => {
    ...
    const transfer = plan.txns.find((t) => t.merchant === "Online/mobile Recurring")!;
...
describe("flowForCaptureRow", () => {
  ...
  it("按类别与方向映射 flow", () => {
    expect(flowForCaptureRow({ ...base, category: "Income", credit: true })).toBe("income");
    expect(flowForCaptureRow({ ...base, category: "Credit Card Payment", credit: false })).toBe("credit_card_payment");
    expect(flowForCaptureRow({ ...base, category: "Savings Transfer", credit: false })).toBe("internal_transfer");
    expect(flowForCaptureRow({ ...base, category: "Shopping", credit: true })).toBe("refund_or_reversal");
    expect(flowForCaptureRow({ ...base, category: "Shopping", credit: false })).toBe("expense");
  });
});

describe("buildAppSnapshot", () => {
  const cashFlows: CashFlowItem[] = [
    { id: "cf-1", name: "OpenAI ChatGPT Plus", type: "expense", frequency: "monthly", amount: 20 },
  ];
  const txns: Txn[] = [
    {
      date: "2026-06-01", month: "2026-06", merchant: "Coffee", category: "Dining",
      account: "Rocket Money", flow: "expense", amount: 5, budgetImpact: -5,
      inSpending: true, inCashFlow: true,
    },
    {
      date: "2026-07-01", month: "2026-07", merchant: "Coffee", category: "Dining",
      account: "Rocket Money", flow: "expense", amount: 5, budgetImpact: -5,
      inSpending: true, inCashFlow: true,
    },
  ];
  ...
  it("隐私模式只导出最小同步元数据", () => {
    const snap = buildAppSnapshot(accounts, txns, cashFlows, [], { privacy: true });
...
describe("resolveTxnScrollStopBefore", () => {
  it("优先取更晚的增量停止日期，避免每次扫完整历史", () => {
    ...
    const withOldest = buildAppSnapshot([], [{
      date: "2026-06-10", month: "2026-06", merchant: "x", category: "x",
      account: "Rocket Money", flow: "expense", amount: 1, budgetImpact: -1,
      inSpending: true, inCashFlow: true,
    }], [], []);
    expect(resolveTxnScrollStopBefore(withOldest, "2026-07-01")).toBe("2026-06-28");
...
describe("filterNewCaptureTxnRows", () => {
  it("按 multiset 跳过 app 已有交易", () => {
    const snap = buildAppSnapshot([], [{
      date: "2026-07-01", month: "2026-07", merchant: "Paris Baguette", category: "Dining",
      account: "Rocket Money", flow: "expense", amount: 6.3, budgetImpact: -6.3,
      inSpending: true, inCashFlow: true,
    }], [], []);
    const rows: CapturedTxnRow[] = [
      { date: "2026-07-01", merchant: "Paris Baguette", category: "Dining", amount: 6.3, credit: false, pending: false },
      { date: "2026-07-02", merchant: "New Shop", category: "Shopping", amount: 10, credit: false, pending: false },
    ];
    const { rows: need, skippedDuplicate } = filterNewCaptureTxnRows(rows, snap, "rocketmoney");
...
describe("accountRowUnchangedInApp", () => {
  it("余额在容差内则跳过", () => {
    ...
    expect(accountRowUnchangedInApp(
      { name: "BofA Checking", balance: 200.2 },
      snap
    )).toBe(true);
    expect(accountRowUnchangedInApp(
      { name: "BofA Checking", balance: 500 },
      snap
    )).toBe(false);
  ...
  it("近似余额放宽容差", () => {
    const snap = buildAppSnapshot(
      [{ id: "x", name: "Fidelity Brokerage", type: "brokerage", balance: 57700 }],
      [], [], []
    );
    expect(accountRowUnchangedInApp(
...
describe("recurringRowAlreadyInApp", () => {
  it("名称频率金额一致则跳过", () => {
    ...
    expect(recurringRowAlreadyInApp(
      { name: "OpenAI", frequency: "Monthly", group: "Subscriptions", amount: 20 },
      snap
    )).toBe(true);
    expect(recurringRowAlreadyInApp(
      { name: "Netflix", frequency: "Monthly", group: "Subscriptions", amount: 15 },
      snap
    )).toBe(false);
  });
});
...
describe("nameMatches", () => {
  it("整词包含才算匹配，防止 substring 误配", () => {
    ...
    expect(nameMatches("Robinhood individual", "Robinhood Individual")).toBe(true);
...
describe("isCaptureEnvelope", () => {
  it("校验合法与非法 envelope", () => {
    ...
    expect(isCaptureEnvelope({ v: 1, id: "x", source: "evil", kind: "holdings", asOfDate: "2026-01-01", data: {} })).toBe(false);
    expect(isCaptureEnvelope({ v: 1, id: "x", source: "rocketmoney", kind: "recurring", asOfDate: "2026-01-01", data: { rows: [] } })).toBe(true);
  });
});
...
describe("computeEnvelopePayloadHashSync", () => {
  it("同一 envelope 产生稳定哈希", () => {
    ...
    expect(computeEnvelopePayloadHashSync(env)).toBe(computeEnvelopePayloadHashSync(env));
  });

  it("payload 变化则哈希变化", () => {
    const a = holdingsEnvelope({ id: "a" });
    const b = holdingsEnvelope({ id: "b" });
    expect(computeEnvelopePayloadHashSync(a)).not.toBe(computeEnvelopePayloadHashSync(b));
  });
});
