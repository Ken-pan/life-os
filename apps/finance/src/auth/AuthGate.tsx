...
import { loadFinanceData, persistFinanceSetup, saveLocale, seedFinanceData } from "../lib/repo";
...
export function AuthGate() {
  ...
  const applyData = useCallback((data: FinanceData) => {
  ...
  const { syncBidirectional, scheduleBidirectionalSync, resetCooldown } = cloudSync;
  ...
  useEffect(() => {
    ...
    return bindVisibilitySync(
      () => {
        void scheduleBidirectionalSync();
      },
      { when: () => phaseRef.current === "ready" }
    );
