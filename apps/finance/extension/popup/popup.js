...
function renderRunSummary(parent, state, perf) {
  ...
  const netWorthBits = [];
  if (Number.isFinite(stats.netWorthFound)) netWorthBits.push(`发现 ${stats.netWorthFound}`);
  if (Number.isFinite(stats.netWorthNeed)) netWorthBits.push(`需同步 ${stats.netWorthNeed}`);
  ...
  if (Number.isFinite(stats.dashboardSuppressed) && stats.dashboardSuppressed > 0) {
    netWorthBits.push(`已省略 Dashboard ${stats.dashboardSuppressed}`);
  } else if (Number.isFinite(stats.dashboardQueued) && stats.dashboardQueued > 0) {
    netWorthBits.push(`Dashboard 兜底 ${stats.dashboardQueued}`);
  }
  appendSummaryLine(summary, "余额", netWorthBits.join(" · "));

  const txnBits = [];
  if (Number.isFinite(stats.newTransactions)) txnBits.push(`新 ${stats.newTransactions}`);
  if (Number.isFinite(stats.skippedTxns) && stats.skippedTxns > 0) txnBits.push(`已有 ${stats.skippedTxns}`);
  if (Number.isFinite(stats.rawTransactions)) txnBits.push(`扫描 ${stats.rawTransactions}`);
  appendSummaryLine(summary, "交易", txnBits.join(" · "));

  const recurBits = [];
  if (Number.isFinite(stats.recurNeed)) recurBits.push(`需同步 ${stats.recurNeed}`);
  ...
  const perfBits = [];
  if (Number.isFinite(lastRun?.totalMs)) perfBits.push(`总耗时 ${fmtDuration(lastRun.totalMs)}`);
  if (slow) perfBits.push(`最慢 ${CRAWL_PHASE_LABEL[slow.phase] ?? slow.phase} ${fmtDuration(slow.ms)}`);
...
async function renderCrawlState(queue = []) {
  const { fos_crawl_state: state, fos_crawl_log: log, fos_crawl_perf: perf } = await chrome.storage.local.get([
    "fos_crawl_state",
    "fos_crawl_log",
    "fos_crawl_perf",
  ]);
  ...
  btn.textContent = pendingRocket > 0 ? CRAWL_BUTTON_PENDING_TEXT : CRAWL_BUTTON_TEXT;
...
function renderRhEnrichState(rhEnrich, rhDetailsCount = 0) {
  ...
  if (!rhEnrich?.running && !(rhEnrich?.total > 0) && !rhEnrich?.failures?.length) {
  ...
  el.style.display = "block";
  el.className = rhEnrich.failures?.length && !rhEnrich.running ? "crawl-state error" : "crawl-state";
  clearNode(el);

  const running = rhEnrich.running === true;
  const total = Number(rhEnrich.total) || 0;
  const done = Number(rhEnrich.done) || 0;
  const progress = total > 0 ? Math.round((done / total) * 100) : running ? 5 : 100;
...
document.getElementById("crawl-rh-details")?.addEventListener("click", async () => {
  const btn = document.getElementById("crawl-rh-details");
  if (btn) btn.disabled = true;
  const res = await chrome.runtime.sendMessage({ type: "FOS_RH_ENRICH_MANUAL" });
  if (!res?.ok) {
    alert(
      res?.error === "no_robinhood_holdings"
        ? "队列里没有 Robinhood 持仓快照。请先打开 Robinhood 投资页抓取列表。"
        : "无法启动详情补齐，请重试。"
    );
...
document.getElementById("download-log").addEventListener("click", async () => {
  ...
  const status = await chrome.runtime.sendMessage({ type: "FOS_STATUS" }).catch(() => null);
  ...
  const tabs = await chrome.tabs
    ...
    .then((list) =>
      list.map((t) => ({
        ...
        isFinanceOs: /localhost|127\.0\.0\.1|netlify\.app/i.test(t.url ?? ""),
      }))
    )
    .catch(() => []);
...
/** 打开已有 Finance OS 标签页（本地 / Netlify），或新开 localhost:5173。 */
