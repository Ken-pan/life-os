...
(() => {
  ...
  async function loadAppSnapshot() {
    const { fos_app_snapshot: snap } = await chrome.storage.local.get("fos_app_snapshot");
  ...
  function planDetail(need, skipped, unit) {
    if (skipped > 0) return `${need} ${unit}需抓，${skipped} 已在 Finance OS 跳过`;
    return `${need} ${unit}需抓`;
  }
  ...
  function findAccountsCard() {
    ...
    return heads[0].closest('[data-testid="card"]') ?? heads[0].parentElement?.parentElement;
  }
  ...
  function isCollapsedChevron(rowEl) {
    const paths = [...rowEl.querySelectorAll("svg path")].map((p) => p.getAttribute("d") ?? "");
  ...
  function clickCollapsedDashboardAccountGroups() {
    ...
    if (!card) return 0;
    let clicked = 0;
    for (const rowEl of card.querySelectorAll('div[role="button"][aria-expanded="false"]')) {
      ...
      try {
        clickLikeUser(rowEl);
        clicked += 1;
        appendCrawlLog("info", "展开 Dashboard Accounts 分组", { group: label });
      } catch (e) {
        appendCrawlLog("warn", "展开 Dashboard Accounts 分组失败", {
          group: label,
  ...
  function inferDashboardKindHint(rowEl, card) {
    const ordered = [
      ...card.querySelectorAll('div[role="button"][aria-expanded], div[tabindex="0"]'),
    ];
    let kindHint;
  ...
  function parseDashboardAccountRow(rowEl, kindHint) {
    ...
    if (!logo) return null;
    const institution = logo.getAttribute("alt").replace(/ logo$/i, "").trim();
  ...
  function probeDashboardDetailAccounts() {
    const card = findAccountsCard();
    if (!card) return null;
    const hasExpanded = card.querySelector('div[role="button"][aria-expanded="true"]');
  ...
  function accountRowLikelySame(a, b) {
    ...
    const ai = String(a?.institution ?? "").trim().toLowerCase();
    const bi = String(b?.institution ?? "").trim().toLowerCase();
    if (ai && bi && (ai === bi || an.includes(bi) || bn.includes(ai))) return true;
    return false;
  }
  ...
  async function waitForDashboardAccounts() {
    ...
    while (Date.now() - start < PAGE_PROBE_TIMEOUT_MS) {
      ...
      const collapsedGroups = card
        ? [...card.querySelectorAll('div[role="button"][aria-expanded="false"]')].filter((el) => {
  ...
  function probeNetWorthAccounts() {
    ...
    for (const rowEl of document.querySelectorAll('[role="row"]')) {
      ...
      const logo = rowEl.querySelector('img[alt$=" logo" i]');
      if (!logo) continue;
      const ps = [...rowEl.querySelectorAll("p")].map((p) => p.textContent.trim()).filter(Boolean);
      if (ps.length < 1) continue;
      const name = ps[0];
      const institution = ps[1] ?? logo.getAttribute("alt").replace(/ logo$/i, "");
      // 余额取最右的 label（credit 行还有 % Utilization 列，money 解析可区分）。
      let money = null;
      const labels = [...rowEl.querySelectorAll("label")];
      for (let i = labels.length - 1; i >= 0; i--) {
        const t = labels[i].textContent.trim();
        if (t.includes("%")) continue;
        const parsed = parseAbbrevMoney(t);
        if (parsed) {
          money = parsed;
          break;
        }
  ...
  function dueTextToISO(text) {
    ...
    if (offsetDays == null) {
      ...
      else if ((m = t.match(/^(\d+) months? ago$/))) offsetDays = -Number(m[1]) * 30;
    }
  ...
  function parseRecurringRow(rowEl, group) {
    const labels = [...rowEl.querySelectorAll("label")].map((l) => l.textContent.trim());
    ...
    const dueText = labels.find((t) => /^in \d+|days? ago$|months? ago$|^today$|^tomorrow$/i.test(t));
    ...
    const platformId = controls ? controls.replace(/^recurring-list-/, "") : undefined;
    return {
      name,
      frequency,
      group,
  ...
  function probeRecurring() {
    ...
    const rows = [];
    for (const section of document.querySelectorAll('[data-testid="subscription-section-card"]')) {
      // 分区标题如 "7 Subscriptions" / "3 Bills & Utilities" / "1 Other"，去掉计数前缀。
      const header = section.querySelector('[data-testid="card-header"] p')?.textContent ?? "";
      const group = header.replace(/^\d+\s*/, "").trim() || "Recurring";
      for (const rowEl of section.querySelectorAll('[data-testid="table-row"]')) {
        const r = parseRecurringRow(rowEl, group);
        if (r) rows.push(r);
      }
    }
    return rows.length > 0 ? rows : null;
  ...
  function parseTxnRow(rowEl) {
    ...
    for (const c of cells) {
      const iso = monthDayToISO(c.textContent.trim());
      if (iso) {
        date = iso;
        break;
      }
    ...
    // 金额：从右往左第一个能解析成钱的 cell；"+$5.60" 表示入账。
    let amount = null;
    let credit = false;
    for (let i = cells.length - 1; i >= 0; i--) {
      ...
      if (v != null && /\$/.test(text)) {
        amount = Math.abs(v);
        credit = text.trim().startsWith("+");
        break;
      }
    }
    if (amount == null) return null;

    // 名称 cell：优先找含 "Statement Description" 的（展开区隐藏字段）；
    // 兜底：日期 cell 之后文本最长、且不是金额的 cell（跳过单字母头像 cell）。
    let nameCell = cells.find((c) => c.textContent.includes("Statement Description"));
    if (!nameCell) {
      const dateIdx = cells.findIndex((c) => monthDayToISO(c.textContent.trim()));
      let best = null;
      for (let i = dateIdx + 1; i < cells.length; i++) {
        const text = cells[i].textContent.trim();
        if (text.length < 2 || parseMoney(text) != null) continue;
        if (!best || text.length > best.text.length) best = { cell: cells[i], text };
      }
      nameCell = best?.cell ?? null;
    }
    if (!nameCell) return null;
    // 逐叶子元素取文本（比 innerText 换行更稳定）。
    const lines = [];
    const walk = (el) => {
    ...
    walk(nameCell);
    if (lines.length === 0 && nameCell.textContent.trim()) lines.push(nameCell.textContent.trim());
    ...
    // 类别 cell：名称 cell 之后第一个有文本、且不是金额的 cell。
  ...
  function probeTransactions() {
    const rows = document.querySelectorAll('[data-testid="transaction-table-row"]');
    if (rows.length === 0) return null;
    const parsed = [...rows].map(parseTxnRow).filter(Boolean);
  ...
  async function finalizeCrawlPerf(status, summary) {
    ...
    const recentRuns = [...(prev?.recentRuns ?? []), run].slice(-PERF_RECENT_RUNS_MAX);
    const phaseTotals = {};
      ...
      .then(() =>
        chrome.storage.local.set({
          [CRAWL_LOG_KEY]: {
            ...
            source: "rocketmoney",
            runId: crawlMeta?.runId,
            startedAt: new Date(crawlMeta?.startedAt ?? Date.now()).toISOString(),
            updatedAt: entries[entries.length - 1]?.at ?? new Date().toISOString(),
            entries,
          },
        })
      )
  ...
  function appendCrawlLog(level, message, extra) {
    ...
    if (crawlLogEntries.length > 300) crawlLogEntries = crawlLogEntries.slice(-300);
  ...
  function setCrawlState(phase, detail, options = {}) {
    ...
    const progress =
      clampProgress(options.progress) ?? CRAWL_PHASE_PROGRESS[phase] ?? CRAWL_PHASE_PROGRESS.starting;
      ...
      const extra =
        options.stats && typeof options.stats === "object"
          ...
          : options.stats != null
            ? { stats: options.stats }
            : {};
      if (options.level === "warn" && !extra.code) extra.code = "CRAWL_PHASE_WARN";
      if (options.level === "error" && !extra.code) extra.code = "CRAWL_STATE_ERROR";
      extra.phase = phase;
  ...
  function clickCollapsedNetWorthGroupRows() {
    ...
    const groupLabels = /^(assets with loans|investments|savings|cash|credit cards|auto loans|mortgages|other)$/i;
    let clicked = 0;
    for (const rowEl of document.querySelectorAll('[role="row"][tabindex="0"]')) {
      const labels = [...rowEl.querySelectorAll("label")].map((l) => l.textContent.trim()).filter(Boolean);
      const group = labels.find((t) => groupLabels.test(t));
      const pct = labels.find((t) => /% of (assets|debts)/i.test(t));
      if (!group || !pct) continue;
      const paths = [...rowEl.querySelectorAll("svg path")].map((p) => p.getAttribute("d") ?? "");
      const isCollapsed = paths.some((d) => /M8\s*10L12\s*14L16\s*10/i.test(d));
      const isExpanded = paths.some((d) => /M16\s*14L12\s*10L8\s*14/i.test(d));
      if (!isCollapsed || isExpanded) continue;
      try {
        const target = rowEl.querySelector('div[role="cell"]:last-child') ?? rowEl;
        clickLikeUser(target);
        clicked += 1;
        appendCrawlLog("info", "展开 Net Worth 分组行", { group, pct });
      } catch (e) {
        appendCrawlLog("warn", "展开 Net Worth 分组行失败", {
          group,
          error: e?.message ?? String(e),
        });
      }
  ...
  async function gotoPath(path, sidebarTestId) {
    ...
    if (link) {
      ...
      appendCrawlLog("info", `侧栏入口不存在，直接跳转 ${path}`, { sidebarTestId });
      location.assign(`${location.origin}${path}`);
    }
    const ok = (await waitFor(() => (isRouteMatch(path) ? true : null), NAV_TIMEOUT_MS)) != null;
    if (!ok) recordRouteTimeout(path, NAV_TIMEOUT_MS);
    appendCrawlLog(ok ? "info" : "warn", ok ? `已进入 ${path}` : `进入 ${path} 超时`, {
      path,
      sidebarTestId,
      currentPath: location.pathname,
    });
  ...
  function findTxnScroller() {
    const container = document.querySelector('[data-testid="transactions-page-list-container"]');
    if (container) {
    ...
    if (pageScroller && pageScroller.scrollHeight > pageScroller.clientHeight + 100) {
      return pageScroller;
    }
  ...
  function findLoadMoreButton() {
    for (const btn of document.querySelectorAll("button")) {
      if (/^load\s*more$/i.test(btn.textContent.trim()) && !btn.disabled) return btn;
    }
    return null;
  }
  ...
  async function scrollAndCollect(stopBefore) {
    ...
    const addVisibleRows = () => {
      let added = 0;
      for (const rowEl of document.querySelectorAll('[data-testid="transaction-table-row"]')) {
    ...
    let idleSteps = 0;
    for (let step = 0; step < CRAWL_MAX_STEPS && collected.size < CRAWL_MAX_ROWS; step++) {
      stepsTaken = step + 1;
      if (stopBefore) {
        let oldest = null;
        ...
        if (oldest != null && oldest < stopBefore) {
          ...
          appendCrawlLog("info", "交易滚动已越过停止点", { oldest, stopBefore, rows: collected.size });
          break;
        }
      }
      ...
      lastAdded = added;
      const progress = 70 + Math.min(25, Math.round((step / CRAWL_MAX_STEPS) * 25));
      setCrawlState("transactions", `已收集 ${collected.size} 笔交易…`, {
        progress,
        stats: { rows: collected.size, step: step + 1, maxSteps: CRAWL_MAX_STEPS, added },
      ...
      if (added === 0 && (atBottom || scroller.scrollTop === before)) {
        ...
        if (loadMore) {
          loadMoreClicks += 1;
          setCrawlState("transactions", `已收集 ${collected.size} 笔，加载下一页…`, {
            progress,
            stats: { rows: collected.size, step: step + 1, maxSteps: CRAWL_MAX_STEPS },
          });
          loadMore.click();
          // 下一批走网络请求，等待放宽；比普通滚动慢，但避免无谓等满 5 秒。
          await waitForMutationOrTimeout(scroller, TXN_LOAD_MORE_TIMEOUT_MS, TXN_LOAD_MORE_SETTLE_MS);
          addVisibleRows();
          idleSteps = 0;
          continue;
        }
        ...
        if (idleSteps >= 3) {
          ...
          stopReason = "bottom";
          appendCrawlLog("info", "交易列表已到底", { rows: collected.size, step: step + 1 });
          break;
        }
      } else {
        idleSteps = 0;
    ...
    if (crawlRunStats) {
      crawlRunStats.transactionScroll = {
        durationMs,
        rows: collected.size,
        rowsPerSecond: durationMs > 0 ? Math.round((collected.size / durationMs) * 100000) / 100 : 0,
        complete,
        stopReason,
        steps: stepsTaken,
        maxSteps: CRAWL_MAX_STEPS,
  ...
  async function startCrawl() {
    ...
    if (!plan) {
      setCrawlState("error", "扩展脚本不完整，请重新加载扩展", { level: "error" });
      return;
    }
    crawling = true;
    ...
    try {
      setCrawlState("starting", "读取 Finance OS 快照与抓取计划…", { progress: 3 });
      const snap = await loadAppSnapshot();
      if (snap) {
        if (snap.privacyRedacted) {
          console.info(`[FOS] 抓取计划：Finance OS ${snap.txnCount} 笔交易，隐私模式下快照已最小化`);
        ...
        console.info("[FOS] 尚无 Finance OS 快照：打开 Finance OS 后可跳过已同步数据");
        appendCrawlLog("warn", "尚无 Finance OS 快照，无法预先跳过已同步数据");
      }

      // 1) Dashboard：账户余额
      setCrawlState("dashboard", "进入 Dashboard，等待 Accounts 卡片…", { progress: 10 });
      const dashboardOk = await gotoPath("/dashboard", "sidebar-Dashboard");
      if (!dashboardOk) appendCrawlLog("warn", "Dashboard 跳转失败，继续尝试读取当前页面");
      const {
        rows: accounts,
      ...
      if (accounts?.length) {
        ...
        if (dashboardMode === "detail" && filtered.rows.length > 0) {
          await enqueue(makeEnvelope("rocketmoney", "accounts", { accounts: filtered.rows }));
          dashboardDetailQueued = true;
          acctNeed += filtered.rows.length;
          skippedAccounts += filtered.skipped;
        }
        setCrawlState("dashboard", "Dashboard 余额已读取，等待 Net Worth 精细账户确认…", {
          progress: 25,
          stats: {
        ...
        setCrawlState("dashboard", "等待 8 秒后仍未找到 Dashboard Accounts 卡片，已跳过", {
          progress: 25,
          level: "warn",
        });
      ...
      if (!netWorthOk) appendCrawlLog("warn", "Net Worth 跳转失败，继续尝试读取当前页面");
      const { rows: nwAccounts, expanded: nwExpanded } = await waitForNetWorthAccounts();
      ...
      if (nwAccounts) {
        ...
        let nwDeduped = 0;
        if (dashboardAccounts?.mode === "detail" && dashboardAccounts.rows.length > 0) {
          const deduped = filterNetWorthAgainstDashboardDetails(nwRows, dashboardAccounts.rows);
          nwRows = deduped.rows;
        ...
        if (nwRows.length > 0) {
          await enqueue(makeEnvelope("rocketmoney", "accounts", { accounts: nwRows }));
        }
        if (dashboardAccounts) {
          if (dashboardDetailQueued) {
            appendCrawlLog("info", "Dashboard 逐账户已入队，Net Worth 去重后补充", {
        ...
        setCrawlState("networth", planDetail(nwRows.length, netWorthSkipped, "账户"), {
          progress: 45,
          stats: {
            found: netWorthFound,
            need: netWorthNeed,
            ...
            dashboardSuppressed: dashboardDetailQueued ? 0 : dashboardSuppressed,
        ...
        if (dashboardAccounts && !dashboardDetailQueued) {
          ...
          if (dashboardAccounts.rows.length > 0) {
            await enqueue(makeEnvelope("rocketmoney", "accounts", { accounts: dashboardAccounts.rows }));
          }
          appendCrawlLog("warn", "Net Worth 未读取成功，已使用 Dashboard 余额兜底", {
            dashboardFound: dashboardAccounts.found,
            dashboardNeed: dashboardAccounts.need,
        ...
        setCrawlState(
            ...
            : "等待 8 秒后仍未找到 Net Worth 账户表，已跳过",
          {
            ...
            stats: {
              expanded: netWorthExpanded,
              dashboardFallbackQueued: dashboardFallbackQueued ? (dashboardAccounts?.need ?? 0) : 0,
            },
          }
        );
      ...
      // 3) Recurring：订阅 / 账单
      setCrawlState("recurring", "进入 Recurring，等待订阅列表…", { progress: 50 });
      const recurringOk = await gotoPath("/recurring", "sidebar-Recurring");
      if (!recurringOk) appendCrawlLog("warn", "Recurring 跳转失败，继续尝试读取当前页面");
      ...
      if (recurring) {
        ...
        if (filtered.rows.length > 0) {
          await enqueue(makeEnvelope("rocketmoney", "recurring", { rows: filtered.rows }));
        }
        setCrawlState("recurring", planDetail(filtered.rows.length, filtered.skipped, "订阅"), {
          progress: 65,
          stats: { found: recurring.length, need: filtered.rows.length, skipped: filtered.skipped },
        });
        console.info(
        ...
        setCrawlState("recurring", "等待 8 秒后仍未找到 Recurring 列表，已跳过", {
          progress: 65,
          level: "warn",
        });
      ...
      // 4) Transactions：滚动收集
      setCrawlState("transactions", "进入 Transactions，等待交易表…", { progress: 68 });
      const transactionsOk = await gotoPath("/transactions", "sidebar-Transactions");
      if (!transactionsOk) appendCrawlLog("warn", "Transactions 跳转失败，继续尝试读取当前页面");
      const initialTxnRows = await waitFor(probeTransactions, TXN_PROBE_TIMEOUT_MS);
      if (!initialTxnRows) {
        recordProbeTimeout("transactions", TXN_PROBE_TIMEOUT_MS);
        setCrawlState("transactions", "等待 10 秒后仍未找到交易表，将尝试滚动当前页面", {
          progress: 70,
          level: "warn",
        });
      }
      ...
      const { rows, skippedDuplicate } = plan.filterNewCaptureTxnRows(rawRows, snap, "rocketmoney");
      skippedTxns = skippedDuplicate;
      if (rows.length > 0) {
        // complete 标记：只有「从最新无空洞收集到 watermark/底部」的爬取才允许推进水位线。
        await enqueue(makeEnvelope("rocketmoney", "transactions", { rows, complete }));
      }
      const doneMsg =
        `完成：${rows.length} 笔新交易` +
      ...
      setCrawlState("done", doneMsg, {
        ...
        stats: {
          ...
          dashboardQueued: dashboardFallbackQueued ? dashboardAccounts?.need ?? 0 : 0,
          dashboardSuppressed,
          netWorthFound,
          netWorthNeed,
          netWorthSkipped,
      ...
      setCrawlState("error", e?.stack ?? errMsg, { level: "error", progress: 100 });
  ...
  void chrome.storage.local.get("fos_pending_crawl").then(({ fos_pending_crawl }) => {
    ...
    if (/transactions/i.test(path)) {
      cancel = captureWhenStable({
        ...
        capture: (rows) => {
          ...
          const sig = JSON.stringify(rows.map((r) => r.platformId ?? `${r.date}|${r.merchant}|${r.amount}`));
          if (sig === lastTxnSig) return;
          lastTxnSig = sig;
          void (async () => {
            const snap = await loadAppSnapshot();
            const { rows: need, skippedDuplicate } = plan.filterNewCaptureTxnRows(
              rows,
              snap,
              "rocketmoney"
            );
            if (need.length === 0) {
              if (skippedDuplicate > 0) {
                console.info(`[FOS] RocketMoney 交易 ${skippedDuplicate} 笔已在 Finance OS，跳过`);
              }
              return;
            }
            await enqueue(makeEnvelope("rocketmoney", "transactions", { rows: need }));
            console.info(
      ...
      cancel = captureWhenStable({
        ...
        capture: (accounts) => {
          ...
          void (async () => {
            const snap = await loadAppSnapshot();
            const { rows: need, skipped } = plan.filterAccountRows(accounts, snap);
            if (need.length === 0) {
              if (skipped > 0) console.info(`[FOS] Net Worth ${skipped} 账户已在 Finance OS，跳过`);
              return;
            }
            await enqueue(makeEnvelope("rocketmoney", "accounts", { accounts: need }));
            console.info(
      ...
      cancel = captureWhenStable({
        ...
        capture: (rows) => {
          ...
          void (async () => {
            ...
            if (need.length === 0) {
              if (skipped > 0) console.info(`[FOS] Recurring ${skipped} 项已在 Finance OS，跳过`);
              return;
            }
            await enqueue(makeEnvelope("rocketmoney", "recurring", { rows: need }));
            console.info(
      ...
      cancel = captureWhenStable({
        ...
        capture: (accounts) => {
          ...
          void (async () => {
            ...
            const { rows: need, skipped } = plan.filterAccountRows(accounts, snap);
            if (need.length === 0) {
              if (skipped > 0) console.info(`[FOS] Dashboard ${skipped} 组余额已在 Finance OS，跳过`);
              return;
            }
            await enqueue(makeEnvelope("rocketmoney", "accounts", { accounts: need }));
