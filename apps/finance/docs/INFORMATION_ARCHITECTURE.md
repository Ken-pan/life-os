...
Finance OS 是个人财务驾驶舱，一级导航收敛为 **首页 / 资金 / 规划 / 审查 / 设置** 五个平行域：

| 层级         | 导航分组 | 核心问题                             |
| ------------ | -------- | ------------------------------------ |
| **首页**     | 首页     | 今天要做什么？资产全景如何？         |
| **资金**     | 资金     | 账户在哪改？钱怎么流？投资配置如何？ |
| **前瞻规划** | 规划     | 5–30 年后会怎样？重大选择怎么比？    |
| **数据治理** | 审查     | 导入、排错、对账与计划校准           |
| **系统配置** | 设置     | 回报假设、备份与设备                 |

数据流闭环：**账户（资金域）→ 记录/审查（真实数据）→ 首页（短期决策）→ 规划（长期规划）**。

---

## 2. 路由与导航机制

### 2.1 路径路由格式（History API）

| 类型         | 格式                  | 示例                                          |
| ------------ | --------------------- | --------------------------------------------- |
| 一级 Tab     | `/{tab}`              | `/accounts`、`/review`                        |
| 二级 Section | `/{tab}/{section}`    | `/home/today`、`/decision/compare`            |
| Query 参数   | `?snapshot={id}`      | 仅 **资产配置** 页（不进路径）                |
| 默认首页     | `/`                   | 启动归一化为 `/home/today`                    |
| Legacy hash  | `#/{tab}[/{section}]` | 仍解析；`main.tsx` 启动时 `replaceState` 迁移 |

映射表：`docs/IA_ROUTE_MAPPING.csv`
...
### 2.2 一级 Tab 清单（9 个路由 tab，5 个导航域）

| ID         | 导航域 | 中文名     | 默认 Section  | 有 URL 二级 Tab |
| ---------- | ------ | ---------- | ------------- | --------------- |
| `home`     | 首页   | 首页       | `today`       | 是（2 个）      |
| `accounts` | 资金   | 账户       | —             | 否              |
| `stocks`   | 资金   | 资产配置   | —             | 否              |
| `history`  | 资金   | 记录       | `insights`    | 是（3 个）      |
| `forecast` | 规划   | 预测       | `forecast`    | 是（2 个）      |
| `decision` | 规划   | 决策工作室 | `compare`     | 是（3 个）      |
| `review`   | 审查   | 审查       | `import`      | 是（5 个）      |
| `settings` | 设置   | 设置       | `assumptions` | 是（3 个）      |

**Legacy alias（只读兼容）**：`today` / `overview` → `home/{section}`；`settings/accounts` → `accounts`。
...
当路由为 `history` / `stocks` / `decision` / `settings` 时，底栏 **「更多」** 高亮（`isMoreNavActive`）。
