...
> **Implementation status (2026-07-13 — DEPLOYED to production, owner-authorized).** The association + decision model, state machine, versioning, idempotency and Undo are implemented as a tested pure engine (`packages/finance-core/src/engine/purchaseReviewDecision.ts`, 14/14) and a migration **deployed to prod `iueozzuctstwvzbcxcyh`** (`20260713120000_purchase_review_associations.sql`: `purchase_associations`, `purchase_decisions`, RLS + 5 policies, `purchase_review_get/_decide/_undo`, **273 transactions backfilled → proposed**). RPC state-machine logic was **verified end-to-end on production** via a self-cleaning round-trip (confirm/replay/not_proposed/version_conflict/undo all correct, net-zero data change). **Still open:** RLS cross-user *runtime* denial proof (needs two real JWT sessions — superuser API bypasses RLS), matcher precedence wiring, UI. Field/name specifics below are design intent; the shipped engine/migration are the source of truth for exact shapes.
