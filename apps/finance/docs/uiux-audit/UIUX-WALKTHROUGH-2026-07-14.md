...
逐个 tab 截图复检修复后基线,并发现 2 个**与本次修复无关的既有 bug**,已一并修掉:

| Tab | 结论 |
|---|---|
| 今日 / 总览 / 账户 / 记录 / 资产配置 / 决策 | ✅ 干净,本次修复(G1/A2/S1/G5/D1/D2/D3)均在真实 app 生效 |
| **预测** | 🐛→✅ **图表叠字**:`.forecast-chart-wrap` 移动端 CSS 高 280,但 `<Chart height={340}>` 写死 → SVG 溢出 60px,底部月份轴标签压到「1年后构成」段落。修:给 ForecastChart/ForecastSplitChart 加 `height` prop,ForecastView 按 `matchMedia(720px)` 传 280/340,与 wrap CSS 对齐。实测 `svgOverlapsStrip: false` |
| **设置** | 🐛→✅ **裸 i18n key**:外观页竖屏开关显示 `settings.lockPortraitOnPhone`/`...Desc` 原文——catalog 缺该键族(连 `rotatePortrait` 也缺)。修:补 4 个键(中英),现正常显示「手机锁定竖屏」 |
| 审查 | ⚠️ 小 nit:「选择 .csv 文件」label 与上方说明间距偏紧(非阻塞,未改) |

> 终检价值验证:静态走查 + 之前几轮都没覆盖到这两个页面深处的运行时 bug,只有登录态逐页实测才暴露。

`svelte-check` 0 errors;i18n 一致性通过;两处修复均 DOM 量化 + 截图双验证。

### 追加:移动端页头布局(用户反馈)

🐛→✅ **移动端 bug 图标跑左上 + 标题独占第二行**。根因:theme 移动端 `.page-header` 是 grid（`'brand trailing' / 'titles titles'`，为有 logo 的 app 设计），但 finance 把反馈按钮当**裸子元素**塞进 header → 被自动排进左上的 `brand` 空格子;且 finance 移动端无 logo（侧栏 `display:none`），标题白占第二行。

修（纯 finance，不动 theme）:①`AppShell.svelte` 把「更新时间 + 反馈按钮」包进 `.page-header-actions`;②finance app.css 移动端把页头收成单行 `'titles trailing'`（标题左、工具右）+ 给 actions `grid-area: trailing`。实测:桌面不变（logo 左上侧栏 + bug 右上）、移动端标题与 bug 同一行、页头高 100→65px。
