...
三端应用共用各仓库内 `packages/life-os-theme/src/layout.css`（通过 `@import '@life-os/theme/layout.css'` 引入）。本文档为 **走查结论 + 统一方案**，供三项目对齐实施。
...
1. 修改断点 / tabbar token 时，同步三个仓库的 `packages/life-os-theme/src/layout.css` 与 `layout.js`
...
- `packages/life-os-theme/src/layout.css` — tablet 断点 + tabbar tokens
