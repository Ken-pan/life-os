...
async function capture(page, viewportId, name, checks = {}) {
  ...
  if (!ok) {
    if (overflow.overflowPx > 1)
      console.log(`  overflow: ${overflow.overflowPx}px`)
...
for (const vp of VIEWPORTS) {
  ...
  await clickMainTab(
    ...
    vp.id,
    '今日',
    vp.id === 'desktop' ? undefined : 'primary',
  )
...
process.exit(
  results.some((r) => r.failedChecks.length > 0 || r.overflow.overflowPx > 1)
    ? 1
    : 0,
)
