...
const shotDir = resolve(root, "docs/ui-qa-screenshots/mobile-spacing-audit");
...
async function measureLayout() {
  return page.evaluate(() => {
    ...
    const firstCard = document.querySelector(".content .card, .content .grid > .card, .content .grid > *");
    ...
    if (padTop > padX + 4) {
      issues.push(`content 上内边距 ${padTop}px > 左右 ${padX}px，首屏可能头重脚轻`);
    }
    if (padX !== 16 && padX !== 12) {
      issues.push(`content 左右内边距 ${padX}px 非 16/12 token`);
    }

    const gapHeaderToContent = headerRect && contentRect
      ? Math.round(firstRect ? firstRect.top - headerRect.bottom : contentRect.top - headerRect.bottom)
      : null;

    if (firstCard) {
      const cardPad = px(cs(firstCard).paddingTop);
      if (cardPad === 14) issues.push(`卡片 padding ${cardPad}px（640 断点 14px 与 16px 体系不一致）`);
    }
...
for (const tab of tabs) {
  if (tab.action === "more") {
    ...
    await page.locator(`nav.mobile-tabbar button[aria-label="${tab.nav}"]`).click();
