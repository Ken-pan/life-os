...
const shotDir = resolve(root, "docs/ui-qa-screenshots/daily-candles-e2e");
...
check(
  "app_ready",
  !bodyAfterLogin.includes("请使用你的账户登录") && (bodyAfterLogin.includes("今日") || bodyAfterLogin.includes("Finance")),
...
check("stocks_page", stocksBody.includes("投资建议") || stocksBody.includes("持仓"), stocksBody.slice(0, 160));

await page.waitForTimeout(12000);

check("ychart_fetched", ychartRequests.length > 0, `${ychartRequests.length} requests`);
...
check("supabase_read", !readErr, readErr?.message ?? `${rowsBefore?.length ?? 0} recent rows`);

const symbolSet = new Set((rowsBefore ?? []).map((r) => String(r.symbol).toUpperCase()));
check("candles_persisted", (rowsBefore?.length ?? 0) > 0, `symbols: ${[...symbolSet].slice(0, 5).join(", ")}`);
...
await page.screenshot({ path: resolve(shotDir, "stocks-after-sync.png"), fullPage: true });
...
writeReport();
console.log(`\nE2E ${report.pass ? "PASSED" : "FAILED"} — report: docs/ui-qa-screenshots/daily-candles-e2e/report.json`);
