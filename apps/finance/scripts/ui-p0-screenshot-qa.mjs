...
const shotDir = resolve(root, "docs/ui-qa-screenshots/p0-mobile");
...
if (bodyText.includes("请使用你的账户登录") || bodyText.includes("需要配置 Supabase") || bodyText.includes("正在加载")) {
  await page.screenshot({ path: `${shotDir}/00_auth_failed.png`, fullPage: true });
...
async function checkTab(name, buttonLabel, checks) {
  const nav = page.locator(".mobile-tabbar").getByRole("button", { name: buttonLabel });
  ...
  results.push({ name, path, overflow: report, failedChecks, textSample: text.slice(0, 200) });
}
...
await page.locator(".mobile-tabbar").getByRole("button", { name: "今日" }).first().click();
await page.waitForTimeout(800);
await page.evaluate(() => window.scrollTo(0, document.body.scrollHeight));
await page.waitForTimeout(400);
await page.screenshot({ path: `${shotDir}/04_today_scrolled_bottom.png`, fullPage: false });

// More sheet
await page.locator(".mobile-tabbar").getByRole("button", { name: "更多" }).click();
...
const tabbarOpacity = await page.evaluate(() => {
  ...
  return { className: el.className, opacity: style.opacity, pointerEvents: style.pointerEvents };
});
results.push({
  name: "05_more_sheet",
  tabbarBackgrounded: tabbarOpacity?.className.includes("is-backgrounded") ?? false,
...
for (const r of results) {
  if (r.failedChecks) {
    ...
    if (r.overflow) {
      console.log(`  overflow: ${r.overflow.overflowPx}px (scroll ${r.overflow.scrollWidth} vs viewport ${r.overflow.viewportWidth})`);
    ...
    console.log(`INFO ${r.name} tabbar backgrounded=${r.tabbarBackgrounded} opacity=${r.tabbarOpacity}`);
  }
}
