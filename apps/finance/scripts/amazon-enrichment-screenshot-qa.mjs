...
const shotRoot = resolve(root, `docs/ui-qa-screenshots/amazon-enrichment-${dateTag}`);
...
async function goHistoryInsights(page) {
  ...
  if (!body.includes("全部流水") && !body.includes("All transactions")) {
    await page.screenshot({ path: resolve(shotRoot, "debug-history-failed.png"), fullPage: true });
...
for (const vp of VIEWPORTS) {
  ...
  manifest.push(await capture(page, vp.id, "01-history-insights-top", { fullPage: vp.id === "desktop" }));
