...
export function getServiceClient() {
  ...
  if (!key) {
    throw new Error("Missing SUPABASE_SERVICE_ROLE_KEY for image upload");
  }
...
async function uploadLineItemImage(item, { userId, source, orderId, supabase }) {
  const merchantUrl = item.imageUrl;
  if (!merchantUrl) return item;
  if (item.imageStoragePath && /\/storage\/v1\/object\/public\//.test(item.imageUrl || "")) {
    return item;
  }
    ...
    uploadCache.set(
      ...
      (async () => {
        try {
          ...
          const { error } = await supabase.storage.from(PURCHASE_IMAGES_BUCKET).upload(path, buf, {
            contentType,
            upsert: true,
          });
          if (error) throw error;
          const { data } = supabase.storage.from(PURCHASE_IMAGES_BUCKET).getPublicUrl(path);
...
export async function uploadPurchaseEnrichmentImages(enrichment, options) {
  ...
  for (const item of enrichment.lineItems) {
    lineItems.push(await uploadLineItemImage(item, { userId, source, orderId, supabase }));
  }
