...
const SUB_TABS = {
  history: [["Insights", "洞察"], ["Recurring", "固定收支"], ["One-off", "大额收支"]],
  forecast: [["Forecast curve", "预测曲线"], ["Long-range plan", "长期规划"]],
  review: [
    ["Import", "导入交易"],
    ["Review queue", "审查队列"],
...
function classifyChinese(line) {
  ...
  if (/[\$¥€£][\d,]+/.test(line) && /[\u4e00-\u9fff]/.test(line) && line.length > 30) return "data";
...
if (localeUpdate.error) {
  console.warn("Could not set server locale (column may be missing):", localeUpdate.error.message);
}
...
async function injectSession(page) {
  ...
  const nav = page.locator(".sidebar").getByRole("button", { name: /Today|今日/ });
  try {
    await nav.first().waitFor({ state: "visible", timeout: 30000 });
  } catch {
    await page.screenshot({ path: resolve(shotRoot, "desktop", "00_boot_failed.png"), fullPage: true });
...
async function clickMainTab(page, label) {
  const btn = page.locator(".sidebar").getByRole("button", { name: new RegExp(`^${label}$`, "i") });
  if (await btn.count()) {
...
async function capture(page, name) {
  ...
  console.log(`${status} ${name} — ${uiChinese.length} UI / ${chinese.length} total Chinese line(s)`);
...
const englishBtn = page.locator(".settings-section .seg button", { hasText: "English" });
...
if (!bodyText.includes("Today") && !bodyText.includes("Safe to spend")) {
  await page.screenshot({ path: resolve(shotRoot, "desktop", "00_locale_failed.png"), fullPage: true });
...
for (const tab of MAIN_TABS) {
  ...
  if (subs) {
    for (const labels of subs) {
      ...
      const slug = (hit ?? labels[0]).toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "");
...
if (await fab.count()) {
  ...
  await page.screenshot({ path: resolve(shotRoot, "desktop", "drawer-txn.png"), fullPage: false });
  const text = await page.locator("body").innerText();
  audit.push({
    name: "drawer-txn",
  ...
  const closeBtn = page.locator(".drawer").getByRole("button", { name: /Close|关闭/ });
...
if (await spendBtn.count()) {
  ...
  await page.waitForTimeout(600);
  await page.screenshot({ path: resolve(shotRoot, "desktop", "drawer-spend.png"), fullPage: false });
  const text = await page.locator("body").innerText();
  audit.push({
    name: "drawer-spend",
...
writeFileSync(
  ...
  JSON.stringify({ date: dateTag, baseUrl, locale: "en-US", audit, summary: { total: audit.length, withCjk: withCjk.length } }, null, 2)
);
