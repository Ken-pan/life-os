...
| P4b/d/e state 维度 + CommandPalette + chrome tokens | ✅ `2a7ad397`        |
| P5 Pixel baseline（80 PNG）                         | ✅ `2a7ad397`        |
| D-P2 结构层 generated + D-P3 前置债                 | ✅ `5b7d4014`        |
| P6 a11y gates（47 Playwright checks）               | ✅ 本次              |
...
npm run test:design-catalog              # smoke only（172；排除 @visual @a11y）
npm run test:design-catalog:a11y         # a11y gates（47；contrast / focus / touch / motion）
...
npm run test:design-catalog:all            # smoke + a11y + snapshots（299 tests）
```
...
输出目录：`docs/ui-qa-screenshots/design-catalog/p3-audit/{runId}/`（gitignore，勿提交）
...
Spec：`tests/visual/design-catalog.a11y.spec.ts`（`@a11y` tag）

| 门禁           | 范围                                                |
| -------------- | --------------------------------------------------- |
| Contrast ≥ 3:1 | btn-primary / btn-danger / toast × 4 app × 2 mode   |
| Focus ring     | primary / segment / toast dismiss / settings action |
| Touch ≥ 44px   | btn-primary / segment / toast dismiss × 4 app       |
| Reduced motion | global transition + toast + command palette         |
...
结构 token 在 `packages/design-tokens/tokens/structural.json`（生成 `generated/tokens.css`）：
