...
function checkExpectations(p, locale, pageId) {
  ...
  const mustHidden = (key, label) => {
    ...
    if (Array.isArray(el)) {
      ...
      checks.push({
        ok: bad.length === 0,
        msg: bad.length
          ? `${label} should be hidden but visible: ${bad.map((b) => b.text).join(', ')}`
          : `${label} hidden ✓`,
      })
    } else if (el.found) {
      checks.push({
        ok: !el.visible,
        msg: el.visible
          ? `${label} should be hidden but visible: "${el.text}"`
          : `${label} hidden ✓`,
      })
    }
  }
  ...
  if (pageId === '01-home') {
    ...
    if (p.coachHead.found) {
      checks.push({
        ok: p.coachHead.visible,
        msg: p.coachHead.visible
          ? `Coach panel header visible ✓ (${p.coachHead.innerText || p.coachHead.text})`
          : 'Coach panel header should be visible when coach panel is shown',
      })
    }
    ...
    if (
      p.eyebrow.visible &&
      /rotation|循环|Chest · Back|胸 · 背/i.test(
        p.eyebrow.innerText || p.eyebrow.text,
      )
    ) {
      checks.push({
        ok: false,
      ...
      checks.push({
        ok: true,
        msg: `Eyebrow date-only ✓ (${p.eyebrow.innerText || p.eyebrow.text})`,
      })
    }
  }
  ...
  if (pageId === '06-focus-home') {
    ...
    checks.push({
      ...
      msg: !p.tag.visible
        ? 'No visible decor tags on focus ✓'
        : 'Focus should have no decor tags',
    })
  }
...
for (const locale of /** @type {const} */ (['en', 'zh'])) {
  ...
  for (const { id, path, wait } of CORE) {
    ...
    await page.screenshot({
      ...
      fullPage: id === '04-settings' || id === '12-stats',
    })
