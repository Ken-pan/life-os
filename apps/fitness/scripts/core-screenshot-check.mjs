...
const OUT = join(process.cwd(), 'screenshots/core-review');
...
await seed(page, {
  logs: {
    [`${dateKey()}|chest`]: {
      c_bench: { sets: [{ weight: 185, reps: 8, rir: 2 }, { weight: 185, reps: 8, rir: 2 }, null, null] }
    }
  }
});
await page.goto(`${BASE}/`);
...
await seed(page, {
  ...
  sessionMeta: {
    [`${dateKey()}|chest`]: { startedAt: new Date(Date.now() - 45 * 60000).toISOString() }
  }
});
...
await seed(page, {
  logs: {
    ...
    [`${dateKey(-3)}|chest`]: {
      c_bench: { sets: [{ weight: 185, reps: 8, rir: 2 }, { weight: 185, reps: 8, rir: 2 }] }
    }
  },
  rotation: {
    next: 0,
...
await page.locator('.tool-card', { hasText: '容量' }).locator('.tool-head').click();
