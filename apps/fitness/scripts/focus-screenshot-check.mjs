...
const OUT = join(process.cwd(), 'screenshots/focus-review');
...
async function seed(page, data = {}) {
  ...
  await page.evaluate((d) => {
    ...
    s.settings = { unit: 'lbs', logDetail: 'quick', plateCollarLbs: 0, plateCollarKg: 0, ...d.settings };
...
// 15. 短视口 + 重量弹窗
if (await sePage.locator('.sheet').isVisible()) await sePage.keyboard.press('Escape');
