...
async function seed(page, data = {}) {
  ...
  await page.evaluate((d) => {
    ...
    if (d.programOverrides !== undefined)
      s.programOverrides = d.programOverrides
...
async function scrollSheet(page) {
  await page.locator('.ptp-focus-scroll').evaluate((el) => {
    el.querySelector('.pb-barbell, .pb-viz')?.scrollIntoView({
      block: 'center',
    })
  })
}
...
async function waitSheetSettled(page) {
  ...
  await page.waitForFunction(() => {
    ...
    return (
      Math.abs(sb.bottom - window.innerHeight) < 3 &&
      fb.bottom <= window.innerHeight + 1
    )
  })
}
...
async function assertBarbellGeometry(page) {
  const m = await page.evaluate(() => {
    ...
    const bar = document
      .querySelector('.pb-barbell-bar')
      ?.getBoundingClientRect()
    const core = document
      .querySelector('.pb-barbell-core')
      ?.getBoundingClientRect()
    const leftCollar = document
      .querySelector('.pb-side--left .pb-collar')
      ?.getBoundingClientRect()
    const rightCollar = document
      .querySelector('.pb-side--right .pb-collar')
      ?.getBoundingClientRect()
    const leftOuter = document
      .querySelector('.pb-side--left .pb-plate:first-of-type')
      ?.getBoundingClientRect()
    const rightOuter = document
      .querySelector('.pb-side--right .pb-plate:last-of-type')
      ?.getBoundingClientRect()
    const leftInner = document
      .querySelector('.pb-side--left .pb-plate:last-of-type')
      ?.getBoundingClientRect()
    if (
      !shaft ||
      !bar ||
    ...
    const orderFromCenter = (side) => {
      const nodes = [
        ...document.querySelectorAll(`.pb-side--${side} .pb-plate`),
      ]
    ...
    return {
      ...
      barCoversPlates:
        bar.left <= leftOuter.left + 1 && bar.right >= rightOuter.right - 1,
      leftCollarOrder:
        leftInner.right <= leftCollar.left + 2 &&
        leftCollar.right <= shaft.left + 4,
  ...
  if (m.gapDelta > 1)
    throw new Error(`左右杆身间隙不对称: L${m.leftGap} R${m.rightGap}`)
  ...
  if (!m.mirrored)
    throw new Error(`左右装片顺序未镜像: L[${m.leftOrder}] R[${m.rightOrder}]`)
  ...
  const bad = await page.evaluate(() => {
    ...
    for (const el of document.querySelectorAll('.ptp-focus-step, .ptp-more')) {
      ...
      if (r.width < 44 || r.height < 44) {
        fails.push(
          `${el.className} ${Math.round(r.width)}×${Math.round(r.height)}`,
        )
      }
    }
...
async function assertBarbellInViewport(
  page,
  { topPad = 48, bottomPad = 64, scroll = true } = {},
) {
  if (scroll) {
  ...
  const ok = await page.locator('.pb-barbell').evaluate(
    (el, pads) => {
      ...
      const footer = document
        .querySelector('.ptp-focus-footer')
        ?.getBoundingClientRect()
      const bottomLimit = footer
        ? Math.min(window.innerHeight - pads.bottom, footer.top - 4)
        : window.innerHeight - pads.bottom
...
async function assertCompactFocus(page) {
  ...
  const sheetH = await page
    .locator('.tool-sheet--focus')
    .evaluate((el) => el.getBoundingClientRect().height)
  const vh = page.viewportSize()?.height ?? 874
  if (sheetH > vh * 0.58)
    throw new Error(`Focus sheet 过高: ${sheetH}px / ${vh}px`)
  ...
  const textOnGraphic = await page
    .locator('.pb-barbell .pb-plate-num, .pb-barbell .pb-shaft-label')
    .count()
...
const compactH = await page
  .locator('.tool-sheet--focus')
  .evaluate((el) => el.getBoundingClientRect().height)
await page.evaluate((h) => {
  window.__focusSheetCompactH = h
}, compactH)
...
await expectVisible(page, '.ptp-focus-weight')
const expandedH = await page
  .locator('.tool-sheet--focus')
  .evaluate((el) => el.getBoundingClientRect().height)
const storedCompactH = await page.evaluate(() => window.__focusSheetCompactH)
if (expandedH <= storedCompactH)
  throw new Error(`展开后高度应大于精简态: ${expandedH} <= ${storedCompactH}`)
...
if (!heavy || heavy.plates < 10)
  throw new Error(`495 铃片数异常: ${JSON.stringify(heavy)}`)
if (heavy.scrollW > heavy.clientW + 2)
  throw new Error(`495 仍有横向溢出: ${JSON.stringify(heavy)}`)
if (parseFloat(heavy.scale) >= 1)
  throw new Error(`495 应缩小铃片: scale=${heavy.scale}`)
await assertBarbellGeometry(page)
await scrollSheet(page)
...
if (seExpandedBar.top < 36)
  throw new Error(`SE 展开后杠铃被顶栏遮挡: top=${seExpandedBar.top}`)
