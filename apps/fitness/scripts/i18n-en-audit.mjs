 ...
 *
 * Outputs:
 *   screenshots/i18n-en-audit/*.png
 *   screenshots/i18n-en-audit/report.json
 *   screenshots/i18n-en-audit/decor-report.json
 *   screenshots/i18n-en-audit/decor-report.md
 */
...
const OUT_EN = join(process.cwd(), 'screenshots/i18n-en-audit');
const OUT_ZH = join(process.cwd(), 'screenshots/i18n-zh-audit');
      ...
      for (const node of el.childNodes) {
        if (node.nodeType === Node.TEXT_NODE) {
          ...
          if (aria && re.test(aria) && !seen.has(aria)) {
            seen.add(aria);
            hits.push({ text: aria.slice(0, 120), tag: el2.tagName.toLowerCase(), cls: 'aria-label' });
          }
          walk(el2);
        }
      }
...
async function collectDecor(page) {
  return page.evaluate(() => {
    ...
    const nearbyTitle = (el) => {
      const header = el.closest('.sec-header, .coach-head, .hero-copy, .today-card, .set-group, .stat-card');
      if (!header) return undefined;
      const title = header.querySelector('.sec-title, .hero-title, .stat-v, h1, h2');
    ...
    const autoMap = {
      ...
      kicker: '.hero-kicker:not([data-ui-decor]), .hm-kicker:not([data-ui-decor])',
      'section-label': '.cycle-label:not([data-ui-decor]), .sg-title:not([data-ui-decor])',
      'callout-label': '.co-label:not([data-ui-decor]), .tc-label:not([data-ui-decor])',
      'meta-strip': '.appbar-meta:not([data-ui-decor]), .coach-sub:not([data-ui-decor])',
...
function summarizeDecor(id, items) {
  ...
  const hidden = items.filter((it) => it.hidden || UI_DECOR_ROLES[it.role]?.hide);
  const visible = items.filter((it) => !it.hidden && !UI_DECOR_ROLES[it.role]?.hide);
  return {
    id,
    total: items.length,
...
function writeDecorReport(locale, allDecor, outDir) {
  const pages = allDecor.map(({ id, url, items, summary }) => ({ id, url, summary, items }));
  const matrix = UI_DECOR_HIDE_MATRIX[locale];

  const catalog = Object.entries(UI_DECOR_ROLES).map(([role, meta]) => ({ role, ...meta }));
  ...
  const lines = [
    ...
    '|------|-------|--------|------|',
    ...catalog.map((c) => `| \`${c.role}\` | ${c.value} | ${c.hide ? 'yes' : 'no'} | ${c.note} |`),
    '',
  ...
  for (const p of pages) {
    ...
    if (p.summary.visible.length) {
      for (const it of p.summary.visible) {
        lines.push(`- \`${it.role}\` — "${it.text}"${it.nearby ? ` _(near: ${it.nearby})_` : ''}`);
      }
    } else {
...
async function audit(page, id, outDir, opts = {}) {
  ...
  if (opts.wait) await page.locator(opts.wait).first().waitFor({ timeout: 15000 });
...
async function runSuite(locale, page, outDir) {
  ...
  report.push(await audit(page, '01-home', outDir, { goto: '/', wait: '.hero-title' }));
  report.push(await audit(page, '02-program', outDir, { goto: '/program', wait: '.sec-title' }));
  report.push(await audit(page, '03-discover', outDir, { goto: '/discover', wait: '.discover-grid' }));
  report.push(await audit(page, '04-settings', outDir, { goto: '/settings', wait: '.sec-title', fullPage: true }));
  report.push(await audit(page, '05-day-overview', outDir, { goto: '/day/chest', wait: '.day-title' }));

  await page.goto(`${BASE}/day/chest/focus`);
  await page.locator('.focus-ex-name').waitFor();
  report.push(await audit(page, '06-focus-home', outDir, { wait: '.focus-ex-name' }));

  await page.locator('.focus-weight').click();
  await page.locator('.modal.wtm').waitFor();
  report.push(await audit(page, '07-weight-modal', outDir, { wait: '.modal.wtm' }));
  await page.keyboard.press('Escape');

  await page.locator('.focus-plates-link').click();
  await page.locator('.tool-sheet').waitFor();
  report.push(await audit(page, '08-plates-sheet', outDir, { wait: '.tool-sheet' }));
  await page.keyboard.press('Escape');
  ...
  report.push(await audit(page, '09-skip-modal', outDir, { wait: '.skip-reasons' }));
  await page.keyboard.press('Escape');

  await page.goto(`${BASE}/day/chest/summary`);
  await page.locator('.summary-grid').waitFor({ timeout: 10000 }).catch(() => {});
  report.push(await audit(page, '10-summary', outDir, { delay: 500 }));

  report.push(await audit(page, '11-tools', outDir, { goto: '/discover/tools', wait: '.tool-list', fullPage: true }));
  report.push(await audit(page, '12-stats', outDir, { goto: '/discover/stats', wait: '.stats-grid', fullPage: true }));
  report.push(await audit(page, '13-records', outDir, { goto: '/discover/records', wait: '.sec-title', fullPage: true }));
  report.push(await audit(page, '14-library', outDir, { goto: '/library', wait: '.sec-title', fullPage: true }));
  report.push(await audit(page, '15-program-edit', outDir, { goto: '/program/edit', wait: '.sec-title', fullPage: true }));

  await seedLocale(page, locale, { logs });
  await page.goto(`${BASE}/day/chest/focus`);
  await page.locator('.focus-cta-set').click();
  await page.locator('.focus-timer-island, .tw-widget').first().waitFor({ timeout: 8000 }).catch(() => {});
...
for (const [locale, report, outDir] of [
  ...
  writeFileSync(join(outDir, 'report.json'), JSON.stringify({ locale, summary, details: report }, null, 2));
  ...
  for (const r of report) {
    console.log(`${r.id}: ${r.decorSummary.visibleCount} visible / ${r.decorSummary.total} in DOM`);
  }
