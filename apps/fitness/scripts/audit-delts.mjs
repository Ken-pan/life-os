...
import {
  ...
  DAY_POOL_GROUPS,
} from '../src/lib/data/exercises.js'
...
for (const prog of PROGRAMS) {
  ...
  assert(
    !prog.rotationOrder.includes('delts'),
    `${prog.id}: delts 误入 rotationOrder`,
  )
  assert(
    delts.ex?.length === 4,
    `${prog.id}: 动作数=${delts.ex?.length}，期望 4`,
  )
  assert(delts.warmup?.length >= 2, `${prog.id}: 缺少热身`)
  assert(delts.note?.includes('前束'), `${prog.id}: note 应说明前束覆盖逻辑`)
  assert(
    /南瓜|侧束|12–16|12-16/.test(delts.note || ''),
    `${prog.id}: note 应含南瓜肩/周容量指引`,
  )
  assert(
    /侧束|南瓜/.test(delts.subtitle || ''),
    `${prog.id}: subtitle 应体现侧束/南瓜肩`,
  )
  ...
  for (const ex of delts.ex) {
    ...
    assert(
      equipType(ex) === equipType(EX_BY_ID[ex.id]),
      `${prog.id}/${ex.id}: equip 不一致`,
    )
  ...
  const sideSets = delts.ex
    .filter((e) => SIDE_IDS.has(e.id))
    .reduce((s, e) => s + e.sets, 0)
  const rearSets = delts.ex
    .filter((e) => REAR_IDS.has(e.id))
    .reduce((s, e) => s + e.sets, 0)
  assert(sideSets >= 7, `${prog.id}: 侧束组数(${sideSets})应 ≥7（南瓜肩宽度）`)
  assert(rearSets >= 7, `${prog.id}: 后束组数(${rearSets})应 ≥7（立体/短板）`)
  assert(
    Math.abs(sideSets - rearSets) <= 1,
    `${prog.id}: 侧/后应大致均衡（${sideSets} vs ${rearSets}）`,
  )
}

const bro = getProgramById('bro-split')
assert(
  JSON.stringify(bro.rotationOrder) ===
    JSON.stringify(['chest', 'back', 'legs', 'arms']),
  '四日轮换被破坏',
)
...
assert(
  pool.some((e) => e.id === 'b_face'),
  '可选池应含面拉',
)
assert(
  pool.some((e) => e.id === 'sh_cableraise'),
  '可选池应含绳索侧平举',
)
assert(
  pool.some((e) => e.id === 'b_revfly'),
  '可选池应含反向飞鸟',
)
...
// 科学配对：肩接胸+腿；腹接腿+臂
for (const prog of PROGRAMS) {
  const deltsSch = prog.suppSchedule?.delts
  const coreSch = prog.suppSchedule?.core
  assert(deltsSch?.after?.length === 2, `${prog.id}: delts 应配对 2 个主日`)
  assert(coreSch?.after?.length === 2, `${prog.id}: core 应配对 2 个主日`)
  assert(deltsSch.timesPerWeek === 2, `${prog.id}: delts 周 2 次`)
  assert(coreSch.timesPerWeek === 2, `${prog.id}: core 周 2 次`)
  for (const id of [...deltsSch.after, ...coreSch.after]) {
    assert(
      prog.days[id] && !prog.days[id].supp,
      `${prog.id}: 配对目标 ${id} 须为主日`,
    )
    assert(
      prog.rotationOrder.includes(id),
      `${prog.id}: ${id} 应在 rotationOrder`,
    )
  }
}
assert(
  JSON.stringify(bro.suppSchedule.delts.after) ===
    JSON.stringify(['chest', 'legs']),
  'bro-split 肩应接胸+腿',
)
assert(
  JSON.stringify(bro.suppSchedule.core.after) ===
    JSON.stringify(['legs', 'arms']),
  'bro-split 腹应接腿+臂',
)
...
console.log(
  'delts 动作:',
  bro.days.delts.ex.map((e) => `${e.name} ${e.sets}×${e.reps}`).join(' · '),
)
console.log('可选池:', pool.length, '个动作')
console.log(
  '肩配对:',
  bro.suppSchedule.delts.after.join('+'),
  '· 腹配对:',
  bro.suppSchedule.core.after.join('+'),
)
console.log(failed ? `\n${failed} 项未通过` : '\n肩补充训练审计全部通过')
