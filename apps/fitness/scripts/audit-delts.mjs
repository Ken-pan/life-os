...
for (const prog of PROGRAMS) {
  ...
  for (const id of [...deltsSch.after, ...coreSch.after]) {
    assert(prog.days[id] && !prog.days[id].supp, `${prog.id}: 配对目标 ${id} 须为主日`);
    assert(prog.rotationOrder.includes(id), `${prog.id}: ${id} 应在 rotationOrder`);
  }
}
assert(
  JSON.stringify(bro.suppSchedule.delts.after) === JSON.stringify(['chest', 'legs']),
  'bro-split 肩应接胸+腿'
);
assert(
  JSON.stringify(bro.suppSchedule.core.after) === JSON.stringify(['legs', 'arms']),
  'bro-split 腹应接腿+臂'
);
...
console.log('肩配对:', bro.suppSchedule.delts.after.join('+'), '· 腹配对:', bro.suppSchedule.core.after.join('+'));
console.log(failed ? `\n${failed} 项未通过` : '\n肩补充训练审计全部通过');
