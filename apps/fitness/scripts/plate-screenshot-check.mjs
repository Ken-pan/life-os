...
async function seed(page, overrides = {}) {
  ...
  await page.evaluate((o) => {
    ...
    if (o.programOverrides !== undefined)
      state.programOverrides = o.programOverrides
...
async function assertBarbell(
  page,
  { sides = 2, minPlates = 0, legend = true } = {},
) {
  if (sides === 2) {
    ...
    if (await titleEl.count()) {
      ...
      if (!title?.includes('杠铃视图'))
        throw new Error(`标题应为杠铃视图，实际: ${title}`)
    }
    const count = await page.locator('.pb-barbell .pb-plate').count()
    if (count < minPlates)
      throw new Error(`铃片数不足: ${count} < ${minPlates}`)
    ...
    if (!title?.includes('单端视图'))
      throw new Error(`标题应为单端视图，实际: ${title}`)
  }
}
...
try {
  ...
  await page.waitForFunction(() => {
    ...
    return (
      Math.abs(sheet.getBoundingClientRect().bottom - window.innerHeight) < 3
    )
  })
