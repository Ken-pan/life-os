...
const { dir: SHOTS } = resolveScreenshotDir({
  app: 'fitness',
  suite: 'plates-e2e',
...
async function expectBarbellMirrored(page) {
  const mirrored = await page.evaluate(() => {
    ...
    const hero =
      document.querySelector('.pb-hero-result')?.textContent?.trim() ?? ''
    return { ok: true, hero }
  })
...
test.describe('PlateToolPanel · 发现页独立工具', () => {
  test('快捷步进、preset、双模式、单位切换', async ({ page }) => {
    ...
    await expect(
      page.locator('.pb-bar-chip.on').filter({ hasText: /^20$/ }),
    ).toBeVisible()
  ...
  test('折叠摘要、片库开关、自定义杆重、卡箍', async ({ page }) => {
    ...
    await page
      .locator('.pb-inventory .pb-inv-chip')
      .filter({ hasText: '2.5' })
      .click()
    await expect(
      page.locator('.pb-inv-chip.off').filter({ hasText: '2.5' }),
    ).toBeVisible()
    await expect(
      page.locator('.pb-rack .pb-add').filter({ hasText: '2.5' }),
    ).toHaveCount(0)
...
test.describe('WeightModal · 训练弹窗', () => {
  test('快捷步进、preset、较轻较重推荐', async ({ page }) => {
    ...
    await expect(
      modal.locator('.pb-snap', { hasText: '较轻 135' }),
    ).toBeVisible()
