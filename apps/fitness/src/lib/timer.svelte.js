...
function finishTimer(fromSw = false) {
  ...
  if (S.settings.sound && typeof navigator !== 'undefined' && navigator.vibrate) {
...
export function ringProgressFrac() {
  if (timer.mode !== 'rest' || timer.remain <= 0 || timer.showDone) return progressFrac();
...
function notificationPayload() {
  ...
  return {
    ...
    body: isWork ? `${timer.name} · 时间到` : `${timer.name} · 可以开始下一组了`,
...
export function notificationCapability() {
  ...
  const inApp = /MicroMessenger|QQ\/|WeiBo|AlipayClient|DingTalk|bytedance|Aweme/i.test(ua);
  ...
  return { kind: /** @type {'granted'|'denied'|'default'} */ (Notification.permission) };
}
...
export function initTimer() {
  ...
  const onVisible = () => {
    if (document.visibilityState === 'visible' && timer.visible && !timer.paused) {
      syncRemain();
    }
  };
