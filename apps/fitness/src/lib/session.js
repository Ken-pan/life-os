...
export function exLogHasActivity(entry) {
  if (!entry || typeof entry !== 'object') return typeof entry === 'number' && entry > 0;
...
export function finalizeStaleSessions() {
  ...
  const keys = new Set([...Object.keys(S.logs || {}), ...Object.keys(S.sessionMeta || {})]);
  let finalized = 0;
  let pruned = 0;
  const newHistory = [];
  ...
  if (newHistory.length) {
    const merged = [...(S.rotation.history || []), ...newHistory]
      ...
      .slice(-60);
    const prevLatest = (S.rotation.history || []).map((h) => h.date).sort().pop() ?? '';
    S.rotation.history = merged;

    // 只有补记的训练比已知的最近一次更新时,才推进轮换指针(不覆盖用户手动设置)
    const latestNew = newHistory.sort((a, b) => a.date.localeCompare(b.date)).pop();
...
export function parseTimedTarget(reps) {
  ...
  const timed = text.match(/(\d+)\s*(min(?:ute)?s?|secs?|seconds?|分钟|秒|m|s)(?![a-z0-9])/);
  if (!timed) return null;

  const value = Number(timed[1]);
  const multiplier = /^(m|min|mins|minute|minutes|分钟)$/.test(timed[2]) ? 60 : 1;
...
/**
 * 历史记录的真实训练时长（分钟）。
 * 优先用 sessionMeta 的起止时间；未正常结束的旧 session 用最后一组的时间戳收口，
 * 避免把"没点完成"的训练算成一直进行到现在。无法确定时返回 null。
 */
export function recordedSessionMinutes(dayId, dateK) {
  ...
  const startIso = meta?.startedAt ?? timestamps[0] ?? null;
  if (!startIso) return null;
  const endIso = meta?.endedAt ?? timestamps[timestamps.length - 1] ?? null;
