  ...
  for (const [key, val] of Object.entries(overrides || {})) {
    ...
    if (key.startsWith('day:') && val && typeof val === 'object') {
      ...
      if (Array.isArray(newVal.addedEx)) {
        newVal.addedEx = [...new Set(newVal.addedEx.map((id) => resolveExerciseId(id)))];
      }
  ...
  if (mode === 'merge') {
    ...
    if (next.rotation?.history?.length) {
      ...
      const merged = [...(S.rotation.history || []), ...next.rotation.history]
        ...
        .slice(-60);
      S.rotation = { ...S.rotation, history: merged, next: next.rotation.next ?? S.rotation.next };
    }
...
export function exUnit(ex) {
  const perSide = (ex.unit && /侧/.test(ex.unit)) || equipType(ex) === 'dumbbell';
  if (perSide) return S.settings.unit === 'kg' ? t('units.kgPerSide') : t('units.lbsPerSide');
...
export function setPlateEnabled(unit, plate, enabled) {
  if (!S.settings.plateInventory) S.settings.plateInventory = { lbs: {}, kg: {} };
  if (!S.settings.plateInventory[unit]) {
    S.settings.plateInventory[unit] = Object.fromEntries(allPlatesFor(unit).map((p) => [p, true]));
  }
...
export function plateCollarFor(unit) {
  return unit === 'kg' ? (S.settings.plateCollarKg ?? 0) : (S.settings.plateCollarLbs ?? 0);
}
...
export function completeDay(dayId) {
  ...
  if (S.rotation.history.length > 60) S.rotation.history = S.rotation.history.slice(-60);
...
export function sessionStats() {
  ...
  const week7 = arr.filter((d) => daysBetween(d, today) >= 0 && daysBetween(d, today) < 7).length;
  const last = arr.length ? arr[arr.length - 1] : null;
  return { total: arr.length, week7, last, daysSince: last ? daysBetween(last, today) : null };
}
