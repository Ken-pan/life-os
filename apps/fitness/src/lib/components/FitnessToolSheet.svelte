<script>
  ...
  const rm1 = $derived(estimate1RM(fitnessToolSheet.rmWeight, fitnessToolSheet.rmReps));
  ...
  $effect(() => {
    if (fitnessToolSheet.open && fitnessToolSheet.tab === 'plates' && !fitnessToolSheet.fromFocus) {
      tick().then(() => {
        sheetEl?.querySelector('.pb-viz-container')?.scrollIntoView({ block: 'center', behavior: 'smooth' });
      });
    }
  });
</script>
...
{#if fitnessToolSheet.open}
  <div class="sheet-bg" role="presentation" onclick={onBackdrop}>
    ...
    >
      ...
      {#if !isFocusPlates}
        <div class="tool-tabs seg seg-track life-os-scroll-x life-os-scroll-x--snap life-os-scroll-x--fade-edge" role="tablist" aria-label={t('tools.tabListAria')}>
          ...
          <div class="tool-pane" role="tabpanel">
            <div class="tool-fields">
              <label class="tool-field">
                ...
                <input type="number" min="1" step="2.5" bind:value={fitnessToolSheet.rmWeight} />
              </label>
              <label class="tool-field">
                <span>{t('tools.reps')}</span>
                <input type="number" min="1" max="15" bind:value={fitnessToolSheet.rmReps} />
              </label>
            </div>
            {#if rm1}
        ...
        {:else if fitnessToolSheet.tab === 'plates'}
          <div class="tool-pane" role="tabpanel">
            <PlateToolPanel
              ...
              showUnitToggle={fitnessToolSheet.plateSides === 2 && !fitnessToolSheet.fromFocus}
          ...
          <div class="tool-pane" role="tabpanel">
            <div class="tool-fields">
              <label class="tool-field">
                ...
                <input type="number" min="1" bind:value={fitnessToolSheet.volSets} />
              </label>
              <label class="tool-field">
                <span>{t('tools.repsPerSet')}</span>
                <input type="number" min="1" bind:value={fitnessToolSheet.volReps} />
              </label>
              <label class="tool-field tool-field-wide">
                <span>{t('tools.weightOptional')}</span>
                <input type="number" min="0" step="2.5" bind:value={fitnessToolSheet.volWeight} />
              </label>
            </div>
            {#if volTotal}
              <div class="tool-result">
                <div>
                  {t('tools.totalReps', { reps: volTotal.reps })}{#if volTotal.volume}{t('tools.totalVol', { vol: volTotal.volume })}{/if}
                </div>
              </div>
            {/if}
            <div class="tool-seg">
