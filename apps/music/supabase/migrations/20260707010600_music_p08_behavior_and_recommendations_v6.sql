-- P0.8: recommendation attribution + play_events metadata + RPC v6 Western calibration

alter table music.play_events
...
create table if not exists music.recommendation_events (
  ...
  event_type text not null check (event_type in (
    'queue_add', 'play', 'skip', 'complete', 'like', 'click'
  )),
  ...
  foreign key (user_id, recommended_track_id)
...
create index if not exists recommendation_events_user_created_idx
  on music.recommendation_events (user_id, created_at desc);

alter table music.recommendation_events enable row level security;

drop policy if exists "recommendation_events_select_own" on music.recommendation_events;
create policy "recommendation_events_select_own"
  on music.recommendation_events for select
  using ((select auth.uid()) = user_id);

drop policy if exists "recommendation_events_insert_own" on music.recommendation_events;
create policy "recommendation_events_insert_own"
  on music.recommendation_events for insert
  with check ((select auth.uid()) = user_id);

-- v6: seed-artist affinity + stronger Western / dark / quirky routing
