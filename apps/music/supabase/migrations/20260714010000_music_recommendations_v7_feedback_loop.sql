-- P0.9: recommendation online-learning feedback loop — RPC v7
-- Reads music.recommendation_events (previously write-only) back into scoring so
-- the recommender learns which tracks actually land WHEN RECOMMENDED. This is
-- distinct from the behavior CTE (general play_events): a track you love on-demand
-- but skip whenever it is auto-suggested gets suppressed as a *recommendation*
-- without hurting its standing elsewhere.

create index if not exists recommendation_events_user_track_idx
  on music.recommendation_events (user_id, recommended_track_id, created_at desc);
...
begin
  ...
  -- v7: online-learning feedback — how each track performed WHEN RECOMMENDED.
  rec_feedback as (
    select coalesce(e.canonical_track_id, re.recommended_track_id) as canon_id,
      count(*) filter (where re.event_type = 'complete') as rec_completes,
      count(*) filter (where re.event_type = 'like') as rec_likes,
      count(*) filter (where re.event_type = 'play') as rec_plays,
      count(*) filter (where re.event_type = 'skip') as rec_skips
    from music.recommendation_events re
    left join music.track_enrichment e
      on e.user_id = re.user_id and e.track_id = re.recommended_track_id
    where re.user_id = v_user
      and re.created_at > now() - interval '45 days'
      and re.event_type in ('play', 'complete', 'like', 'skip')
    group by coalesce(e.canonical_track_id, re.recommended_track_id)
  ),
  candidates as (
  ...
  scored as (
    select c.track_id, c.title, c.artist, c.album, c.canon_id, c.norm_key,
      (
        ...
        -- v7 feedback loop: reward tracks that landed when recommended, suppress ones repeatedly skipped as picks.
        + least(coalesce(rf.rec_completes, 0) * 0.05
            + coalesce(rf.rec_likes, 0) * 0.07
            + coalesce(rf.rec_plays, 0) * 0.012, 0.15)
        - least(coalesce(rf.rec_skips, 0) * 0.05, 0.20)
      ...
      (coalesce(rf.rec_completes, 0) + coalesce(rf.rec_likes, 0)) > 0 as rec_endorsed
    from candidates c
    ...
    left join rec_feedback rf on rf.canon_id = c.canon_id
  ...
  select r.track_id, r.title, r.artist, r.album, round(r.score, 4) as score, r.matched_tags,
    array_remove(array[
      ...
      case when r.rec_endorsed then 'liked as a pick' end
    ], null) as reasons
