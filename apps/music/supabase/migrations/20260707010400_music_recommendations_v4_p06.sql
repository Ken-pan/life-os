-- P0.6: canonical dedupe + Western genre gate + context/vibe reweight
...
as $$
  select lower(trim(regexp_replace(
    regexp_replace(
      regexp_replace(
        coalesce(p_title, ''),
        '\s*\([^)]*(explicit|clean|radio edit|radio-edit|instrumental|live|remix|version|edit|sped|slowed|dirty)[^)]*\)',
        '',
        'gi'
      ),
      '[?!.,''""`]+',
...
set search_path = music, public
as $$
  select music.canonical_title(p_title) || '::' || music.canonical_artist(p_artist);
$$;
...
set search_path = music, public
as $$
  select music.canonical_track_key(p_title, p_artist);
$$;
  ...
  idf_mult as (
    select
      ...
      case
        ...
        when tc.tag_slug in ('confident', 'party', 'gym') then 0.20::numeric
        else 1::numeric
      end as mult
      ...
      coalesce(sum(
        case when td.namespace = 'vibe' and st.tag_slug is not null then
          ...
          case tt.tag_slug
            ...
            when 'night-drive' then 0.34 when 'dark' then 0.34 when 'sexy' then 0.34
            when 'dramatic' then 0.32 when 'euphoric' then 0.28
            when 'confident' then 0.12 else 0.30
          end
        else 0 end
      ), 0) as vibe_score,
      coalesce(sum(
        case when td.namespace = 'genre' and st.tag_slug is not null then
          coalesce(im.mult, 0) * tt.confidence *
          case tt.tag_slug
            when 'pop' then 0.02 when 'dance-pop' then 0.20 when 'alt-pop' then 0.18
            when 'k-pop-solo' then 0.28 when 'k-pop' then 0.22 when 'girl-group' then 0.22
            when 'electropop' then 0.18 when 'dark-pop' then 0.18
            when 'r-and-b' then 0.16 when 'alt-r-and-b' then 0.16
            when 'edm' then 0.10 when 'hip-hop' then 0.12 when 'rap' then 0.12
            when 'mandopop' then 0.14 when 'c-pop' then 0.12 else 0.14
          end
        else 0 end
      ), 0) as genre_score,
      coalesce(sum(
        case when td.namespace = 'context' and sc.tag_slug is not null and st.tag_slug is not null then
          coalesce(im.mult, 0) * tt.confidence *
          case tt.tag_slug
            when 'party' then 0.06 when 'gym' then 0.05
            when 'background' then 0.12 when 'focus' then 0.10
            when 'homepage-safe' then 0.10 when 'shower' then 0.08
            else 0.08
          end
        else 0 end
      ), 0) as context_score,
  ...
  scored as (
    select
      ...
      (
        ...
        + coalesce(tc.genre_score, 0) * 0.28
        + coalesce(tc.context_score, 0) * 0.08
        + case
...
grant execute on function music.canonical_title(text) to authenticated, service_role;
grant execute on function music.canonical_artist(text) to authenticated, service_role;
grant execute on function music.canonical_track_key(text, text) to authenticated, service_role;
