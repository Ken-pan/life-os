export default {
  app: {
    name: 'Korben Music',
    tagline: '本地音乐，留在心里',
  },
  common: {
    back: '返回',
    brand: 'Korben Music',
    play: '播放',
    playAll: '播放全部',
    playAlbum: '播放专辑',
    shuffle: '随机播放',
    pause: '暂停',
    loading: '加载中',
    import: '导入音乐',
    search: '搜索',
    more: '更多',
    empty: '还没有音乐',
    emptyHint: '选择本机音频文件导入，或登录后从云端同步资料库。',
    songs: '{count} 首',
    save: '保存',
    cancel: '取消',
    close: '关闭',
    skipToContent: '跳到主要内容',
    delete: '删除',
    sort: '排序',
    filter: '筛选',
    density: '密度',
    compact: '紧凑',
    comfortable: '舒适',
    selectAll: '全选',
    selected: '已选 {count} 项',
    addToQueue: '加到队列末尾',
    playNext: '下一首播',
    playNow: '立即播放',
  },
  nav: {
    mainAria: '主导航',
    home: '首页',
    library: '资料库',
    browse: '浏览',
    playlists: '歌单',
    settings: '设置',
    search: '搜索',
    liked: '我喜欢的',
    import: '导入音乐',
    groupMain: '主导航',
    groupMore: '更多',
    groupQuick: '快捷',
    groupAccount: '账号',
    quickSwitch: '快速切换',
  },
  home: {
    title: '此刻',
    kicker: '留在心里',
    heroTitle: '把喜欢的声音\n收进这里',
    heroDesc: '本地与云端资料库，离线也能听。',
    greeting: '晚上好',
    greetingHint: '从最近喜欢的音乐开始',
    nowPlaying: '正在播放',
    continueImmersion: '继续沉浸',
    openLyrics: '打开歌词',
    shuffleLiked: '随机播放收藏',
    recent: '最近播放',
    continueListening: '继续听',
    recentAdded: '最近添加',
    topArtists: '常听艺术家',
    continuePlaying: '继续播放',
    continueHint: '从上次停下的地方继续',
    quick: '快捷入口',
    speedDial: '私人速拨',
    speedDialKicker: '个人',
    speedDialPersonal: 'PERSONAL',
    speedDialAdd: '添加到速拨',
    speedDialAddShort: '新建速拨',
    speedDialSurpriseMe: '惊喜一下',
    speedDialSurpriseHint: '智能随机推荐',
    speedDialSurpriseLoading: '正在挑选…',
    speedDialSurpriseEmpty: '暂无可推荐曲目',
    speedDialSurpriseFailed: '推荐失败，请稍后重试',
    speedDialPlayAll: '播放全部',
    speedDialPin: '固定到速拨',
    speedDialUnpin: '取消固定',
    speedDialHide: '不再显示',
    speedDialRemove: '移除',
    speedDialEditHint:
      '拖动排序会固定布局。右键首页卡片可快速固定，右下角可一键惊喜播放。',
    speedDialReasonPinned: '已固定',
    speedDialReasonFrequent: '常点',
    speedDialReasonRecent: '最近常听',
    speedDialReasonImport: '最近导入',
    speedDialReasonTime: '此时段常听',
    speedDialReasonExplore: '试试看',
    quickPicks: '快速精选',
    quickPicksPlayAll: '播放全部',
    speedDialPrev: '上一页速拨',
    speedDialNext: '下一页速拨',
    speedDialPages: '速拨分页',
    speedDialRandom: '随机推荐',
    liked: '我喜欢的',
    import: '导入音乐',
    shuffleAll: '随机播放全部',
    dropHint: '拖拽音频文件到这里',
    emptyAction: '导入音频文件',
    emptyLogin: '登录同步云端资料库',
    filterAll: '全部',
    filterRecent: '最近播放',
    filterLiked: '收藏',
    filterChinese: '中文',
    filterLateNight: '深夜模式',
    filterOffline: '离线',
  },
  library: {
    title: '资料库',
    filterPlaceholder: '筛选曲目…',
    colTitle: '歌曲',
    colArtist: '艺人',
    colAlbum: '专辑',
    colAdded: '添加时间',
    colDuration: '时长',
    colLike: '喜欢',
    sortAdded: '添加时间',
    sortTitle: '标题',
    sortArtist: '艺人',
    sortAlbum: '专辑',
    sortDuration: '时长',
  },
  browse: {
    title: '浏览',
    albums: '专辑',
    artists: '艺术家',
    tracks: '歌曲',
    playlists: '歌单',
    sortRecent: '最近添加',
    sortAlpha: '字母',
    sortPlayed: '最近播放',
  },
  playlists: {
    title: '歌单',
    create: '新建歌单',
    namePrompt: '歌单名称',
    created: '已创建歌单',
    emptyUserHint: '点右上角「新建歌单」，把喜欢的曲目收进来。',
  },
  playlist: { title: '歌单' },
  search: {
    title: '搜索',
    placeholder: '歌曲、艺术家、专辑…',
    recent: '最近搜索',
    suggestions: '建议',
    scopeAll: '全部',
    scopeTracks: '歌曲',
    scopeAlbums: '专辑',
    scopeArtists: '艺人',
    scopePlaylists: '歌单',
    noResults: '没有找到结果',
    noResultsFor: '没有找到 “{query}”',
    noResultsInScope: '「{scope}」下没有找到结果',
    noResultsInScopeFor: '「{scope}」下没有找到 “{query}”',
    closeSuggestions: '关闭搜索建议',
    emptyHint: '输入关键词，或从最近搜索、热门艺人开始',
    minCharsHint: '再输入 {count} 个字符',
    clearRecent: '清除',
    clearQuery: '清除搜索',
    viewAll: '查看全部结果',
    viewAllForQuery: '查看全部「{query}」结果',
    viewAllInScope: '查看全部 {count} 条{scope}',
    loading: '搜索中…',
    resultSummary: '「{query}」· 共 {count} 条结果',
    resultSummaryScoped: '「{query}」· {scope} {count} 条',
    resultSummaryAllHint: '（全部 {total} 条）',
    resultSummaryPending: '「{query}」· 搜索中…',
    sortRelevance: '相关度',
    sortTitle: '标题',
    sortRecent: '最近添加',
    sortLabel: '排序',
    tryInstead: '试试这些',
    broadenScope: '查看全部分类',
    popularArtists: '热门艺人',
    playlistTrackCount: '{count} 首',
    shortcut: '⌘K 搜索',
  },
  liked: {
    title: '我喜欢的',
    empty: '还没有红心歌曲',
    emptyHint: '在曲目旁点红心，它们会出现在这里。',
    ariaLabel: '喜欢歌曲',
    add: '喜欢',
    remove: '取消喜欢',
  },
  import: {
    title: '导入',
    drop: '拖放音频或 LRC 文件到此处',
    or: '或从文件 App 多选音频',
    pickFiles: '从「文件」多选音频或歌词',
    hint: '支持 MP3、M4A、FLAC、WAV 等音频，以及同名 .lrc 歌词。在 iPhone 上点下方按钮从「文件」多选；文件保存在本机 IndexedDB。',
    hintCloud:
      '支持 MP3、M4A、FLAC、WAV 等音频及 .lrc 歌词。可从「文件」多选导入；登录后会自动上传到私有云存储，并写入推荐标签。',
    importing: '正在导入 {done}/{total}…',
    phaseArt: '正在补全封面…',
    phaseMetadata: '正在校正元数据…',
    phaseUpload: '正在上传云端 {done}/{total}…',
    phaseTags: '正在打标 {done}/{total}…',
    phaseEnrich: '正在深度标注…',
    phaseSync: '正在同步元数据…',
    phaseLyrics: '正在补抓歌词 {done}/{total}…',
    done: '已导入 {count} 首',
    doneLocal: '已导入 {count} 首（登录后可自动上传与打标）',
    doneCloud: '已导入 {count} 首，上传 {uploaded} 首，打标 {tagged} 首',
    doneCloudPartial:
      '已导入 {count} 首，上传成功 {uploaded} 首（失败 {failed}），打标 {tagged} 首',
    doneMixed: '已导入 {audio} 首音频，匹配 {lrc} 首歌词',
    tagFailed: '{count} 首打标失败',
    syncFailed: '云同步失败，请在设置中重试',
    enrichFailed: '深度标注未完成，可在设置查看标签审核',
  },
  utilityPane: {
    resize: '拖拽调整面板宽度',
  },
  nowPlaying: {
    title: '正在播放',
    volume: '音量',
    mute: '静音',
    unmute: '取消静音',
    queue: '播放队列',
    queueEmpty: '队列为空',
    playFromCurrent: '从当前曲目播放',
    reorder: '拖拽排序',
    moveUp: '上移',
    moveDown: '下移',
    remove: '从队列移除',
    clearQueue: '清空队列',
    continueSimilar: '相似续播',
    continueSimilarLoading: '正在推荐…',
    continueSimilarAdded: '已加入 {count} 首相似歌曲',
    continueSimilarEmpty: '暂无推荐，请先登录并同步资料库',
    continueSimilarFailed: '推荐失败，请稍后重试',
    continueSimilarHint: '基于当前曲目推荐相似歌曲',
    recDebugPreviewTitle: '推荐调试（开发）',
    recReasonsTitle: '推荐依据',
    lyrics: '歌词',
    synced: '同步',
    noLyrics: '暂无歌词',
    fetchingLyrics: '正在从网络获取歌词…',
    plain: '文本',
    playing: '正在播放',
    paused: '已暂停',
    modeLabel: '播放视图',
    modeCover: '封面',
    modeLyrics: '歌词',
    modeQueue: '播放清单',
    queueNowPlaying: '正在播放',
    queueHistory: '已播放',
    queueUpNext: '接下来',
    queueUpNextEmpty: '暂无后续曲目',
    queueShuffleHint: '随机播放已固定顺序，列表与即将播放一致',
  },
  player: {
    playFailed: '无法播放，请确认已登录并完成云同步',
    noSource: '找不到音频：云端曲目需先登录并同步资料库',
    lyricsOnlyHint: '仅有歌词 · 导入或同步音频后可播放',
    importAudio: '去导入',
    offlineUncached: '离线且未缓存此曲，请联网后重试',
    seek: '播放进度',
    seekUnavailable: '暂无时长信息',
  },
  album: { title: '专辑' },
  artist: { title: '艺术家' },
  settings: {
    title: '设置',
    language: '语言',
    languageLabel: '界面语言',
    languageDesc: '跨设备同步；登录后会自动备份到云端。',
    languageZh: '中文',
    languageEn: 'English',
    theme: '主题',
    themeAuto: '跟随系统',
    themeLight: '浅色',
    themeDark: '深色',
    appearance: '外观',
    albumAmbience: '氛围色跟随专辑',
    albumAmbienceDesc:
      '从封面取色，用于进度条、光晕与播放卡片；导航与按钮仍使用品牌色。',
    albumAmbienceOn: '跟随',
    albumAmbienceOff: '关闭',
    playback: '播放',
    gapless: '无缝衔接',
    gaplessDesc:
      '预加载下一首并在切换时交换音频通道；Supabase 云音频会写入 Service Worker 缓存（最多 4 首）。',
    gaplessOn: '开启',
    gaplessOff: '关闭',
    crossfade: '交叉淡入淡出',
    crossfadeDesc:
      '切歌时旧曲渐弱、新曲渐强（Web Audio 等功率曲线）；设为 0 则仅无缝硬切。云端曲目会同步预缓存下一首。',
    crossfadeOff: '关闭',
    crossfadeSeconds: '{seconds} 秒',
    autoContinueSimilar: '相似续播',
    autoContinueSimilarDesc:
      '队列播完时，按当前曲目的 vibe 标签自动追加相似歌曲（需登录）。',
    autoContinueSimilarOn: '开启',
    autoContinueSimilarOff: '关闭',
    immersiveViewMode: '正在播放默认视图',
    immersiveViewModeDesc: '进入正在播放页时的默认视图；页内可随时切换。',
    privacy: '隐私',
    privacyDesc:
      '默认保存在本机。登录后可选择将音频备份到私有 Supabase Storage（private bucket，播放时用 signed URL）。',
    iosBackground:
      'iPhone 后台播放：请用 Safari「添加到主屏幕」安装本应用，锁屏后可通过控制中心/耳机控制。',
    library: '曲库',
    libraryCount: '{count} 首歌曲保存在本机',
    missingLyrics: '{count} 首曲目尚未获取歌词',
    fetchLyrics: '远程补抓歌词',
    fetchLyricsDesc:
      '从 lrclib / QQ音乐 / 网易云 等源批量获取缺失歌词并同步云端',
    fetchLyricsProgress: '正在补抓歌词 {done}/{total}…',
    fetchLyricsDone: '已处理 {total} 首，成功获取 {repaired} 首（已同步云端）',
    fetchLyricsEmpty: '没有需要补抓歌词的曲目',
    lyricsAutoRunning: '正在自动补全歌词',
    lyricsAutoPending: '还有 {count} 首待补全 · 已在后台自动处理',
    lyricsAutoDone: '歌词已全部补全',
    maintArt: '正在补全封面',
    maintMetadata: '正在整理元数据',
    rescanMeta: '补扫 ID3 元数据',
    rescanMetaDesc: '从本地音频文件重新读取标签、封面与内嵌歌词',
    rescanning: '正在补扫 {done}/{total}…',
    rescanDone: '已扫描 {scanned} 首，更新 {updated} 首',
    rescanEmpty: '没有可扫描的本地音频',
    export: '导出库信息',
    tagReview: '标签审核',
    tagReviewDesc: '低置信度或 LLM 建议的标签，确认后用于推荐排序。',
    tagReviewEmpty: '暂无待审核标签',
    tagReviewApprove: '采纳',
    tagReviewReject: '忽略',
    tagReviewReasonLow: '启发式打标置信度低',
    tagReviewReasonLlm: 'LLM 建议需确认',
    tagReviewTags: '建议标签：{tags}',
    recommendationHealth: '推荐健康度',
    recommendationHealthDesc:
      '播放行为、向量 embedding 与待审核项越多，推荐越准。',
    healthPlayEvents: '播放事件 {count} 条',
    healthEmbeddings: '向量 embedding {count} 首',
    healthPendingReviews: '待审核 {count} 项',
    playLoadDebug: '播放加载指标',
    playLoadDebugDesc:
      '本机记录最近约 50 次播放的点击→可播耗时（不上报）。用于判断是否还需优化。',
    playLoadSamples: '样本 {count} 次',
    playLoadP50: 'P50 可播 {ms} ms',
    playLoadP95: 'P95 可播 {ms} ms',
    playLoadSources: '来源：{summary}',
    playLoadFails: '近期失败 {count} 次',
    playLoadClear: '清空指标',
    offlineCache: '离线音频缓存',
    offlineCacheDesc:
      '最近播放的云曲目会渐进写入本机 IndexedDB，便于重复播放与弱网。',
    offlineCacheStats: '已缓存 {count} 首 · 约 {size}',
    account: '账号',
    signIn: '登录',
    signOut: '退出登录',
    about: '关于',
    version: 'Korben Music 0.1',
  },
  auth: {
    title: '账号',
    signIn: '登录',
    signUp: '注册',
    signedIn: '已登录',
    signedInDesc:
      '可与 Life OS 账号同步元数据；可在设置中将本地音频备份到私有云存储。',
    goSettings: '前往设置',
    email: '邮箱',
    password: '密码',
    passwordHint: '至少 6 位',
    signInSuccess: '登录成功',
    signUpSuccess: '注册成功',
    syncFailed: '同步失败，请稍后在设置中重试',
    pleaseWait: '请稍候…',
    noAccount: '还没有账号？',
    hasAccount: '已有账号？',
    createAccount: '创建账号',
    confirmSent: '请查收邮件',
    checkInbox: '验证链接已发送至 {email}',
    confirmHint: '确认后即可登录并同步歌单元数据。',
    backSignIn: '返回登录',
    footnote:
      '与 Planner / Fitness / Finance / Home 等共用同一 Life OS 账号；本机登录一次后，其它站点可自动接上。',
    errInvalidCredentials: '邮箱或密码不正确',
    errEmailNotConfirmed: '请先验证邮箱',
    errAlreadyRegistered: '该邮箱已注册',
    errPasswordShort: '密码至少 6 位',
    errInvalidEmail: '邮箱格式不正确',
    errRateLimit: '请求过于频繁，请稍后再试',
    errNetwork: '网络异常，请检查连接',
    errGeneric: '操作失败',
  },
  sync: {
    title: '云同步',
    desc: '同步设置、歌单与曲目元数据（含歌词与封面链接）。音频需单独在下方「云备份」上传。',
    descCloud:
      '已启用云端播放：同步会合并歌单、歌词、封面链接与播放统计；新设备登录后可直接播放已上传曲目。',
    now: '立即同步',
    ok: '同步完成',
    failed: '同步失败',
    defaultError: '同步失败',
    banner: '云同步失败：{reason}',
    schemaCache: '云端 API 暂时不可用，请稍后在设置中重试同步',
    offline: '当前离线，已暂存同步任务',
    offlineQueued: '离线中：本地播放不受影响，联网后会自动同步',
    pending: '网络已恢复，正在准备同步…',
    notSignedIn: '请先登录',
    signInFirst: '登录 Life OS 账号',
  },
  cloudAudio: {
    title: '云备份',
    desc: '将本地音频上传到 Supabase 私有 bucket。超过 6MB 自动使用断点续传。播放时通过临时 signed URL 访问，不会公开外链。',
    stats:
      '本机音频 {localAudio} 首 · 待上传 {pending}（{size}）· 已在云端 {cloud} 首',
    upload: '上传待备份音频',
    uploading: '正在上传 {done}/{total}…',
    current: '当前：{title}',
    done: '已上传 {uploaded} 首（{size}）',
    doneWithFail: '已上传 {uploaded} 首，失败 {failed} 首',
    empty: '没有待上传的本地音频',
    noPath: '缺少云存储路径',
    signedFailed: '无法获取播放链接',
  },
}
