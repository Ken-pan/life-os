...
/** Credits / metadata rows at the top of a lyrics sheet. */
/** @param {string} text */
export function isLyricsMetaLine(text) {
  const s = (text || '').trim()
  if (!s) return false
  if (/^(词|曲|编曲|制作人)\b/.test(s)) return true
  if (/^(Lyrics|Composed|Producer)\b/i.test(s)) return true
  if (/\b(Lyrics|Composed)\s+by\s*:/i.test(s)) return true
  return false
}
