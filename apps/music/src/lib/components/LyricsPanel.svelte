<script>
  ...
  const visualActive = $derived(
    model.timed && model.lines.length ? Math.max(active, 0) : -1,
  )
  const canSeek = $derived(
    seekable && model.timed && typeof onSeek === 'function',
  )
...
{#if fetching}
  <section
    class="now-playing-lyrics now-playing-lyrics--loading"
  ...
  >
    ...
    <div class="now-playing-lyrics-skeleton" aria-hidden="true">
      {#each Array(5) as _, i (i)}
        <span
          class="now-playing-lyrics-skeleton-line"
          style:width="{55 + (i % 3) * 12}%"
        ></span>
      {/each}
    </div>
    <p class="now-playing-lyrics-loading-copy">
      {t('nowPlaying.fetchingLyrics')}
    </p>
  </section>
{:else if model.lines.length}
  <section
    class="now-playing-lyrics"
    class:now-playing-lyrics--stage={stage}
    class:now-playing-lyrics--sing-along={singAlong}
    aria-label={t('nowPlaying.lyrics')}
  >
    {#if !stage}
      <div class="now-playing-lyrics-head">
        ...
        {:else}
          <span class="now-playing-lyrics-tag now-playing-lyrics-tag--plain"
            >{t('nowPlaying.plain')}</span
          >
        {/if}
      </div>
    {/if}
    <div
      class="now-playing-lyrics-body"
      class:timed={model.timed}
    ...
    >
      {#each model.lines as line, i (i)}
        ...
        {@const isNear =
          model.timed &&
          visualActive >= 0 &&
          Math.abs(i - visualActive) <= 2 &&
          !isActive}
        {@const isFar =
          model.timed && visualActive >= 0 && !isActive && !isNear}
        {#if canSeek}
          <button
            type="button"
            ...
            class:meta={isLyricsMetaLine(line.text)}
            data-lyric-idx={i}
