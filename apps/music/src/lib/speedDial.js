...
/** @typedef {'tile' | 'add'} SpeedDialVariant */

/** @typedef {'track' | 'artist' | 'album' | 'playlist' | 'collection' | 'add'} SpeedDialEntityType */
...
async function resolveSlot(slot) {
  switch (slot.entityType) {
    ...
    case 'artist': {
      ...
      return cellFromArtist({ artist: tracks[0].artist, artistKey: slot.entityId }, tracks, slot.reason, slot.pinned);
    }
    case 'album': {
      const tracks = await db.tracks.where('albumKey').equals(slot.entityId).toArray();
      if (!tracks.length) return null;
      const hydrated = tracks.map(hydrateTrack);
      return cellFromAlbum(
        { album: hydrated[0].album, albumKey: slot.entityId, artist: hydrated[0].artist, artUrl: hydrated[0].artUrl },
        hydrated,
        slot.reason
      );
    }
...
function reasonForStats(key, stats) {
  ...
  if (s.timeMatches >= 2 && s.timeMatches >= s.activeLaunches * 0.4) return 'time_context';
  if (s.activeLaunches >= 2) return 'frequent_launch';
  return 'recent_return';
}

/** @param {Set<string>} hidden @param {Set<string>} used @param {string | null} [fatigueTrackId] */
async function buildAutoCandidates(hidden, used, fatigueTrackId = null) {
  ...
  for (const [key, score] of [...launchScores.entries()].sort((a, b) => b[1] - a[1])) {
    if (hidden.has(key) || used.has(key)) continue;
    ...
    if (!cell) continue;
    let adjusted = scoreFromStats(key, playbackStats) || score;
    if (fatigueTrackId && cell.entityType === 'track' && cell.entityId === fatigueTrackId) {
      adjusted *= 0.55;
    }
    if (reasonForStats(key, playbackStats) === 'time_context' && timeBucket === 'late_night') {
      adjusted += 0.2;
    }
    candidates.push({ score: adjusted, cell, reason: reasonForStats(key, playbackStats) });
  }
  ...
  for (const track of recent) {
    ...
    if (hidden.has(key) || used.has(key)) continue;
    const base = scoreFromStats(key, playbackStats);
    candidates.push({
      score: Math.max(0.75, base) + (launchScores.get(key) || 0) * 0.2,
      cell: cellFromTrack(track, 'recent_return'),
  ...
  for (const artist of artists) {
    ...
    candidates.push({
      score: 0.55 + (launchScores.get(key) || 0),
      cell: cellFromArtist(artist, tracks, 'frequent_launch'),
      reason: 'frequent_launch'
    });
  }

  const playlists = (await getPlaylists()).filter((p) => p.kind === 'user').slice(0, 4);
  for (const playlist of playlists) {
    ...
    if (!tracks.length) continue;
    candidates.push({
      score: 0.45 + (launchScores.get(key) || 0),
...
function pagesFromBoard(board, overflow = []) {
  const primary = [...board.filter(Boolean).slice(0, 8), addCell()];
  /** @type {SpeedDialPage[]} */
  ...
  if (overflow.length) {
    for (let i = 0; i < overflow.length; i += 8) {
      pages.push({
        id: `more-${i / 8 + 1}`,
        label: '更多',
        cells: [...overflow.slice(i, i + 8), addCell()]
      });
    }
  }
...
export async function getSpeedDialPages(fatigueTrackId = null) {
  ...
  const [slots, hidden] = await Promise.all([getSpeedDialSlots(), getHiddenEntityKeys()]);
  ...
  for (const slot of slots.filter((s) => s.pinned && s.position >= 0 && s.position < 8)) {
    const cell = await resolveSlot(slot);
    if (!cell || hidden.has(cell.id)) continue;
    board[slot.position] = cell;
    used.add(cell.id);
  }

  for (const slot of slots.filter((s) => !s.pinned && !s.hidden && s.position >= 0 && s.position < 8)) {
    if (board[slot.position]) continue;
    const cell = await resolveSlot(slot);
    if (!cell || hidden.has(cell.id) || used.has(cell.id)) continue;
    board[slot.position] = cell;
    used.add(cell.id);
  }

  const candidates = await buildAutoCandidates(hidden, used, fatigueTrackId);
  ...
  for (let i = 0; i < 8; i++) {
    ...
    const next = candidates.find((c) => !used.has(c.cell.id));
    if (!next) continue;
    board[i] = { ...next.cell, reason: next.reason };
    used.delete(existing.id);
    used.add(next.cell.id);
    replacements += 1;
    await persistAutoSpeedDialSlot(i, next.cell.entityType, next.cell.entityId, next.reason);
  }
...
export async function getSpeedDialEditCells() {
  ...
  return pages[0]?.cells.filter((cell) => cell.variant !== 'add') ?? [];
}
