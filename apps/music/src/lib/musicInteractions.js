...
export async function recordMusicInteraction(input) {
  ...
  if (count > INTERACTION_CAP) {
    ...
    const stale = await db.interactions
      .orderBy('createdAt')
      .limit(excess)
      .primaryKeys()
  ...
  if (input.entityType === 'track' || input.trackId) {
    ...
    if (eventType) {
      ...
      const trackId =
        input.trackId ?? (input.entityType === 'track' ? input.entityId : null)
      if (trackId) {
        void recordPlayEvent({
          ...
          playedRatio:
            durationMs > 0 && playedMs > 0 ? playedMs / durationMs : undefined,
...
/**
 * Next-play probability score — weights launch success, completion rate, and skips.
 * @param {string} key
 * @param {Map<string, EntityPlaybackStats>} stats
 * @param {Map<string, number>} launchScores
 */
export function scoreNextPlay(key, stats, launchScores) {
  const s = stats.get(key)
  const launch = launchScores.get(key) ?? 0
  if (!s) return launch

  const plays = s.activeLaunches + s.passivePlays
  const completionRate = plays > 0 ? s.completes / plays : 0
