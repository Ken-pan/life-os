<script>
  ...
  import { librarySignals, S, save } from '$lib/state.svelte.js';
  ...
  function onSort(key) {
    if (sortKey === key) sortDir = sortDir === 'asc' ? 'desc' : 'asc';
      ...
      sortKey = key;
      sortDir = key === 'title' || key === 'artist' || key === 'album' ? 'asc' : 'desc';
    }
  }

  function toggleDensity() {
    S.settings.libraryDensity =
  ...
  $effect(() => {
    ...
    if (tracks.length > 0) {
      actions.push({
        label: S.settings.libraryDensity === 'compact' ? t('common.comfortable') : t('common.compact'),
        icon: 'layout-list',
        variant: 'ghost',
...
<div class="wrap library-page">
  {#if selectedIds.size}
    <div class="batch-bar">
      ...
      <button type="button" class="btn-secondary" onclick={addSelectedToQueue}>{t('common.addToQueue')}</button>
      <button type="button" class="btn-ghost" onclick={() => (selectedIds = new Set())}>{t('common.cancel')}</button>
    </div>
  {/if}
  ...
  {:else if tracks.length}
    {#each tracks as track, i (track.id)}
      <TrackRow {track} tracks={tracks} index={i} compactActions />
    {/each}
  {:else}
