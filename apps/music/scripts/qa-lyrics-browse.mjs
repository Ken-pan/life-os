...
if (
  !/首有音频但尚未获取歌词|missingLyrics/i.test(settingsText) &&
  !settingsText.includes('尚未获取歌词')
) {
  mark(
    'medium',
    'settings',
    'desktop',
    '缺歌词统计文案未展示（可能已全部有歌词或计数为 0）',
  )
}
...
if (await fetchBtn.isVisible()) {
  ...
  if (disabled)
    mark('low', 'settings', 'desktop', '「补抓歌词」按钮处于禁用状态')
}
...
if (await syncedEmpty.isVisible()) {
  ...
  const hasSyncedTag = await dPage
    .locator('.now-playing-lyrics-tag')
    .isVisible()
  if (!hasSyncedTag)
    mark('medium', 'now-playing', 'desktop', 'LRC 歌词未显示「同步」标签')
  ...
  if (lineCount < 5)
    mark(
      ...
      'now-playing',
      'desktop',
      `同步歌词行数过少（${lineCount} 行）`,
    )
  const lyricsBox = await syncedLyrics.boundingBox().catch(() => null)
  const layout = await dPage
    .locator('.now-playing-layout')
    .boundingBox()
    .catch(() => null)
...
if (afterFetchEmpty) {
  mark(
    ...
    'now-playing',
    'desktop',
    '懒加载 14s 后仍未获取到 Bohemian Rhapsody 歌词',
  )
...
const plainSyncedTag = await dPage
  .locator('.now-playing-lyrics-tag')
  .isVisible()
if (plainSyncedTag)
  mark('low', 'now-playing', 'desktop', '纯文本歌词误显示「同步」标签')
const plainLines = await dPage.locator('.now-playing-lyrics-line').count()
if (plainLines < 2)
  mark('high', 'now-playing', 'desktop', '纯文本歌词未正确渲染')
...
if (mEmpty) {
  const mEmptyText = await mPage
    .locator('.now-playing-lyrics-empty')
    .innerText()
...
if (!issues.length) {
  ...
  for (const i of issues) {
    lines.push(
      `- **[${i.id}] ${i.severity.toUpperCase()}** · ${i.page} / ${i.viewport}: ${i.issue}`,
    )
  }
}
