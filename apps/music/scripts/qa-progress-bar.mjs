...
async function playFirstTrack(page) {
  ...
  await page.waitForSelector('.track-table-row, .track-row', { timeout: 15_000 });
    ...
    const rects = (sel) =>
      [...document.querySelectorAll(sel)].map((el, i) => {
        ...
        const thumb = el.matches('input[type="range"]')
          ? {
              webkitThumbVisible: cs.getPropertyValue('-webkit-appearance') !== 'none'
            }
          : null;
        return {
          selector: sel,
          ...
          progressPct: cs.getPropertyValue('--progress-pct') || el.style.getPropertyValue('--progress-pct'),
...
async function setSimulatedProgress(page, pct = 0.35) {
  await page.evaluate((pct) => {
    ...
    for (const input of document.querySelectorAll('input[type="range"][aria-label="进度"]')) {
    ...
    if (fill instanceof HTMLElement) {
      ...
      if (rail instanceof HTMLElement) rail.style.setProperty('--progress-pct', `${pct * 100}%`);
    }
  }, pct);
}
...
await dPage.evaluate(() => {
  ...
  const inputs = document.querySelectorAll('input[type="range"][aria-label="进度"]');
...
const topBar = dMini.bars.find((b) => b.selector.includes('mini-player-top-progress'));
const inlineBar = dMini.bars.find((b) => b.selector.includes('mini-player-progress--inline'));

if (topBar && inlineBar) {
  mark('PB-01', 'high', 'MiniPlayer 桌面', '1440×900', '桌面端仍同时存在顶栏细线与内联进度条，违反单一控制源原则', s);
} else if (inlineBar && !topBar) {
  mark('PB-01', 'low', 'MiniPlayer 桌面', '1440×900', '桌面端仅保留内联标准进度条 ✓', s);
}
if (topBar && topBar.height < 20) {
  mark('PB-02', 'low', 'MiniPlayer 顶部进度', '1440×900', `顶部进度条可见高度仅 ${topBar.height}px，默认无 thumb（需 hover 才出现），可发现性弱`, s);
}
if (inlineBar && !inlineBar.visible) {
  mark('PB-03', 'high', 'MiniPlayer 桌面', '1440×900', '桌面展开态内联进度条未显示', s);
}

// Focus keyboard (desktop uses inline seek on expanded mini player)
const inlineInput = dPage.locator('.mini-player-progress--inline .seek-bar-input');
if (await inlineInput.count()) {
  ...
  mark('PB-04', 'low', 'MiniPlayer 内联进度', '1440×900', 'focus-visible 显示 thumb 光晕，键盘用户可定位播放位置', s);
}
...
const heroSeek = dNp.bars.find((b) => b.selector.includes('np-desktop-hero-seek'));
const quietProgress = dNp.bars.find((b) => b.selector.includes('player-progress--quiet'));

if (heroSeek) {
  if (heroSeek.height < 20) {
    mark('PB-05', 'medium', 'Now Playing 桌面紧凑 Hero', '1440×900', `Hero seek 热区高度 ${heroSeek.height}px，已可交互`, s);
  }
} else if (quietProgress?.visible) {
  if (quietProgress.height <= 5) {
    mark('PB-05', 'medium', 'Now Playing 桌面', '1440×900', `quiet 进度轨道高度 ${quietProgress.height}px`, s);
  }
}
if (!quietProgress?.visible && !heroSeek) {
  mark('PB-07', 'medium', 'Now Playing 桌面', '1440×900', '未检测到可 seek 的进度控件', s);
}

// Check time labels on desktop NP
const hasElapsed = dNp.timeEls.some((t) => t.visible && t.text && !t.text.startsWith('-') && t.text.includes(':'));
const hasTotal = dNp.timeEls.some((t) => t.visible && t.text && !t.text.startsWith('-') && t.text.includes(':') && t.text !== '0:00');
if ((heroSeek || quietProgress) && !hasElapsed && !hasTotal) {
  mark('PB-08', 'medium', 'Now Playing 桌面', '1440×900', '进度条旁无 elapsed/total 时间标注', s);
}

// Lyrics panel progress sync indicator?
const lyricsProgress = await dPage.locator('.lyrics-panel input[type="range"], .now-playing-lyrics input[type="range"]').count();
if (lyricsProgress > 0) {
  s = await shot(dPage, '05-desktop-lyrics-progress');
  mark('PB-09', 'low', '歌词面板', '1440×900', '歌词区域存在额外进度控件，需确认是否与主进度条重复', s);
}
...
const mTop = mMini.bars.find((b) => b.selector.includes('mini-player-top-progress'));
const mInline = mMini.bars.find((b) => b.selector.includes('mini-player-progress--inline'));

if (mTop && mInline) {
  mark('PB-10', 'high', 'MiniPlayer 移动', '390×844', '移动端同时显示顶部细条 + 内联三件套进度（时间-滑条-时间），垂直空间浪费且双层 seek 易误触', s);
}
if (mInline && mInline.height < 44) {
  mark('PB-11', 'medium', 'MiniPlayer 移动内联进度', '390×844', `内联滑条热区高度 ${mInline.height}px，低于 44px HIG 建议（顶部条 ${mTop?.height ?? '?'}px）`, s);
}
...
const mobileProgress = mNp.bars.find((b) => b.selector.includes('np-mobile-progress'));
if (mobileProgress) {
  if (mobileProgress.height < 44) {
    mark('PB-12', 'high', 'Now Playing 移动', '390×844', `np-mobile-progress 触控高度 ${mobileProgress.height}px < 44px`, s);
  } else {
    mark('PB-12', 'low', 'Now Playing 移动', '390×844', `np-mobile-progress 触控高度 ${mobileProgress.height}px ✓ 达标`, s);
  }
}

// Time format: unified elapsed + total
const times = mNp.timeEls.filter((t) => t.visible).map((t) => t.text).filter(Boolean);
if (times.length >= 2) {
  const hasNegative = times.some((t) => t.startsWith('-'));
  if (hasNegative) {
    mark('PB-13', 'medium', 'Now Playing 移动', '390×844', `时间仍使用 remaining 格式（${times.join(' / ')}），应与桌面统一为 total`, s);
  }
}
...
if (await npSlider.count()) {
  ...
  if (box) {
    ...
    mark('PB-14', 'low', 'Now Playing 移动', '390×844', '拖拽时 thumb 放大效果正常，但轨道 visual 仍仅 12px（padding 撑热区），视觉反馈偏弱', s);
  }
}

// ── Cross-surface consistency ──
mark('PB-15', 'low', '全局', 'all', '进度条已统一为 SeekBar 组件 + seek-bar-input 样式，variant 控制布局', '');
...
await dPage.locator('.track-table-row').filter({ hasText: 'Zero Duration' }).locator('.track-table-icon-btn.play').click();
...
const edge = await dPage.evaluate(() => {
  const input = document.querySelector('.mini-player-top-progress .seek-bar-input, .mini-player-progress--inline .seek-bar-input');
  return input instanceof HTMLInputElement
    ? { max: input.max, value: input.value, disabled: input.disabled, pct: input.style.getPropertyValue('--progress-pct') }
    : null;
});
if (edge && !edge.disabled) {
  mark('PB-17', 'medium', '边界态', '1440×900', 'duration=0 时进度条应 disabled；检查 max/disabled 状态', s);
}
