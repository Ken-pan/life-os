    ...
    async ({ msg, tone }) => {
      ...
      const hrefs = [
        ...document.querySelectorAll(
          'link[rel="modulepreload"][href*="chunks/"]',
        ),
      ].map((el) => el.href)
      ...
      for (const href of hrefs) {
        try {
          ...
          for (const value of Object.values(mod)) {
            ...
            value('__qa_probe__', 'success', {
              key: '__qa_probe__',
              dedupeMs: 0,
            })
            if (stateObj.msg !== '__qa_probe__' || stateObj.show !== true)
              continue
            stateObj.show = false
...
const scenarios = [
  ...
  },
  {
    ...
    setup: async (page) => {
      await page.evaluate(() => {
        document
          .querySelector('.bottom-shell')
          ?.setAttribute('data-player-chrome', 'mini')
...
const failed = report.filter(
  (r) => !r.ok || r.opacity === '0' || !r.className?.includes('show'),
)
