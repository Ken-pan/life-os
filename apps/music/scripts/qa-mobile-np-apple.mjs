...
async function auditContrast(page) {
  return page.evaluate(() => {
    ...
    const bgSample = parseRgb(
      getComputedStyle(
        document.querySelector('.np-ambient-back-scrim') || document.body,
      ).backgroundColor,
    ) || [18, 14, 12]
    ...
    return {
      mode: document.querySelector('.np-mobile-compact-head')
        ? 'lyrics|queue'
        : document.querySelector('.np-mobile-cover-stage')
          ? 'player'
          : 'unknown',
      artLoaded: (() => {
        ...
        return img instanceof HTMLImageElement
          ...
          : Boolean(
              document.querySelector(
                '.np-mobile-art.placeholder, .now-playing-art.placeholder',
              ),
            )
      })(),
      ambientArtLoaded: (() => {
...
try {
  ...
  if (!a1.artLoaded)
    finding('A-01', 'warn', 'cover', '封面未加载或已回退 placeholder', s1)
  if (!a1.ambientArtLoaded)
    finding('A-02', 'warn', 'ambient', '模糊背景图未加载（mesh 仍可用）', s1)
  for (const c of a1.checks) {
    if (c.missing) finding('C-' + c.label, 'fail', c.label, '元素缺失', s1)
    else if (!c.ok)
      finding(
        'C-' + c.label,
        c.contrast >= 3 ? 'warn' : 'fail',
        c.label,
        `对比度 ${c.contrast}:1 (${c.color})`,
        s1,
      )
    else
      finding('C-' + c.label, 'pass', c.label, `对比度 ${c.contrast}:1 OK`, s1)
  }

  await page
    .locator('.np-mobile-dock-btn')
    .first()
  ...
  if (a2.mode !== 'lyrics|queue')
    finding('V-02', 'fail', 'lyrics', '未切换到歌词紧凑顶栏', s2)
...
const report = [
  ...
  ...findings.map(
    (f) =>
      `- **[${f.severity.toUpperCase()}] ${f.id}** (${f.area}) ${f.detail}${f.shot ? ` · \`${f.shot}\`` : ''}`,
  ),
  '',
].join('\n')
