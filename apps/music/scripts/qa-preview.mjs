...
function getServiceKey() {
  ...
  if (res.status !== 0)
    throw new Error(res.stderr || 'supabase api-keys failed')
...
async function runBrowserSuite(browserType, label) {
  ...
  const antiRow = page
    .locator('.track-row, li')
    .filter({ hasText: /Anti-Hero/i })
    .first()
  ...
  const playback = await page.evaluate(() => {
    const a =
      document.getElementById('music-os-player-a') ||
      document.getElementById('music-os-player-b') ||
    ...
    const mini =
      document.querySelector('.mini-player')?.textContent?.trim() || ''
    const sessionType =
      'audioSession' in navigator ? navigator.audioSession.type : ''
  ...
  log(
    `${label}-signed-url`,
    signedOk ? 'pass' : 'fail',
    signedOk ? 'Using cloud signed URL' : playback.src || 'no src',
  )

  const critical = consoleErrors.filter(
    (e) =>
      !/favicon|sourcemap|404.*\.map|Failed to fetch|Load failed|\/api\/lyrics/i.test(
        e,
      ),
  )
  log(
...
console.log(
  `PASS ${report.filter((r) => r.status === 'pass').length} / FAIL ${fails.length}`,
)
...
if (
  !chromiumOk ||
  !webkitOk ||
  fails.some((f) => /load|sync|idb|search|playback|signed/.test(f.step))
) {
  process.exit(1)
}
