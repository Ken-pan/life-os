...
const DICT = {
  genre: [
    'pop', 'dance-pop', 'electropop', 'dark-pop', 'alt-pop', 'k-pop', 'k-pop-solo', 'girl-group',
    'j-pop', 'c-pop', 'mandopop', 'hip-hop', 'rap', 'asian-hip-hop', 'r-and-b', 'alt-r-and-b',
    'edm', 'house', 'techno', 'trance', 'melodic-techno', 'hyperpop', 'soundtrack', 'game-ost',
    'anime', 'ambient', 'sleep'
  ],
  vibe: [
    'baddie', 'girl-crush', 'runway', 'club', 'sexy', 'quirky', 'meme', 'dark', 'euphoric', 'cute',
    'dramatic', 'confident', 'rebellious', 'luxury', 'neon', 'night-drive', 'soft', 'sad', 'angry',
    'playful', 'cinematic', 'boss-fight'
  ],
  context: [
    'homepage-safe', 'playlist-continue-good', 'gym', 'walking', 'running', 'party', 'focus', 'shower',
    'game', 'background', 'karaoke', 'transition-safe'
  ],
...
async function loadDictionaryFromDb() {
  const { data, error } = await db.from('tag_dictionary').select('slug, namespace').eq('is_active', true);
...
async function callLlm(batch) {
  ...
  const tracks = parsed.tracks || parsed.results || (Array.isArray(parsed) ? parsed : []);
  if (!Array.isArray(tracks)) throw new Error('LLM response missing tracks array');
  return tracks;
}
...
function pickTags(slugs, ns, baseConf) {
  ...
  for (const raw of list) {
    ...
    if (!slug) continue;
    out.push({ slug, confidence: Math.min(0.88, Math.max(0.55, baseConf)), source: 'llm' });
  }
  return out;
}
...
function mergeTags(llmRow, track, keepTags) {
  ...
  const hasGenre = merged.some((t) => slugNamespace.get(t.slug) === 'genre' && t.source === 'llm');
  const hasVibe = merged.some((t) => slugNamespace.get(t.slug) === 'vibe' && t.source === 'llm');
  if (!hasGenre) {
...
function buildEnrichment(llmRow, tags) {
  const avgConf = tags.length ? tags.reduce((s, t) => s + t.confidence, 0) / tags.length : 0;
  const hasLlmVibe = tags.some((t) => t.source === 'llm' && slugNamespace.get(t.slug) === 'vibe');
  const hasLlmGenre = tags.some((t) => t.source === 'llm' && slugNamespace.get(t.slug) === 'genre');
  ...
  const versionSlugs = tags.filter((t) => slugNamespace.get(t.slug) === 'version').map((t) => t.slug);
...
async function persistTrack(track, llmRow, existingTagRows) {
  ...
  if (dryRun) {
    console.log(`  ${track.title} — ${track.artist} → ${enrichmentPatch.tagging_status}`);
...
const tracks = await fetchPartialTracks();
console.log(`LLM tagging ${tracks.length} partial tracks (model=${MODEL}, batch=${batchSize})${dryRun ? ' [dry-run]' : ''}...`);
...
for (let bi = 0; bi < batches.length; bi++) {
  ...
  try {
    ...
    for (const track of batch) {
      ...
      await persistTrack(track, llmRow, existingByTrack.get(track.track_id) || []);
      ok += 1;
      console.log(`[${ok + fail}/${tracks.length}] ${track.title} — ${track.artist}`);
    }
  } catch (err) {
    fail += batch.length;
    console.error(`Batch ${bi + 1}/${batches.length} FAIL: ${err.message || err}`);
  }
