...
const approvalReadSource = read(
  'apps/aios/src/lib/kenos/approvalReadSource.core.js',
)
const approvalReadTests = read(
  'apps/aios/src/lib/approvalReadSource.core.test.js',
)
...
const contractManifest = JSON.parse(
  read('packages/contracts/fixtures/kenos/v1/manifest.json'),
)
const contractCorpus = JSON.parse(
  read('packages/contracts/fixtures/kenos/v1/corpus.json'),
)
const swiftContracts = read(
  'clients/apple/Packages/KenosContracts/Sources/KenosContracts/KenosContracts.swift',
)
const approvalSql = read(
  'apps/planner/supabase/review/20260719030000_kenos_action_approvals.sql',
)
const approvalSqlTest = read(
  'apps/planner/supabase/tests/kenos_action_approvals.sql',
)
...
for (const [path, name] of requiredRoutes) {
  if (
    !manifest.routes.some((route) => route.path === path && route.name === name)
  ) {
...
for (const token of [
  ...
  if (!readSources.includes(token))
    fail(`read adapter missing expected boundary: ${token}`)
}
...
if (
  JSON.stringify(contractManifest.approvalStatuses) !==
...
for (const token of [
  ...
  if (!contracts.includes(token))
    fail(`canonical Approval contract missing ${token}`)
}
for (const field of [
  'id',
  'version',
...
const validApprovalFixtureIds = new Set(
  contractCorpus.valid
    .filter((fixture) => fixture.contract === 'approvalRecord')
    .map((fixture) => fixture.id),
)
const invalidApprovalFixtureIds = new Set(
  contractCorpus.invalid
    .filter((fixture) => fixture.id.startsWith('approval-'))
    .map((fixture) => fixture.id),
)
for (const id of [
  'approval-record-pending-r2',
  'approval-record-pending-r3',
  'approval-record-approved',
  'approval-record-rejected',
  'approval-record-expired',
  'approval-record-cancelled',
  'approval-record-superseded',
  'approval-record-redacted-sensitive',
  'approval-record-multiple-entity-refs',
]) {
  if (!validApprovalFixtureIds.has(id))
    fail(`canonical Approval valid corpus missing ${id}`)
}
for (const id of [
  'approval-malformed-uuid',
  'approval-owner-mismatch',
  'approval-unknown-status',
  'approval-invalid-risk',
  'approval-approved-without-decision',
  'approval-pending-with-decision',
  'approval-invalid-expiry',
  'approval-secret-summary',
  'approval-action-id-mismatch',
  'approval-unsupported-version',
  'approval-illegal-transition',
]) {
  if (!invalidApprovalFixtureIds.has(id))
    fail(`canonical Approval invalid corpus missing ${id}`)
}
for (const token of [
  'KenosApprovalStatus',
  ...
  if (!swiftContracts.includes(token))
    fail(`Swift Approval parity missing ${token}`)
}

for (const token of [
  "rpc('kenos_list_action_approvals'",
  'projectApprovalRows',
  'executorAvailable',
  'sourceFreshness',
  'requestingActor',
...
for (const token of ['expired', 'superseded', 'Executor', '只读']) {
  if (!approvalsPage.includes(token))
    fail(`Approvals UI missing read-only state token ${token}`)
}
if (
...
for (const token of [
  ...
  if (!approvalSql.includes(token))
    fail(`Approval review SQL missing security invariant: ${token}`)
}
for (const token of [
  'set local role anon',
  'set local role authenticated',
  'generic service_role must not become the Approval writer',
  'expected direct insert denial',
  'expected direct update denial',
  'expected direct delete denial',
  'user B must see exactly one own Approval',
  'read RPC leaked another owner',
  'approval_expired',
  'approval_superseded',
]) {
  if (!approvalSqlTest.includes(token))
    fail(`Approval SQL privilege test missing ${token}`)
}
for (const category of [
  ...
  if (!readProjections.includes(`'${category}'`))
    fail(`shadow diagnostics missing ${category}`)
}
for (const [path, contents] of [
  ...
  for (const forbidden of [
    ...
    if (forbidden.test(contents))
      fail(`${path} crosses the read-only/no-Executor boundary: ${forbidden}`)
  }
}
...
if (/life_events|outbox/i.test(approvalReadSource)) {
  fail(
    'Assistant must not treat Activity/life_events/Outbox as canonical Approval',
  )
}
for (const token of [
...
if (
  !/PORTAL_PRODUCTION_APPS\s*=\s*PORTAL_APPS\.filter\(\(app\)\s*=>\s*!app\.experimental\)/.test(
    portalApps,
  )
) {
...
for (const token of [
  ...
  if (!portalActions.includes(`id: '${token}'`))
    fail(`Portal command palette missing ${token}`)
}
...
for (const token of [
  ...
  "hostname: 'localhost'",
  "'?kenos=1'",
  "'/assistant?return=%2Finbox'",
]) {
  if (!routingTest.includes(token))
    fail(`Portal compatibility routing test missing ${token}`)
}
...
for (const lock of [
  ...
  if (!runbook.includes(lock))
    fail(`Phase 2 runbook missing production lock: ${lock}`)
}
...
for (const document of [runbook, ledger, state]) {
  if (!document.includes('LOCAL_READ_ONLY_READY_NO_HOSTED_APPLY')) {
    fail(
      'runbook, ledger, and execution state must use honest LOCAL_READ_ONLY_READY_NO_HOSTED_APPLY primary label',
    )
  }
  if (
    !document.includes('TEMPORARY_APPROVED_FOR_PHASE_2_APPROVAL_READ_MODEL')
  ) {
...
if (
  !controlCore.includes('formatQueueCount') ||
  !controlCore.includes('inboxAvailable')
) {
...
if (
  !shadowFixtures.includes('legacyPortalPendingShadowFixture') ||
  !controlCenter.includes('legacyPortalPendingShadowFixture')
) {
  fail('Inbox/Activity shadow must use independent legacy fixtures')
}
if (
  !read('apps/aios/src/lib/mcp.presets.js').includes(
    'writeToolsBlockedUntilHostedRpc',
  )
) {
...
for (const document of [runbook, ledger, state]) {
  for (const lock of ['Executor', 'production', 'cutover']) {
    if (!document.includes(lock))
      fail(`Phase 2 docs must preserve ${lock} as a locked gate`)
  }
}
