...
deploy_one() {
  ...
  if [[ -n "$functions" ]]; then
    npx netlify deploy --prod --site="$site_id" --dir="$dir" --functions="$functions"
  else
    npx netlify deploy --prod --site="$site_id" --dir="$dir"
  fi
}
...
deploy_one abf63261-ae26-46f9-ac18-d24987a44113 apps/music/build
