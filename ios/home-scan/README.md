...
6. 柜内扫描：主页已上传扫描点「柜内」→ 挑柜页应标出已扫柜子的
   内腔摘要（「已扫：内 80×35×190cm · 2 层」，按钮变「重测」）→
   打开柜门按引导点六个位置（第 N/6 点；点完对应两点 HUD 立刻出
   「内宽 78 cm」实时读数）→ 六点拟合成功后柜内应出现**黄色线框**，
   框住的就是内腔（框歪 = 测歪，点「重新测量」）→ 层板自动识别 +
   手动点前沿补，每块显示为**青色半透明面** → 正面/斜侧两张照 →
   确认页可 ±1cm 微调三维（原实测值仍随 payload 保留为
   `measuredInteriorIn`）→ 上传。拿卷尺对比内腔三维(目标 ±3cm)与
   每层高；桶里应出现 `container-{placementId}.json` + 两张 jpg；
   回到挑柜页摘要应刷新
...
8. 目标标记与「不卡死」（模拟器测不了，RoomPlan 要 LiDAR 真机）：
   - **指得出是哪一件**：出现补拍引导时，画面里应同时有黄色准星套住那件家具；
     目标转出画面后，准星应换成屏幕边缘的黄色箭头（箭头朝向 = 该转的方向，
     带名字与距离）；人转回去箭头应变回准星
   - **对上了要说话**：把目标整个纳入画面，准星应立刻变绿并显示「对上了，别动」，
     且**半秒内**真的抓到照片（HUD 的「N 件家具照」+1）。绿了却不拍 = 回归
   - **引导不许说谎**：文案不该再出现「稳住 2 秒」（代码里没有任何停留计时）
   - **高家具**（≥1.8m 的高柜/货架/衣柜）：引导距离应明显比旧版远（约 2.4–3m），
     且到位后能拍全；旧版会把人叫到 1.2–1.4m 处，那里镜头竖直方向装不下它，
     于是永远拍不上（本次修的主 bug）
   - **超时换人**：故意对着一件怎么都拍不到的家具（比如塞在角落的高柜）干等，
     30 秒后引导应**自动换到下一件**，不该把 HUD 永久钉死在同一句上
   - **机位提示要说真原因**：关灯 → 应提示「这里太暗…」；站回刚拍过的位置不动 →
