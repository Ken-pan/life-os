...
1. **P-MOVE-UI Slice 1.1 device re-verify** — native + QML commits landed; operator pass before Slice 2.
2. **P-MOVE-VERIFY** — device production fetch with token (route OK: `curl` → 401, 2026-07-10).
3. **P-MOVE-UI Slice 2** — 合并 Home/Today + 最终 drawer IA（1.1 设备复验后）。
...
10. **生产写启用**（P2，被 staging 验证阻塞）— 见 roadmap P-MOVE-5。
