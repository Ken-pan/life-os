...
| 字体集成 | 实现方式未指定 | **已实现，正是报告推荐的保守方案** — `QFontDatabase::addApplicationFont()` + 全局默认 family（未依赖 Qt 6.8 fallback API）；缺字体时回退平台默认、不 crash | `src/main.cpp` `loadPaperOsFont()`；[P-MOVE-3 gate](PRO_MOVE_P_MOVE_3_CJK_PAGINATION_GATE.md) |
...
| 离线 / last-good cache | 未指定，列为 P0 差距 | **已实现** — 启动先读 `cache.json` 再发网络请求；成功响应原子写入（`.tmp` + rename）；失败时保留旧数据并显示错误，`last_sync.txt` 单独维护；断网启动已实测通过 | `src/ApiClient.cpp` `loadCache()/saveCache()`；[P-MOVE-2 gate](PRO_MOVE_P_MOVE_2_READ_CACHE_GATE.md) 离线验证 |
...
| Device launcher | 未指定 | **已实现且超出报告的 home-only 方案** — `open-paperos.sh`（trap 恢复 + exit code 记录）加 `paperos.service`（unit 文件在 /home，根分区仅 symlink） | [P-MOVE-4 gate](PRO_MOVE_P_MOVE_4_EXIT_RECOVERY_LAUNCHER_GATE.md) |
...
| Scheduled Planner Mode | 未指定，P1 | **未实现（有意推迟）** — 与报告建议一致：先手动、后定时 | roadmap P-MOVE-6 |
...
1. **手动 Sync 按钮**（P1）— UI 已有 sync 状态展示，但无"Sync now"按钮；
   `ApiClient::fetchDashboard()` 已可直接复用，工作量小。
2. **定时 cache 刷新 timer**（P1）— `refresh-cache.sh` + systemd timer
   （同样走 link 方案），先只刷 cache，不做自动进入 PaperOS。
3. **性能基线**（P1）— 冷启动时长、翻页延迟、RSS 内存记录进 gate 文档。
4. **字体回归测试**（P1）— 中英混排 + 新页面的固定验收字符串。
5. **OS 升级演练**（P1）— 升级后重 link unit + 重传 binary 的实操记录。
6. **CI/CD 与部署 profile**（P2）— 自托管 runner 或保持本机脚本；
   Claude Code 部署权限单列 profile。
7. **生产写启用**（P2，被 staging 验证阻塞）— 见 roadmap P-MOVE-5。
