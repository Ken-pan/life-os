...
## 8m. P1-3 Canvas 能力层 + Peek 回家路(2026-07-23 第二轮真机)

### Canvas 终于不再是「大号 Capture」
此前 Canvas 档 = 同一个输入框拉高到 95%,多出来的高度没换来任何新能力。
现在它**按行拆成多条各自定向的草稿**:人在 Canvas 里写的本来就是清单,
一次性存成一条 blob 等于把拆分工作推给未来的自己。

```
修复登录跳转            → 草稿 1
- 明天下午过一遍预算     → 草稿 2(时间语义 → plan 候选)
买跑鞋                  → 草稿 3
```

每条**各自**过 `KorbenCaptureRouter` —— 整段一起分类会让「明天…」的时间语义
被其它行的噪音淹掉。创建**前**在面板里摊开给你核对(将创建几条、哪几条已识别
去向),主按钮文案随之变成「创建 3 条」:按下去会发生什么,按之前就该知道。

纪律不变:拆分只影响**草稿条数与 hint**,执行仍是 L1 `capture.review` 队列,
原生端不直写任何 Domain 数据。拆错了在收件箱合并即可,写不坏东西。

**这层真正的风险不是"拆得不够聪明",是吞掉用户写的东西** —— 故:
- 超过 20 条的溢出部分**并入最后一条**,不静默丢弃;
- 清单符号只在后随空白时剥离(`跨-应用同步`、`2026. 计划` 不受影响);
- 部分失败**全批回滚**(半批成功最难收拾:用户不知道哪几条进去了,重试必然重复);
- Undo 是整批的,且只要**有任何一条**已派发就判定撤销失败(否则会回填输入框,
  用户以为全撤了重新创建,已派发那条就重复了)。
10 个单测锁死上述边界,其中一半锁的是"不丢字"。

### Peek 的回家路(真机测试逼出来的真缺口)
Peek 列的是「其它空间」,而 Today 不是其中之一 —— 它是壳的家。于是从任何
Domain 打开 Peek **回不去**,只能绕道 Space Center。测试里 `korben.spacePeek.today`
查不到,才把这个洞暴露出来。已补一行整行样式的 Today(与磁贴刻意不同形:
它是回家,不是又一个平级目的地)。

### 行为变更连带的测试债(诚实记录)
Orb Tap 从「开 Switcher」改成「开 Peek」后,旧 Device Gate 套件里 **13 个用例**
一次性变红 —— 它们的导航助手编码的是旧契约。这不是 flake,是我改行为该付的账。
