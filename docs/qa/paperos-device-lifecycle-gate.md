...
**Status:** ⏳ **BLOCKED** — 依赖 `P-MOVE-SYS-1` launch surface + implementation · `SYS-2` · `SYS-3`
**Owner:** Ken + Codex
**Agent 线:** Line B（Shell）

> **Antigravity 不能**替代本 gate：网页截图无法证明物理睡眠、Folio 或 system suspend。

**前置状态（2026-07-11）：** VERIFY PASS · SYS-0 accepted · SYS-1A closed · SYS-1B
incomplete · SYS-1 implementation blocked · SYS-2 hard blocked。见
[`paperos-device-lifecycle-discovery.md`](./paperos-device-lifecycle-discovery.md)。

## 仍待验证项（SYS-0 / discovery 未覆盖）

以下在 SYS-GATE 前仍为开放项，不得因 SYS-0 accepted 或 P-MOVE-4 PASS 而视为已测：

- 冷启动与原生解锁（LC-01）
- 设备端无 Mac/SSH 进入（LC-04）— SYS-1B discovery 进行中
- Folio 合盖/开盖（LC-06）— 当前设备无 Folio
- 重复 enter/exit 循环（Mode B 升级表 10/10）
- 重复崩溃 / crash-loop fallback（LC-10、LC-11）— 仅单次 `kill -9` 已测
- readiness 失败路径
- wrapper 中断恢复
- PaperOS 前台短按电源键睡眠 UX（LC-05）— SYS-0 判定 FAIL
- 写字中 suspend（LC-08）
