...
**灵动岛下细线**：根因是双层顶栏（tint + appbar）在 safe-top 接缝。策略：有顶栏时隐藏 tint；iOS 移动端 `--safe-top-effective: max(59px, env())`；`.app-shell::before` 固定绘制状态栏带（Sorcha/chronosnap）；`.appbar-inner` 承担 safe-top padding。验收：`node scripts/pwa/capture-chrome-screenshots.mjs` + `npm run pwa:sim:capture`。
...
npm run pwa:sim:capture              # 六端核心路由 Xcode 截图（?pwa_sim=1）
PWA_APP=fitness npm run pwa:sim:capture
