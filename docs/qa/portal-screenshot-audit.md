...
**证据目录：** `ui-qa-screenshots/portal/main/latest/`（截图为 gitignored 可重生成产物；跑 `qa-ui-screenshots.mjs` 重建）
...
- `ui-qa-screenshots/portal/main/latest/manifest.json`（gitignored；`qa-ui-screenshots.mjs` 重建）
