...
| App         | 命令                                              | 结果                              | 说明                                          |
| ----------- | ------------------------------------------------- | --------------------------------- | --------------------------------------------- |
| **Planner** | `cd apps/planner && CI=1 npm run test:e2e`        | ✅ desktop **22/22**              | **PLNR.CORE.2** ✅ `localDateKey()` 修复 P-1 Insight |
| **Fitness** | `cd apps/fitness && CI=1 npm run test:e2e`        | ✅ **20/20** pass                 | GYMS.CORE.0 ✅（5190）                              |
| **Finance** | `qa:ia-routes` + `qa:ia-nav`                      | ✅ 22/22 + 31/31                  | FINC.CORE.0 ✅ `ia-qa-auth.mjs`（FN-1 已修）         |
| **Music**   | `test:sw:full` + `qa:ui-flow` + `qa:rec-behavior` | ✅ 21/21 + 15/15 + **6/6** (MUSC.PIPE.5) | MUSC.UI.2 + MUSC.PIPE.5 ✅                                |
...
- **Desktop `[desktop]`**：**22/22** ✅（含 Insight 批量排期 — **PLNR.CORE.2** 2026-07-09）
...
### 问题 P-3 · 控制台警告（非阻断）— ✅ PLNR.CORE.6 已修复（2026-07-08）
...
### 问题 QA-GYMS-0 · 端口冲突（已修复 2026-07-08）✅

（legacy 问题码：`F-0` · ~~`QA-FITN-0`~~）
...
### 问题 FN-1 · 路由 smoke 未注入 Auth — ✅ 已修复（FINC.CORE.0）
...
UI_QA_URL=http://127.0.0.1:5180 npm run qa:ia-routes         # 22/22 ✅（FINC.CORE.0）
```
...
**命令：** `npm run test:sw:full`（SW）· `npm run qa:ui-flow` / `test:e2e`（UI 流，**MUSC.UI.2**）

### 结果 ✅

- **SW 21/21 passed**（static / lifecycle / precache / fetch / purge / trim / integration）
- 报告：`docs/ui-qa-screenshots/music/service-worker/latest/REPORT.md`

### UI 流 E2E ✅（MUSC.UI.2 · `scripts/qa-ui-flow.mjs`）

| 项       | 内容                                                                        |
| -------- | --------------------------------------------------------------------------- |
| **命令** | `MUSIC_QA_URL=http://127.0.0.1:5189 npm run qa:ui-flow`                     |
| **覆盖** | 8 条路由 smoke · IDB seed · 曲库播放 · 正在播放队列 · 导入页可达            |
| **Auth** | `ia-qa-auth.mjs` — `signInWithPassword` + `life_os_auth`（同 Finance FINC.CORE.0） |
...
| 项                                                        | 结果                               |
| --------------------------------------------------------- | ---------------------------------- | --------------- |
| **PORT.GROWTH.8** pending 铃铛角标 + 状态区深链                    | ✅ `.portal-inbox-btn`             |
...
| **PORT.GROWTH.9** smoke — 登录 · **五卡** · inbox href · ⌘K · Esc  | ✅                                 |
| **PORT.GROWTH.4b-M / PORT.GROWTH.6 / PORT.GROWTH.4b-H**                              | 五卡摘要 · 深链 · Home 储藏        | ✅ 截图 + smoke |
| **MUSC.PIPE.5** Music 摘要有播放记录（seed 后）                  | ✅ `desktop-summary.png`           |
...
| -------- | ------------- | ---------------------------- | ---------------------------- | -------------------------- |
| —        | **FINC.CORE.3**      | Finance                      | STS / Scenarios / Spend 对齐 | ✅ 2026-07-09              |
| —        | **PORT.GROWTH.4b-M**   | Portal                       | Music 第四卡                 | ✅ 2026-07-09              |
| —        | **PORT.GROWTH.6**      | Portal                       | ⌘K 跨站深链                  | ✅ 2026-07-09 · 见走查     |
| —        | **PLNR.CORE.2**      | Planner                      | Insight 排期（P-1）          | ✅ desktop 22/22           |
| —        | **GYMS.CORE.0**     | Fitness                      | E2E 20/20                    | ✅ 2026-07-09              |
| —        | **GYMS.EVENTS.1**     | Fitness                      | workout_logged 触发器        | ✅ 2026-07-09              |
| —        | **MUSC.UI.2**      | Music                        | UI E2E `qa-ui-flow` 15/15    | ✅ 2026-07-09              |
| —        | **FINC.CORE.0**      | Finance                      | route smoke 22/22            | ✅ 2026-07-09              |
| **MUSC.PIPE.5** | Music         | 行为分 `qa:rec-behavior` 6/6 | ✅ 2026-07-09                |
| —        | **PORT.GROWTH.8**      | Portal                       | pending → inbox 深链         | ✅ 2026-07-09 · 走查第五轮 |
| —        | **PORT.GROWTH.9**      | Portal                       | `qa:smoke` 五卡              | ✅ 2026-07-09              |
| —        | **P-5b/P-12** | Portal                       | More sheet · 实验卡虚线      | ✅ 2026-07-09              |
| —        | **PORT.GROWTH.4b-H**   | Portal                       | Home 第五卡                  | ✅ 2026-07-09              |
| —        | **HOME.PROJ.6a**     | Home                         | 储藏元数据 → core\_\*        | ✅ 2026-07-09              |
