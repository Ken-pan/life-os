...
| **FN-02** | 设置页未展示 `graphOpenings.length` / 顶点·墙段统计 | UI 未接 | 「户型编辑模式」卡片增加只读统计 | H-W5 或 quick win |
| **FN-03** | 墙图浏览仍显示 508 房间填充快照（预期内） | `carry.rooms` | H-W3 用 `zones[]` 替换；浏览态标注「房间名为 508 快照」 | H-W3 |
