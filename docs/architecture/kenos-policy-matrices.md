...
> This table is a readiness gate for `KR-P1-001 Plan create task Action/Outbox vertical slice`. It is not approval; Ken remains the only approver. `TEMP_DEFAULT` means the implementation may proceed only if Ken explicitly accepts the temporary default before coding starts.
