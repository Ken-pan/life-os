...
| Phase 2 Approval read model | TEMPORARY_APPROVED_FOR_PHASE_2_APPROVAL_READ_MODEL | Ken | 2026-07-19 | Platform/System owns the canonical lifecycle; Assistant reads only; requesting domain owns Action/business object; Activity/Outbox are references, never truth | Mandatory review before any real approve/reject or Executor integration |
| Phase 3 Work ownership model | TEMPORARY_APPROVED_FOR_PHASE_3_WORK_FOUNDATION | Ken | 2026-07-19 | Work owns Project/Deliverable/Meeting/Decision/context/status/source refs; Plan owns Task lifecycle; Library owns documents; Assistant/Connector are non-owners; WorkActionProposal≠Task; OPEN-002 still blocks body mirroring | Mandatory review before production cutover / Executor / auto Connector write |
