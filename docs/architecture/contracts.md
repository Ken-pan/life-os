...
远程 DB：`life_events` 表 migration `20260708000000` **已 apply**（见 [`../ops/supabase.md`](../ops/supabase.md)）；Planner 消费端 `lifeEventsInbox.js` ✅。

### 不 export（future / docs only）

见 [`native-readiness.md`](./native-readiness.md)：`WidgetSnapshot`、`LiveActivitySession`、`PlaybackState`、`SecurityCapability` 等。
...
`.seg`, `.seg-scroll` — IME guard 见 [`../qa/input-ime.md`](../qa/input-ime.md)（web-only）。
