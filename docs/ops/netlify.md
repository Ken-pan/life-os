...
> **Kenos 主站（`apps/aios`）：** Netlify site `kenos-www`（id `5bfa64b2-7108-479d-b9e2-45f9c4d9f791`），正式域 `www.kenos.space`（rollback `kenos-www.netlify.app`）。`www` 若 CNAME 到 apex，则 apex `@` 必须是 Netlify A（`13.52.188.95` / `52.52.192.191`），不能停在 GoDaddy 停车页。详见 [`../roadmap/apps/aios.md`](../roadmap/apps/aios.md)。
...
Finance / Fitness MCP（`/api/mcp`）CLI 兜底同样必须带 `--functions=netlify/functions`（相对 `apps/<app>`）。`deploy-all-netlify.sh` 已对齐；漏传则生产对 `/api/mcp` 返回 **404**。

```bash
npm run build:finance
CI=1 npx netlify deploy --prod --no-build --filter finance-os --dir=apps/finance/build --functions=netlify/functions --site=fc92f305-8dcf-46c3-82f5-ef511597df1c

npm run build:fitness
CI=1 npx netlify deploy --prod --no-build --filter fitness-os --dir=apps/fitness/build --functions=netlify/functions --site=0394cf19-7fb7-4fea-81d7-d4a9d025fab3
```

部署后验收：`npm run qa:mcp-fleet`（Home/Planner/Finance/Fitness initialize 均应 HTTP 200）。
