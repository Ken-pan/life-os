...
- state 文件记录 baseline、每个 slice、blocker、checkpoint commit/PR metadata（若平台产生）和最终报告；
- 没有生产写入、部署、自行创建额外 branch/worktree/stash/ref、跨范围代码迁移或虚假“已实现”。
...
1. 确认输入 baseline 来自现有 `origin/master` 或 owner 指定的 master-derived commit，记录 `git rev-parse HEAD` 和 parent。平台临时 work branch 只作为审查载体，不是 source of truth。
2. 确认没有 merge/rebase 状态；不得自行创建额外 branch/worktree/stash/ref。
3. 读取全部权威文档。
4. 运行并记录：
...
8. 继续下一 slice；可以让平台在单一临时 work branch 上产生范围清晰的 checkpoint commit/PR metadata，但不得自行 push、merge 或创建额外 branch/worktree/stash/ref；不等待普通实现确认。
...
- 不得自行创建额外 branch、worktree、stash、checkpoint ref 或并行 PR；平台自动临时 work branch/PR metadata 只能作为审查载体。
- 不得 push、merge 或修改远程 Git 状态；checkpoint commit 仅限平台临时 branch 上的清晰范围。
...
7. Phase 完成并形成 readiness/final report；必须停止等待人工审核，不能自动进入下一 Phase runtime implementation。
