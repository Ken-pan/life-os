...
owner: kenpan
last_verified: 2026-07-14-aios-seventh-app-and-0714-ship-sync
...
Life OS 是 **个人生活平台**（Planner / Fitness / Finance / Music 四生产站 + Portal 启动器 + **Home 实验** + **AIOS 本地优先 AI 助手** + **KnowledgeOS 本地优先长期记忆层**），通过共享身份、事件总线和设计 token 保持各 app 独立又一致。六个 web 站部署在 Netlify;AIOS / KnowledgeOS 是原生 Mac app（云端仅只读查看器/尚未接云）。
...
### 六 app 一览（+ AIOS 第七、KnowledgeOS 第八实验站）
...
| AIOS    | 实验/本地优先 | [aios-kenos.netlify.app](https://aios-kenos.netlify.app)（云端只读） | `aios-os` | ✅ | ❌ 待研判 | 高速迭代中（AIOS.1–25）· 详见 [`roadmap/apps/aios.md`](./roadmap/apps/aios.md) |
| KnowledgeOS | 实验/本地优先 | [knowledgeos-ken.netlify.app](https://knowledgeos-ken.netlify.app)（云端 localStorage 模式） | `knowledge-os` | ✅ | ❌ 待研判 | **KNOW.VAULT.0** Vault 路径可配置+监听 · 详见 [`roadmap/apps/knowledge.md`](./roadmap/apps/knowledge.md) |
