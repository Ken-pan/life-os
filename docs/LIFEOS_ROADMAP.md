...
Life OS 是 **个人生活平台**（Planner / Fitness / Finance / Music 四生产站 + Portal 启动器 + **Home 实验** + **AIOS 本地优先 AI 助手**），通过共享身份、事件总线和设计 token 保持各 app 独立又一致。六个 web 站部署在 Netlify;AIOS 是原生 Mac app（云端仅只读查看器）。
