...
| 模式                    | 代表                                                                                                                | 对 Life OS 的启示                                                  |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------ |
| **统一入口 + 边缘 SSO** | [brainbot](https://github.com/slaguardia/brainbot) `#apps` launcher                                                 | Portal 已是 launcher；差的是**读 `core_*` 而非仅 localStorage**    |
...
| 项       | 说明                                                                                                          |
| -------- | ------------------------------------------------------------------------------------------------------------- |
| **现状** | `createCoreIdentityHandler` 已在各站登录写 `last_opened_at`；Portal `recentApp.svelte.js` **仅 localStorage** |
...
| 项       | 说明                                                                           |
| -------- | ------------------------------------------------------------------------------ |
| **现状** | migration `20260707230000` 已有 `core_profiles.default_app`                    |
...
| 项       | 说明                                                         |
| -------- | ------------------------------------------------------------ |
| **现状** | 推荐 RPC + 标签管道 ~62%；生产 `play_events` 为 0            |
...
| 项       | 说明                                                                                  |
| -------- | ------------------------------------------------------------------------------------- |
| **现状** | `apps/finance/extension` 抓取 Robinhood/Rocket Money/Fidelity → `ExtensionSyncBridge` |
...
按需
  ...
  H-P2 + H-P3  Home SSO（每天用 Home 时可提前到 Week 1）
  G-P4  今日摘要卡片
