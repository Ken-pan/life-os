# Growth 主线（G-_ / M-_ / F-\* / HOME.PORTAL.1–HOME.SSO.3）

Hub 状态见 [`../LIFEOS_ROADMAP.md`](../LIFEOS_ROADMAP.md) §Next · §推荐执行顺序。

**桶定义：** 在 Core/Infra 闭环后，用**极小改动**放大已有基建（`core_*`、`life_events`、Portal、各 app RPC/扩展），提升跨 app 一体感。不做全站 AI；**Home 保持实验 tier**（HOME.PORTAL.1–HOME.SSO.3 ✅ 2026-07-09）。
...
| 模式                    | 代表                                                                                                                | 对 Life OS 的启示                                                  |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------ |
| **统一入口 + 边缘 SSO** | [brainbot](https://github.com/slaguardia/brainbot) `#apps` launcher                                                 | Portal 读 `core_*` + PORT.GROWTH.4 摘要 + Home 实验入口 ✅（2026-07-09）    |
...
- 无场景的第二条 `life_events` 或 INTG.EVENTS.2 智能推荐

---

## PORT.GROWTH.1: Portal「继续」读 `core_user_app_settings` {#g-p1}
...
| **投入** | 0.5–1d                                                                                         |
| **依赖** | INTG.EVENTS.1 DB constraint 含 `portal`                                                                 |
...
## PORT.GROWTH.2: Portal 待办 / 事件角标 {#g-p2}
...
| **投入** | 1–2d                                                                               |
| **依赖** | INTG.IDENTITY.0 SSO 验收；Supabase RPC 或复用 Planner 已有 poll 逻辑的只读端点                |
...
## PORT.GROWTH.3: `default_app` 登录后自动跳转 {#g-p3}
...
| **投入** | 0.5d                                                                           |
| **依赖** | PORT.GROWTH.1 同批 DB 可读                                                              |
| **验收** | Settings 设 `planner` → 下次 Portal 登录直达 planner.kenos.space               |

---

## PORT.GROWTH.4: Portal「今日摘要」只读卡片 {#g-p4}
...
| **触发** | PORT.GROWTH.1–PORT.GROWTH.3 完成后；有明确「每天从 Portal 开始」习惯时                                                           |

**对标：** MyCapsul HTML tiles / Aura Today——**聚合视图，非聊天 Agent**。

---

## PORT.GROWTH.5: 六站 PWA 安装引导（从 Portal，含 Home） {#g-p5}
...
## HOME.PORTAL.1: Portal Launcher 加 Home 实验入口 {#h-p1}

| 项       | 说明                                                                                      |
| -------- | ----------------------------------------------------------------------------------------- |
| **状态** | ✅ 2026-07-09 — `PORTAL_APPS` 含 Home；独立「实验」区 + badge；`default_app` 仍仅四生产站 |
| **目标** | Launcher 增加 Home 卡（实验标签）；`rememberApp` / PORT.GROWTH.1 可跟踪 `home` 点击                |
| **投入** | 0.5–1d                                                                                    |
| **依赖** | `packages/theme` `LIFE_OS_SITE_META.home`；Portal 加 icon                                 |
| **验收** | Portal 可见 HOME.OS 卡 → 点击打开 `home.kenos.space`                                      |

**不做：** 把 Home 与四生产站同权重默认展示（除非 HOME.EXPER.0 升 tier）。

---

## HOME.SSO.2 / HOME.SSO.3: Home Integration（SSO + redirect）{#h-p2}
...
| **投入** | 1–2d（复用 INTG.IDENTITY.0 模式）                                                                |
| **依赖** | INTG.IDENTITY.0 四站 SSO 验收通过 ✅                                                             |
| **验收** | Portal 登录 → Home 设置页显示已登录 ✅；`core_user_app_settings` 有 `app_id=home` 行  |

Home 数据分层见 [`INTEGRATION.md`](./INTEGRATION.md#h-p0)：扫描/照片/追加事件均已在生产；object recognition schema 也已生产但代码仍待版本史闭环；完整可编辑 spatial 项目跨设备同步仍未完成。

## INTG.EVENTS.1b: Fitness 完练 → Planner 打卡（候选第二条事件）{#i-p15b}

| 项       | 说明                                                                  |
| -------- | --------------------------------------------------------------------- |
| **目标** | `fitness.workout_logged`（contracts 测试名）→ Planner 幂等 habit 任务 |
| **投入** | 3–5d                                                                  |
| **依赖** | 产品规则定稿（自动 vs 确认）；INTG.EVENTS.1.5 管道已通                         |
| **ROI**  | ○ 按需——**须有每天用 Fitness + Planner 的明确场景**                   |

**优于 INTG.EVENTS.2：** 具体链路、可验收，复用 outbox 模式。

---

## MUSC.CORE.1: Music `play_events` 生产闭环 {#m-p1}
...
## FINC.GROWTH.1: Finance 扩展同步反馈 UX {#f-p1}
...
| ID          | 主题                                   | 何时开                              |
| ----------- | -------------------------------------- | ----------------------------------- |
| **PORT.GROWTH.6**    | ⌘K 跨站深链 + 最近搜索                 | ✅ 2026-07-09                       |
| **FINC.CORE.2**    | 扩展增加 1 个券商/聚合源               | DOM 稳定且你实际在用                |
| **MUSC.PIPE.3**    | LLM 批量补标剩余 partial 曲目          | 推荐质量卡住时                      |
| **PLNR.CORE.1**    | 跨站推送策略对齐（Planner SW 为 SSOT） | PWA 通知成为痛点时                  |
| **HOME.PROJ.4**    | 完整可编辑 spatial 项目跨设备同步      | 先解决项目身份、冲突与云扫描映射；不要与已落地的 scan ingest 混为一谈 |
| **PORT.GROWTH.4b-M** | Portal Music 摘要卡（优先）            | ✅ 2026-07-09 · 见 POTENTIAL        |
| **PORT.GROWTH.4b-H** | Portal Home 储藏卡                     | ✅ 2026-07-09 · HOME.PROJ.6a               |

---

## PORT.GROWTH.4b: Portal 今日摘要扩卡 {#g-p4b}

研判 → [`POTENTIAL.md`](./POTENTIAL.md)。拆 **Music（先做）** 与 **Home（HOME.PROJ.6a 后）**。

### PORT.GROWTH.4b-M {#g-p4b-m}
...
| -------- | --------------------------- |
| **状态** | ✅ 已发货（2026-07-09）                                     |
| **目标** | 扩 `portal_today_summary()` + Home 第五卡；读 `core_user_app_settings` `portal_summary` |
| **投入** | ~1d（与 HOME.PROJ.6a 同批）                                        |
...
```text
Phase 5 — Portal 打磨 + Music 行为分
  PORT.GROWTH.8   pending → Planner inbox
  PORT.GROWTH.9   Portal smoke
  MUSC.PIPE.5   行为分验收

已完成（2026-07-09 四轮计划）
  FINC.CORE.3 · PORT.GROWTH.4b-M · PORT.GROWTH.6 · PLNR.CORE.2 · GYMS.CORE.0/GYMS.EVENTS.1 · INTG.EVENTS.1b · CI 接线 ✅

已完成（2026-07-09 早前）
  FINC.CORE.0 route smoke 22/22 ✅ · PLNR.CORE.2 前 PLNR.CORE.2 21/22 ✅ · INTG.IDENTITY.0 · PORT.GROWTH.4 · HOME.PORTAL.1/HOME.SSO.2/HOME.SSO.3 · PORT.GROWTH.2 · MUSC.CORE.1 · FINC.GROWTH.1
```

**已完成（2026-07-08）：** PORT.GROWTH.1 · PORT.GROWTH.3 · PORT.GROWTH.5 · CI-补 · QA-F0 — 见 [`SHIPPED.md`](./SHIPPED.md)。
