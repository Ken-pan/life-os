...
**桶定义：** 在 Core/Infra 闭环后，用**极小改动**放大已有基建（`core_*`、`life_events`、Portal、各 app RPC/扩展），提升「六 app 一体」体感。不做全站 AI；**Home 保持实验 tier**，Integration 按需（H-P1–H-P3）。
...
| 模式                    | 代表                                                                                                                | 对 Life OS 的启示                                                        |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| **统一入口 + 边缘 SSO** | [brainbot](https://github.com/slaguardia/brainbot) `#apps` launcher                                                 | Portal 读 `core_*`（G-P1–G-P3 ✅）；差的是 G-P4 摘要卡片与 Home 实验入口 |
...
| 项       | 说明                                                                                  |
| -------- | ------------------------------------------------------------------------------------- |
| **状态** | ✅ 代码 2026-07-08 — `PortalAppBar` 只读 pending 计数；🟡 Finance `bill_due` 生产验收 |
...
| 项       | 说明                                                                      |
| -------- | ------------------------------------------------------------------------- |
| **现状** | `home.kenos.space` 已部署；`PORTAL_APPS` 仅 planner/finance/fitness/music |
| **目标** | Launcher 增加 Home 卡（实验标签）；`rememberApp` / G-P1 未来可跟踪 `home` |
| **投入** | 0.5–1d                                                                    |
...
| **投入** | 1–2d（复用 I-P0 模式）                                                               |
| **依赖** | I-P0 四站 SSO 验收通过                                                               |
| **验收** | Finance 登录 → Home 免登；`core_user_app_settings` 有 `app_id=home` 行               |

云同步（H-P4）与工坊（H-P5）见 [`INTEGRATION.md`](./INTEGRATION.md#h-p0) §Parked。

## I-P1.5b: Fitness 完练 → Planner 打卡（候选第二条事件）{#i-p15b}

| 项       | 说明                                                          |
| -------- | ------------------------------------------------------------- |
| **目标** | `fitness.session_complete`（或等价）→ Planner 幂等 habit 任务 |
| **投入** | 3–5d                                                          |
...
| 项       | 说明                                                                    |
| -------- | ----------------------------------------------------------------------- |
| **状态** | 🟡 代码 2026-07-08 — `playEvents.js` + Queue reasons UI；待生产播歌验收 |
...
| 项       | 说明                                                                          |
| -------- | ----------------------------------------------------------------------------- |
| **状态** | 🟡 代码 2026-07-08 — popup 失败重试 + `ExtensionSyncBridge` toast；待生产验收 |
...
| **H-P5** | Home 工坊默认可用                      | spatial 产品定稿后     |

---

## 推荐顺序（与 hub 同步 · 2026-07-08 后）

```text
当前焦点 — 生产验收
  I-P0  SSO 生产 E2E
  M-P1  Music play_events + reasons
  F-P1  Finance 扩展同步反馈
  AppBrandSwitcher 六站侧栏切换

按需
  G-P2  Finance bill_due → Portal 角标（生产）
  H-P1  Portal Home 实验卡
  H-P2 + H-P3  Home SSO（每天用 Home 时可提前）
  G-P4  今日摘要卡片
  I-P1.5b  Fitness → Planner（有场景再开）
```

**已完成（2026-07-08）：** G-P1 · G-P3 · G-P5 · CI-补 · QA-F0 · M-P1/F-P1 代码 — 见 [`SHIPPED.md`](./SHIPPED.md)。
