...
| Apple native daily loop                 | 无 Kenos iOS/macOS 产品；仅 KenosContracts + 领域 companion | 单一 Kenos iOS/iPadOS + macOS daily client（shared contracts/client/store/actions/design）      | PARTIAL_PASS_NATIVE_FOUNDATION_READY_WITH_DISTRIBUTION_GATES；Mac Web Daily Beta READY；iOS Personal Daily Beta NOT READY（Auth+letterbox PASS；FLOW A/B 等 17 Pro 重连） | Phase 1–3 contracts；inventory；mock auth；offline queue；simulator QA；17 Pro Auth/letterbox/LAN smoke 2026-07-21         | Apple 不拥有 Task/Approval/Work/Activity 真源；canonical writes 仍走 Action boundary；iOS origin LAN-DEPENDENT | 移除 Apps/新 packages；Contracts 可保留；Mac Web fallback 保留 | production signing/OAuth/push/universal links；watchOS 产品；Executor；App Store；FLOW A/B 真机日常闭环 + a11y/Account B |
...
- **IOS PERSONAL DAILY BETA: READY** (LAN-DEPENDENT) on Ken’s 17 Pro.
