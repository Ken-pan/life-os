...
- Status: `PARTIAL_PASS_DISPOSABLE_DB_VERIFIED` — non-production contracts, server command handler, review-only SQL artifact, and tests are implemented; the SQL/auth/RLS/idempotency path passes a scoped disposable local database test. Production DB apply, caller cutover, deploy, and old-path deletion were not performed.
...
- Implemented artifacts: `packages/contracts/src/kenos.ts` defines Entity/Action/Capture/Approval/Activity/Outbox v1 schemas; `apps/planner/server/kenos/createTaskCommand.mjs` provides the server-side command boundary and testable transaction abstraction; `apps/planner/supabase/review/20260719010000_kenos_plan_create_task_command.sql` is deliberately outside the auto-applied migration chain and drafts idempotency/outbox/activity tables plus the RPC against the real `planner_tasks(user_id,id,data,updated_at)` shape; `apps/planner/supabase/tests/kenos_plan_create_task_command.sql` provides disposable-DB auth/RLS/idempotency checks; `scripts/check-kenos-phase1.mjs` validates static invariants.
- Acceptance covered: duplicate idempotency key returns the first Task; transaction failure rolls back Task/Outbox/Activity together in the test boundary; disposable SQL proves one canonical Task/Outbox/Activity result plus dual-user RLS/auth isolation and direct-write denial; Outbox lifecycle covers pending/processing/delivered/retry/terminal; retry uses capped exponential backoff with jitter; permanent errors become terminal; Activity redacts sensitive fields.
- Deferred production gate: repository/ops evidence confirms structured `planner_tasks(user_id,id,data,updated_at)` is the current canonical cloud Task table, with `planner_user_state` retained for settings/legacy backup. The review RPC now targets that real shape, but production apply remains blocked until owner approves the private-definer/public-invoker privilege model, RLS evidence, server caller integration, and writer cutover.
