...
- Status: `PARTIAL_PASS_GREEN_ARTIFACTS_READY` — non-production contracts, server command handler, migration artifact, and tests are implemented in this Cloud task. Production DB apply, RLS/auth changes, writer cutover, deploy, and old-path deletion were not performed.
...
- Implemented artifacts: `packages/contracts/src/kenos.ts` defines Entity/Action/Capture/Approval/Activity/Outbox v1 schemas; `apps/planner/server/kenos/createTaskCommand.mjs` provides the server-side command boundary and testable transaction abstraction; `apps/planner/supabase/migrations/20260719010000_kenos_plan_create_task_command.sql` drafts idempotency/outbox/activity tables and RPC; `scripts/check-kenos-phase1.mjs` validates the migration artifact invariants.
- Acceptance covered: duplicate idempotency key returns the first Task; transaction failure rolls back Task/Outbox/Activity together in the test boundary; Outbox lifecycle covers pending/processing/delivered/retry/terminal; retry uses bounded exponential backoff with jitter; permanent errors become terminal; Activity redacts sensitive fields.
- Deferred production gate: current production Planner cloud truth is `planner_user_state` JSON, not a dedicated `planner_tasks` server table. The SQL artifact therefore remains a review draft and must not be applied until owner approves the canonical server Task table/RPC mapping or an equivalent transaction strategy against `planner_user_state`.
...
- Cutover trigger: owner-approved production migration design, dry-run against a disposable database with the actual Planner schema, RLS review, dual-user authorization tests, and explicit approval to route Assistant/remote create-task traffic to the server RPC.
