...
**近程成功（日用闭环）：** 「东西在哪」可答（app + AIOS）；平面图/储藏够日常用；硬件线由 DEVICE.12 决定。

**Done when：**
1. `/storage` 物品可搜可定位；MCP `where_is` ✅（AIOS 可问）
2. 安静扫描 / 认亲记忆可用；refine LaunchAgent 由用户激活
3. 可编辑 spatial 以本地为真源仍可接受（完整项目云同步非近程必须）
4. `HOME.DEVICE.12`（装 HA 后）产出真实设备能力表，再决定 DEVICE.14 / VISION

**故意不做：** 无第二真实项目时多项目云同步（HOME.PROJ.7）；无 HA 时开 VISION/能耗大线；把 Home 默认升为四站同级。

**与底座：** SSO ✅ · Portal 储藏卡 ✅ · MCP where_is ✅ · object_ref 未做。
...
| **HOME.MCP.13**     | `/api/mcp` + `where_is`     | ✅   | Product | —      | （19）    | **已发货：** `home.storage_snapshots` + 防抖同步 · `/api/mcp` `where_is` 薄封装 `searchStorageItems`；AIOS 设置配 URL+JWT 即入 loop |
...
**建议顺序：** `HOME.DEVICE.12`（Ken 装 HA 后，半天摸底）→ `HOME.DEVICE.14` → `HOME.VISION.15` → `HOME.STORAGE.18` / `HOME.ENERGY.16` / `HOME.SCENE.17`（前面没跑通则无意义）。**HOME.MCP.13 `where_is` 已发货**，不占前置位。

### 为什么 `HOME.MCP.13` 曾排在硬件前面（已兑现）

`/storage` 清单 `HOME.STORAGE.19` 后已能在 app 内搜到并定位到平面图；**MCP.13 已接进 AIOS**，「登山包在哪」跨 app 可答。对照 AIOS.20/21：**新增跨 app 能力 = 写几个 tool handler**。硬件线仍由 DEVICE.12 gate。
...
| ~~**HOME.RECOG.1r**~~ | ~~认亲窄残余~~ ✅ 真机收口（2026-07-17 夜）：iPhone 17 Pro 连扫两遍，遥测坐实双模式开关（安静 0 引导 / 高精度启动）+ 高精度只引导 1-3 区（Ken 观感确认）+ 认回 6/9 + 最近件 3.7cm；质量摘要因用户「不看」按用量删除，改「只在有事时弹」（`ReviewView`） |
