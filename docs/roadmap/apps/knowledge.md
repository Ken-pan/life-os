...
**近程成功（日用闭环）：** Ken 日写进 Vault，日常不再依赖 Obsidian；Planner/Finance 备注能链回笔记。

**Done when：**
1. KnowledgeOS.app 为日写真源；收集箱 / 块编辑器 / library 稳定
2. `KNOW.VAULT.0` watcher：外部改 .md 后索引跟上（代码 ✅ · app 已重装 · **Ken 点验**）
3. RAG / recall 可用；local-ai 不可达时优雅降级
4. wikilink + `knowledgeos://` 深链被 Planner/Finance 消费 ✅
5. Vault **正文不上云**（隐私默认）

**故意不做：** watcher 未稳前 Vault 原文上云（KNOW.SYNC.1）；无痛点强上 `object_ref` / 关系图主航道。

**与底座：** SSO 读 Planner ✅ · wikilink/深链 ✅ · 无 MCP（检索走 local-ai）· object_ref 按痛点。
...
| **KNOW.VAULT.0** | 外部文件变更监听（路径配置拆后） | ✅ 代码 + **Agent 已 `npm run app:knowledge` 重装**；剩 Ken 点验 — [`know-vault-0-acceptance.md`](../../qa/know-vault-0-acceptance.md) |
...
| **KNOW.XREF.5** | 跨 OS 引用 / `object_ref` | 🟡 wikilink + **原生 deep-link（5n）已发**；`object_ref` 稳定契约仍未做 |
...
- Planner 任务备注里的 `[[wikilink]]` → 备注下渲染引用 chip（label 走 `|显示名`）；**主链 `knowledgeos://open?title=…`（原生 Vault）**，旁路「网页」→ `knowledge.kenos.space/library?title=…`。
- **契约不破**：不动 markdown、不动 blocks 模型；Planner 备注仍是纯 `<textarea>`（wikilink 只是从纯文本里解析出的引用，不是富文本编辑）。
- **KNOW.XREF.5n（2026-07-18）：** Tauri `deep-link` 登记 `knowledgeos` scheme；壳内 `bindKnowledgeDeepLinks` → `/library?title|note`。**需 `npm run app:knowledge`（或 tauri build）一次**后系统才认 scheme。
- **第二消费者（2026-07-18）：** Finance 采购备注同样解析 `[[wikilink]]` → 原生/网页双链。`object_ref` 稳定契约仍按需，不阻塞日用。
...
| 原生壳 | `src-tauri/` |
| 原生深链 | `src/lib/deepLink.js` · `tauri-plugin-deep-link` · `knowledgeos://` |
