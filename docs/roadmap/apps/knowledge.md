...
| **KNOW.XREF.5** | 跨 OS 引用 / `object_ref` | 🟡 双向检索试点已有；稳定契约见 hub §Next |
...
**决策 · 文档编辑器不做跨 app 共享（2026-07-17 审计）：** 排查 Knowledge vs Planner 的「文档编辑」重复度 ≈ 0。Knowledge 独有块状 WYSIWYG（`NoteEditor.svelte` 1515 行 + `editor/{blocks,inline,caret}.js` + `markdown.js`）；Planner 的备注/摘要全是裸 `<textarea>`，全库无 `renderMarkdown/contenteditable/@html`，两 app 零 import。二者解决的是不同问题（Knowledge=写文档，Planner=记任务元数据 + 外链引用如 `paperLinks`）。**结论：不抽共享编辑器包**（无重复可抽、且 Planner 备注刻意保持速记）。真正的复用杠杆是**跨 OS 引用**（`KNOW.XREF.5`）——让 Planner wikilink 到 Knowledge 笔记，复用已有 `extractWikilinks`/`resolveWikilink`，文档归 Knowledge 写、Planner 只引用。唯一有价值的小抽取候选：`extractWikilinks`（27 行）提到共享包供 Planner 备注「跳转到笔记」。
