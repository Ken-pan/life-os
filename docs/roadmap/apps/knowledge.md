...
**编辑器（2026-07-17）：** 块状所见即所得 + library/overview 重构已 **checkpoint 入仓**（`KNOW.EDITOR.7`）；后续 freeze/QA 提交继续打磨。unit / check 绿；可选补 library/editor 浏览器 smoke。

## Next（按 ROI）

| ID | 主题 | 备注 |
| --- | --- | --- |
| **KNOW.EDITOR.7** | 块编辑器稳定化 | ✅ checkpoint 入仓；可选 browser smoke |
| **KNOW.VAULT.0** | 外部文件变更监听（路径配置拆后） | **高 ROI / hub Now**：curator/Obsidian 写入免重启即现 |
