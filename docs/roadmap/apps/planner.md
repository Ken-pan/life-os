...
| MCP 面（AIOS 用） | ✅ 代码 | `/api/mcp`：`ping`/`today_agenda`/`list_tasks`/`add_task`/`complete_task`；Bearer=Life OS JWT + RLS；写 `planner_tasks.data` 与客户端 LWW 同源 — 见 PLNR.MCP.0 |
...
| **PLNR.MCP.0**       | Planner MCP 面（AIOS 推理内核）            | ◆◆  | Integration | ✅ 代码 · 用户 gate | Claude | 纯逻辑 unit ✅ · JSON-RPC 协议本地验 ✅（initialize/tools/list/ping/auth 守卫）；剩**真实 JWT 读写** + **AIOS 配 URL**（`https://planner.kenos.space/api/mcp` + Life OS token）| §Next |
...
| PLNR.MCP.0     | `netlify/functions/mcp.mjs`（`createMcpHandler` + Supabase JWT/RLS，config.path `/api/mcp`）· `server/mcpTasks.mjs`（纯：buildTask/select/complete，与 createTask 同形）· `server/mcpTasks.test.mjs` · dep `@life-os/mcp-server` + netlify.toml watch |
...
AIOS ──MCP /api/mcp (JWT+RLS)──► planner_tasks 读/建/完成 (PLNR.MCP.0)
```
