...
| --------------- | -------------------------------- | --- | -------- | ---- | -------------------------------- | ------------------------------------------------------ | ----- |
| **GYMS.SUB.5**  | 替代动作完整训练流               | 🔥  | Product  | 2–3d | **Codex** 实现 · Fable 短 review | 跳过选替代 → Focus 练替代；Summary/Coach/容量归因；E2E | §Now  |
| **GYMS.MEDIA.3** | 动作示意图                      | ○   | Product  | 2–3d | Fable                            | Focus 流可见缩略图                                     | —     |
| **GYMS.SYNC.4** | `SyncErrorPresentation` 契约对齐 | ○   | Platform | 1d   | Codex                            | PLAT.CORE.2                                            | —     |

**GYMS.SUB.5 现状缺口：** `SkipModal` + `skipExercise(substituteId)` 仅**记录**替代 ID，未插入 Focus 训练流；周容量与 Coach 未按替代动作计组。程序数据里部分动作已有 `alternatives[]`（`program.js` / `exercises.js`），需补全覆盖 + 运行时切换。
