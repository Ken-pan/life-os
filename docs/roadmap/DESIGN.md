...
像素基线随后继续扩容（新增 overlay/forms showcase + 393×852 mobile 基线；
...
当日阶段验收曾到 smoke 405 / a11y 48 / canonical 202 项全绿。
...
**2026-07-17 代码矩阵快照：** 注册表已覆盖九个产品品牌（新增 HealthOS / KnowledgeOS），
图表 categorical 色板改为按各 app accent 经 OKLCH 演算，树状图按块亮度自动选择深/浅墨色。
Playwright `--list` 当前收集 **922 smoke / 147 a11y / 524 visual**；这些数字表示代码矩阵规模，
不替代实际 CI 通过结论。对应提交：`9bb72a414` · `fbd690611` · `37b24de6b` ·
`3feff760d` · `2f05e3fa1`。

**已知静态告警（2026-07-17 HealthOS build 复核）：** build 成功，但 Svelte 编译器仍对共享
Bar/Line/Heatmap/Timeline/MindMap/Treemap/Donut 图表报 SVG role / tabindex / pointer handler
a11y warnings。它们不是 HealthOS 业务错误，也不是“全绿”；应在共享 `platform-web` 图表层集中修复。
