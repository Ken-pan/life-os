...
> **状态：已归档研判。** 本文保留当时的 ROI 证据与决策过程，不再代表当前排序。当前 Now / Next 只看 [`../LIFEOS_ROADMAP.md`](../LIFEOS_ROADMAP.md)；**2026-07-10 复核**后产品优先级为 **PLNR.SCHED.0 → PAPR.DATA.verify / PAPR.UI → GYMS.SUB.5**（legacy：`PAPR.DATA.verify`）；Agent 路由见 [`AGENT_WORKSTREAMS.md`](./AGENT_WORKSTREAMS.md)。
