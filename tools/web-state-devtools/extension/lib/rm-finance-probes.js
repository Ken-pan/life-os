...
(() => {
  ...
  function parseAbbrevMoney(text) {
    ...
    const mult = m[3].toLowerCase() === "k" ? 1e3 : m[3].toLowerCase() === "m" ? 1e6 : 1;
    return {
  ...
  function probeNetWorthAccounts() {
    ...
    for (const rowEl of document.querySelectorAll('[role="row"]')) {
      ...
      let money = null;
      for (let i = [...rowEl.querySelectorAll("label")].length - 1; i >= 0; i--) {
    ...
    for (const section of document.querySelectorAll('[data-testid="subscription-section-card"]')) {
      ...
      for (const rowEl of section.querySelectorAll('[data-testid="table-row"]')) {
        ...
        rows.push({
          ...
          platformId: menu?.getAttribute("aria-controls")?.replace(/^recurring-list-/, ""),
        });
      }
    }
    return rows.length > 0 ? rows : null;
