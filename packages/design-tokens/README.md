...
tokens/structural.json         布局 / motion / chrome / toast 结构 token（flat key → --var）
...
packages/theme/src/generated/tokens.css
        ↓ import
@life-os/theme/design-system.css  （含 generated/component.css + tokens re-export）
        ↓
...
| `commandPalette` | `--command-palette-*` | CommandPalette panel + items                            | ✅         |
| `portraitGate`   | `--portrait-gate-*`   | `portrait-gate.css`                                     | ✅         |
...
输出：`docs/ui-qa-screenshots/design-catalog/p3-audit/{runId}/`（本地，不提交）

## Toast 结构 token（generated structural.json）

位于 `tokens/structural.json` → `generated/tokens.css`：`--toast-pad-*`、`--toast-min-h`、`--toast-line-height`、`--toast-action-gap` 等。语义色仍走 `component.json` → `feedback.*`。

## 命令

```bash
npm run build:tokens      # tokens JSON → packages/theme/src/generated/*.css
npm run validate:tokens   # refs / 契约 / 重复 key / generated CSS staleness
```
