...
import { isAppLocale, type AppLocale } from '../i18n/types.js'
import { sanitizePortfolioAllocationTarget } from './sanitizePortfolioAllocation.js'
export { sanitizePortfolioAllocationTarget } from './sanitizePortfolioAllocation.js'
import {
  sanitizeFundAllocations,
  sanitizeUnderlyingAllocation,
} from '../engine/portfolioAllocation.js'
import {
  BASELINE_SCENARIO_ID,
  DATA_VERSION,
  defaultAssumptions,
} from '../defaults.js'
import type { FlowType, Txn } from '../engine/transactions.js'
import type { ExpectedOccurrence, OccurrenceState } from '../engine/timeline.js'
import { purchaseEnrichmentFromRow } from '../engine/purchaseEnrichment.js'
import type { SupabaseClient } from '@supabase/supabase-js'

let _supabase: SupabaseClient | null = null

/** Bind the app Supabase client before calling any repo function. */
export function bindFinanceSupabase(client: SupabaseClient): void {
  _supabase = client
}
...
function supabase(): SupabaseClient {
  if (!_supabase) {
    throw new Error(
      'Finance repo not bound — call bindFinanceSupabase(supabase) first',
    )
  }
  return _supabase
}

import { SB } from '../supabase-tables.js'
...
function isScenariosTableMissing(
  error: { message?: string } | null | undefined,
): boolean {
  return Boolean(error?.message?.includes('public.finance_scenarios'))
}

function isHoldingsTableMissing(
  error: { message?: string } | null | undefined,
): boolean {
  const msg = error?.message ?? ''
  return (
    msg.includes('public.finance_holdings_snapshots') ||
    msg.includes('public.finance_holding_positions')
  )
}

function isPortfolioTargetColumnMissing(
  error: { message?: string } | null | undefined,
): boolean {
  return (error?.message ?? '').includes('portfolio_allocation_target')
}

function isFundAllocationsColumnMissing(
  error: { message?: string } | null | undefined,
): boolean {
  return (error?.message ?? '').includes('fund_allocations')
}

function isUnderlyingAllocationColumnMissing(
  error: { message?: string } | null | undefined,
): boolean {
  return (error?.message ?? '').includes('underlying_allocation')
}

function isPurchaseEnrichmentColumnMissing(
  error: { message?: string } | null | undefined,
): boolean {
  return /purchase_enrichment|column.*does not exist/i.test(
    error?.message ?? '',
  )
}
...
function scenarioFromRow(r: Row): Scenario {
  return prune({
    ...
    scenarioType:
      (ostr(r.scenario_type) as Scenario['scenarioType']) ?? 'custom',
...
function decisionRecordFromRow(r: Row): DecisionRecord {
  return prune({
    ...
    expectedOutcomeJson:
      (r.expected_outcome_json as Record<string, unknown>) ?? undefined,
    actualOutcomeJson:
      (r.actual_outcome_json as Record<string, unknown>) ?? undefined,
...
export function validateFinancialBackupPayload(
  ...
  if (p.schemaVersion === BACKUP_SCHEMA_VERSION) {
    if (p.scenarios != null && !Array.isArray(p.scenarios))
      errors.push('scenarios 必须是数组')
    ...
    if (
      p.expectedOccurrences != null &&
      !Array.isArray(p.expectedOccurrences)
    ) {
...
function accountFromRow(r: Row): Account {
  return prune({
    ...
    underlyingAllocation: sanitizeUnderlyingAllocation(
      r.underlying_allocation as
        | import('../types').AccountUnderlyingSlice[]
        | null
        | undefined,
    ),
    updatedAt: ostr(r.updated_at),
...
function accountToRow(userId: string, a: Account): Row {
  return {
    ...
    fund_allocations:
      a.fundAllocations && a.fundAllocations.length > 0
        ? a.fundAllocations
        : null,
...
function holdingsSnapshotFromRow(r: Row): HoldingsSnapshot {
  return prune({
    ...
    reconciliationStatus:
      ...
      ) as HoldingsSnapshot['reconciliationStatus']) ?? 'incomplete',
...
export async function loadFinanceData(): Promise<FinanceData | null> {
  ...
  if (!scenariosUnavailable && scenarios.length === 0) {
    ...
    const ins = await supabase()
      .from(SB.finance.scenarios)
      .insert(scenarioToRow(userId, baseline))
    if (!ins.error) scenarios = [baseline]
  }
  const ensuredScenarios =
    scenarios.length > 0 ? scenarios : [baselineScenario()]
  const preferredActive =
    ostr(settings.active_scenario_id) ?? ensuredScenarios[0].id
  const activeScenarioId = ensuredScenarios.some(
    (s) => s.id === preferredActive,
  )
    ? preferredActive
  ...
  const holdingsUnavailable =
    isHoldingsTableMissing(hs.error) || isHoldingsTableMissing(hp.error)
  ...
  if (!holdingsUnavailable) {
    for (const row of ((hp.data as Row[]) ?? []).map(holdingPositionFromRow)) {
      if (!positionsBySnapshot.has(row.snapshotId))
        positionsBySnapshot.set(row.snapshotId, [])
  ...
  const holdingsSnapshots = holdingsUnavailable
    ...
    : ((hs.data as Row[]) ?? []).map((row) => {
        ...
        snapshot.positions = (positionsBySnapshot.get(snapshot.id) ?? [])
          .slice()
          .sort((a, b) => b.marketValue - a.marketValue)
        return snapshot
      })
  const portfolioAllocationTarget = sanitizePortfolioAllocationTarget(
    settings.portfolio_allocation_target as
      | PortfolioAllocationTarget
      | null
      | undefined,
  )
  ...
  return {
    version:
      settings.data_version != null
        ? Number(settings.data_version)
        : DATA_VERSION,
    ...
    locale: isAppLocale(ostr(settings.locale))
      ? (ostr(settings.locale) as AppLocale)
      : 'zh-CN',
  }
}
...
export async function seedFinanceData(data: FinanceData): Promise<void> {
  ...
  jobs.push(
    supabase()
      .from(SB.finance.scenarios)
      .upsert(scenarios.map((s) => scenarioToRow(userId, s))),
  )
      ...
      supabase()
        ...
        .upsert(
          data.events.map((e) =>
            eventToRow(userId, {
              ...e,
              scenarioId: e.scenarioId ?? activeScenarioId,
            }),
          ),
        ),
    )
  if (data.goals.length)
    jobs.push(
      supabase()
        ...
        .upsert(data.goals.map((g) => goalToRow(userId, g))),
    )
...
function isLocaleColumnMissing(error: unknown): boolean {
  ...
  return (
    code === 'PGRST204' ||
    /could not find.*locale|locale.*schema cache|column.*does not exist/i.test(
      msg,
    )
  )
}
...
export async function upsertAccount(a: Account): Promise<void> {
  ...
  if (error) {
    if (
      isFundAllocationsColumnMissing(error) ||
      isUnderlyingAllocationColumnMissing(error)
    ) {
      ...
      if (isUnderlyingAllocationColumnMissing(error))
        delete row.underlying_allocation
...
export async function upsertHoldingsSnapshot(
  ...
  const upsertSnapshot = await supabase()
    .from(SB.finance.holdingsSnapshots)
    .upsert(holdingsSnapshotToRow(userId, snapshot), {
    ...
    const ins = await supabase()
      ...
      .insert(
        snapshot.positions.map((p) =>
          holdingPositionToRow(userId, snapshot.id, p),
        ),
      )
...
export async function loadHoldingDailyCandleState(symbols: string[]): Promise<{
  ...
  await mapWithConcurrency(
    normalizedSymbols,
    HOLDING_DAILY_READ_CONCURRENCY,
    async (symbol) => {
      try {
        ...
        if (isHoldingDailyCandleTableMissing(error as { message?: string }))
          return
        throw error
      }
    },
...
export async function upsertHoldingDailyCandles(
  ...
  for (const point of points) {
    ...
    if (!symbol || !date || !Number.isFinite(point.close) || point.close <= 0)
      continue
...
export async function upsertEvent(
  ...
  const { error } = await supabase()
    .from(SB.finance.scenarioEvents)
    .upsert(eventToRow(userId, { ...e, scenarioId }), {
      onConflict: 'user_id,id',
    })
  if (error) throw error
}
...
export async function undoLatestScenarioApply(): Promise<UndoScenarioApplyResult> {
  ...
  if (!row)
    throw new Error('undo_latest_scenario_apply_v1 returned empty response')
...
function txnFromRow(r: Row): Txn {
  ...
  const inSpending =
    r.include_in_spending_analytics != null
      ? Boolean(r.include_in_spending_analytics)
      : Boolean(r.in_spending)
  const inCashFlow =
    r.include_in_cash_flow_history != null
      ? Boolean(r.include_in_cash_flow_history)
      : Boolean(r.in_cash_flow)
  return {
    id: ostr(r.id),
    date,
...
async function writeTxnInsert(
  userId: string,
  t: Partial<Txn> & { date: string },
): Promise<Txn> {
  ...
  if (
    error &&
    isPurchaseEnrichmentColumnMissing(error) &&
    row.purchase_enrichment != null
  ) {
    console.warn(
...
export async function insertTxns(
  ...
  for (let i = 0; i < items.length; i += TXN_INSERT_CHUNK) {
    ...
    if (
      error &&
      isPurchaseEnrichmentColumnMissing(error) &&
      rows.some((r) => r.purchase_enrichment != null)
    ) {
      console.warn(
...
export async function finalizeTransactionImport(
  ...
  const { data, error } = await supabase().rpc(
    'finalize_transaction_import_v1',
    {
      payload,
    },
  )
...
/** 加载余额断言，按账户 + 日期降序。 */
export async function loadBalanceAssertions(
  accountId?: string,
...
function occurrenceFromRow(r: Row): ExpectedOccurrence {
  return {
    ...
    varianceDays:
      onum(r.variance_days) != null ? Number(r.variance_days) : undefined,
  }
}
...
/** 批量 upsert 预期条目（按 id）。 */
export async function upsertExpectedOccurrences(
  ...
  const { error } = await supabase()
    .from(SB.finance.expectedOccurrences)
    .upsert(
      rows.map((r) => occurrenceToRow(userId, r)),
      { onConflict: 'id' },
    )
  if (error) throw error
}
...
export async function updateOccurrenceState(
  ...
  if (patch.varianceAmount !== undefined)
    row.variance_amount = patch.varianceAmount
  if (patch.varianceDays !== undefined) row.variance_days = patch.varianceDays
  if (patch.reconciledPeriodId !== undefined)
    row.reconciled_period_id = patch.reconciledPeriodId
  const { error } = await supabase()
    .from(SB.finance.expectedOccurrences)
    .update(row)
    .eq('user_id', userId)
...
export async function exportFinancialBackup(): Promise<FinancialBackupPayload> {
  const userId = await requireUserId()
  const [acc, cf, ev, gl, st, txns, sc, hs, hp, dr, ba, occ] =
    await Promise.all([
      supabase().from(SB.finance.accounts).select('*').eq('user_id', userId),
      supabase().from(SB.finance.cashFlows).select('*').eq('user_id', userId),
      supabase()
  ...
  const holdingsSnapshots = hs.error
    ...
    : ((hs.data as Row[]) ?? []).map((row) => {
        const snapshot = holdingsSnapshotFromRow(row)
        snapshot.positions = (
          positionsBySnapshot.get(snapshot.id) ?? []
        ).slice()
        return snapshot
      })
...
export async function deleteAllFinancialData(): Promise<DeleteFinancialDataResult> {
  ...
  if (
    error &&
    /delete_all_financial_data_v2|does not exist/i.test(
  ...
  if (error) throw error
  const deleted = ((data as { deleted?: Record<string, number> } | null)
...
export async function restoreFinancialBackup(
  ...
  if (
    ...
    /restore_finance_backup_v2|does not exist/i.test(
      String(error.message ?? ''),
    )
  ) {
    const fallback = await supabase().rpc('restore_finance_backup_v1', {
      payload,
    })
    data = fallback.data
