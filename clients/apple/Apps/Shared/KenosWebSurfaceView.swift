...
enum KenosSharedWebAuth {
    ...
    private static let secureStore: KenosSecureStore = SecItemSecureStore(
        service: "space.kenos.app.ios.webAuth"
    )
...
struct KenosDailyBetaSurface: View {
    ...
    var body: some View {
        ...
        ZStack {
            ...
            if unreachable {
                    ...
                    VStack(spacing: 8) {
                        Text(
                            KenosDailyBetaConfig.isLanDependentOrigin
                                ? "Mac Daily Beta is offline. Use Production (cellular) or start companions on the same Wi‑Fi."
                                : "Kenos shell origin did not respond. Check network, then Retry."
                        )
                ...
                KenosWebSurfaceView(
                    url: url,
                    onTitle: { _ in shellDidPaint = true },
    ...
    private func probeInBackground() async {
        ...
        do {
            ...
            if KenosDailyBetaConfig.useProductionOverride || !KenosDailyBetaConfig.isLanDependentOrigin {
                ...
                {
                    await MainActor.run { KenosWebRuntime.dailyBetaReachable = true }
                    return
                }
            }
            await MainActor.run {
    ...
    private func handleShellProbeFailure() async {
        ...
        await MainActor.run { unreachable = true }
    }
}
#else
import Foundation
