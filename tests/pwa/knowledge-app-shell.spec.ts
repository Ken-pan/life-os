...
test.describe('KNOWLEDGE.OS app shell', () => {
  ...
  test('single content scroll root on mobile', async ({ page }) => {
    ...
    const metrics = await page.evaluate(() => {
      const main = document.querySelector('[data-testid="knowledge-shell-main"]')
    ...
    await expect(page.getByTestId('knowledge-shell-navigation-mobile')).toBeVisible()
    await expect(page.getByTestId('knowledge-shell-navigation-desktop')).toBeHidden()
  })

  test('desktop/mobile breakpoint transition is stable', async ({ page }) => {
    await page.setViewportSize({ width: 1280, height: 800 })
    await page.goto('/', { waitUntil: 'networkidle' })
    const desktopNavigation = page.getByTestId('knowledge-shell-navigation-desktop')
    const mobileNavigation = page.getByTestId('knowledge-shell-navigation-mobile')
